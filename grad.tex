...
Moreover, the same conclusion holds for a locally Lipschitz $\lambda$-concave subfunction $f\:\spc{Z}\subto \RR$ if for any $t\in[0,t_{\max})$ there is a geodesic $[\alpha(t)\,\beta(t)]$ in $\Dom f$.
\end{thm}
...
For example, consider the concave functions $f\:\RR\to\RR$, $f(x)=-|x|$. The 
...
%??? A: We need a convergence theorem, which would work for radial curves as well???
...
Suppose that $\spc{Z}$ is either $\Alex{}$ or $\CAT{}$.
For any $t\ge 0$, we write $\GF^t_f(x)=y$ if there is a $f$-gradient curve $\alpha$ such that $\alpha(0)=x$ and $\alpha(t)=y$.
...
Recall that a line in a metric space is a both-side infinite geodesic; thus it minimizes the length on each segment.
...
Jost-Hinrich Eshenburg obtained an analogous result for  Lorentzian manifolds \cite{eshenburg-split}, that is, pseudo-Riemannian manifolds of signature $(1,n)$.
...
$\spc{L}'=\bus_+^{-1}(0)=\bus_-^{-1}(0)}$ is a closed convex subset of $\spc{L}$, and therefore forms an Alexandrov space.
Consider the map $h\:\spc{L}'\times \RR\to \spc{L}$ defined by $h\:(x,t)\mapsto \GF^t(x)$.
Note that $h$ is onto.
Applying distance estimate \ref{thm:dist-est} for $\GF_{\bus_+}^t$ and $\GF_{\bus_-}^t$ shows that $h$ is short and non-contracting at the same time; that is, $h$ is an isometry.

Next, let $p=\gamma(0)$.
For any point $x$ pick a shortest geodesic $[px]$. For any $t>0$ we have $\mangle\hinge p x {\gamma(t)} \ge \angk{0} p{x}{\gamma(t)}$ and  $\mangle\hinge p x {\gamma(-t)} \ge \angk{0} p{x}{\gamma(-t)}$.
Therefore 
...
Now consider a solution of the  differential equation for  $\tau(t)$, $\tau'=(\tg\kappa\tau)^2$, $\tau(0)=r$. 
...
Let us define the \index{gradient exponent}
%\emph{gradient exponent} as
%*S. note chang in terminology
 \emph{gradient exponential map} as 
\[
...
This type of flow was studied by Chanyoung Jun \cite{jun-thesis,jun:grad}, by Lucas C. F. Ferreira and Julio C. Valencia-Guevara \cite{ferreira-valencia}, and by Alexander Mielke, Riccarda Rossi, and Giuseppe Savar\'{e} \cite{mielke-rossi-savare}.
