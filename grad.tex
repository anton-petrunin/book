...
Next, let $p=\gamma(0)$.
For any point $x$ pick a shortest geodesic $[px]$. For and $t>0$ we have that  $\mangle\hinge p x {\gamma(t)} \ge \angk{0} p{x}{\gamma(t)}$ and  $\mangle\hinge p x {\gamma(-t)} \ge \angk{0} p{x}{\gamma(-t)}$.
Therefore $\pi=\mangle\hinge p x {\gamma(t)}+\mangle\hinge p x {\gamma(-t)}\ge \angk{0} p{x}{\gamma(t)}+\angk{0} p{x}{\gamma(-t)}$. Also, both $t\mapsto \angk{0} p{x}{\gamma(t)}$ and $t\mapsto \angk{0} p{x}{\gamma(-t)}$ are non-increasing. On the other hand, the condition $\bus_+(x)+\bus_-(x)= 0$ means that 
\[
