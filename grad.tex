...
A curve $\hat\alpha\:[s_{\min},s_{\max})\to\Dom f$ is $f$-gradient-like if and only if it is $1$-Lipschitz and
...
\[\alpha(t)=
\begin{cases}
(x-t,y-t) &\text{for}\quad 0\le t\le  y,
\\
(x-t,0) &\text{for}\quad y\le t\le  x,
\\
...
Let us define \index{integral $\kappa$-tangent}\emph{integral tangent} \index{$\itg\kappa$ (integral $\kappa$-tangent)} 
...
Let $\spc{L}$ be a complete length $\Alex{\kappa}$ space, and let
...
let $p, q\in \spc{L}$,
and let $\sigma\:[s_{\min},\infty)\to \spc{L}$ be a $(p,\kappa)$-radial curve.
Then for any $w\ge 1$, 
the function
\[
...
Similarly, \ref{eq:tangle-radcomp} follows from  $(*)\mc0_\phi$ and $(**)\mc0_\phi$.
...
\parit{Remark.}
Alternatively, we could redefine $\cos \psi$ and $\cos \phi$ via 
...
By swopping $\rho$ and $\sigma$, we obtain the second inequality in $(*)\mc\pm_\ell$.
...
Let $\spc{L}$ be a complete length $\Alex{\kappa}$ space, 
let $p\in \spc{L}$, 
and let $\xi\in \Sigma_p$.
Consider a sequence of points $x_n\in \spc{L}$ such that $x_n\to p$ and $\dir p{x_n}\to \xi$ as $n\to\infty$.
...
The \index{gradient exponential map}\emph{gradient exponential map} 
$\gexp\mc\kappa_p\:\cBall[\0,\tfrac{\varpi\kappa}2]_{\T_p}\to \spc{L}$
is defined by
\[
\gexp\mc\kappa_p\: r\cdot\xi\mapsto\sigma_\xi(r).
\]
\index{$\gexp\mc\kappa_p$ (gradient exponent)}
...
Let $\spc{L}\subset\EE^2$ be the Euclidean half-plane. 
...
%??? this exexercise use the notion of boundary --- it is good for next vol.
%\begin{thm}{Exercise}\label{ex:bry-cover}
%Let $\spc{L}$ be an $m$-dimensional complete length $\Alex\kappa$ space. Writing $\rad\spc{L}=R$,  prove that there is a $(\sn\kappa R)$-Lipschitz map $\map\:\mathbb{S}^{m-1}\to\spc{L}$ such that $\Im\map\supset\partial\spc{L}$.
%\end{thm}
%\parbf{\ref{ex:bry-cover}.} 
%Choose a regular point $p\in\spc{L}$.
%Observe that $\gexp_p^\kappa(\cBall[\0,R]_{\T_p})\z=\spc{L}$.
%
%Use \ref{ex:inv-gexp} to show that $\gexp_p^\kappa(\partial\cBall[\0,R]_{\T_p})\supset\partial\spc{L}$ and apply \ref{thm:prop-gexp:short}.
...
Consider a sequence  $a\z=t_0<t_1<\dots<t_n\z=b$, and a piecewise constant family of functions on $B(p,\eps\cdot L)$ defined by $\hat f_t=f_{t_i}$ if $t_i\le t<t_{i+1}$.
...
Let $\spc{Z}$ be a complete length space, and let $f\in\LSCSC(\spc{Z})$.
Suppose that $\spc{Z}$ is either $\Alex{}$ or $\CAT{}$.
...
\begin{thm}{Example}
Let $\spc{X}=L^2(\R^n)$, and let $F\co \spc{X}\to \R$ be given  by $F(f)=-\int|\nabla f|^2d\mathcal L$.
Then $F$ is a concave but not locally Lipschitz functional on $\spc{X}$ and it is finite precisely at functions $f\in W^{1,2}(\R^n)\subset L^2(\R^n)$.
