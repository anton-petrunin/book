...
In $\Alex{}$ spaces, it was first used by in  \cite{perelman-petrunin:qg}.
...
Assume $\spc{L}$ is a complete $\Alex\kappa$ space
...
there is an $f$-gradient-like curve $\hat\alpha\:[0,\eps)\to\spc{L}$ which starts at $p$ (that is $\hat\alpha(0)=p$);
\end{subthm}
...
Further generalizations of Splitting theorem for Riemannian manifolds with nonnegative Ricci curvature were obtained by Cheeger and Gromoll in \cite{cheeger-gromoll-split}.
Eshenburg in \cite{eshenburg-split} obtained an analogous result for the Lorentzian manifolds, that is, pseudo-Riemannian manifolds of signature $(1,n)$.

We present a proof which use gradient flow for Busemann's functions, it is close in the spirit to the proof in \cite{cheeger-gromoll-split}.
...
\parit{Proof.} Consider two Busemann's functions $\bus_+$ and $\bus_-$ asociated with rays $\gamma:[0,\infty)\to \spc{L}$ and $\gamma:(-\infty,0]\to \spc{L}$; that is,
\[
\bus_\pm(x)
=
