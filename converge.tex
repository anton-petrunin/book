...
%Therefore if $\dist{A_n}{x}{}\to\infty$ for some (and therefore any) $x\in \spc{X}$, then we can assume that the Hausdorff limit of $A_n$ is the empty set.
...
they were designed to be used in very specific problems.

The ultra-limits,
another important type of convergence,
...
Both concepts quickly spread into all branches of geometry.

The definition \ref{def:hausdorff-coverge} was given by Zdeněk Frolík \cite{frolik} %CHECK
and was rediscovered by Robert Wijsman \cite{wijsman}.

This definition is a slight modification of the convergence for subsets of metric space,
first introduced by Felix Hausdorff \cite{hausdorff}%CHECK
, two years later the same convergence was introduced by Wilhelm Baschke \cite{blaschke}. 
Since Blaschke does not refer to Hausdorff; it is likely that this type of convergence was used in mathematical folklore before these publications. 
