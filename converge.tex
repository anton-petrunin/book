...
Gromov--Hausdorff convergence is also defined in a nonstandard way. %V: What does that mean???
...
A metric space $\bm{X}$
is called \index{common space}\emph{common space} of $\set{\spc{X}_\alpha}{\alpha\in\IndexSet}$ if its underlying set is formed by the disjoint union $$\bigsqcup_{\alpha\in\IndexSet} \spc{X}_\alpha$$ 
...
The existence of  such a sequence $M_1<M_2<\dots$ follows from the main assumption of the theorem.
...
%$\dist{f(x)}{f(x')}{\spc{Y}}\lege\dist{x}{x'}{\spc{X}}\pm\eps$ for any $x,x'\in\spc{X}$. %V: the notation $\lege$ is not really standard. It should be defined if introduced but it might be easier just not to use it.
\end{subthm}
