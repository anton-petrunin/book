...
%It seems that 
To the best of our knowledge, Hausdorff convergence of subsets of a fixed metric space was first introduced by Felix Hausdorff \cite[VIII § 6]{hausdorff},
and a couple of years later an equivalent definition was given by Wilhelm Blaschke \cite[§ 17(IV)]{blaschke}.
...
Let $\bm{X}$ be a proper common space for proper metric spaces
...
Note also that any sequence of proper metric spaces admits trivial convergence to the empty space.
Therefore, a constant sequence $\spc{X}_n\iso\spc{X}$ of a nonempty proper space $\spc{X}$ has at least two distinct limit spaces: $\emptyset$ and $\spc{X}$.
Furthermore, if one removes the properness of the common space from the definition of convergence (\ref{def:GH}), then any closed subset of a proper metric space $\spc{X}$ may occur as its limit under an appropriate convergence of the constant sequence $\spc{X}_n\iso\spc{X}$.
For example, let $\bm{X}$ be the set $\{x_\infty,x_1,y_1,x_2,y_2,\dots\}$ with a metric such that $\dist{x_n}{y_n}{}=1$ for all $n$,
$\dist{y_n}{y_m}{}=2$ for all $n\ne m$ and
$x_n\to x_\infty$ as $n\to\infty$.
Note that the subsets $\{x_n,y_n\}$ converge to $\{x_\infty\}$ in the sense of Hausdorff.
This defines a convergence of a sequence of two-point spaces (with the two points at distance 1) to a one-point space.
As shown in \ref{thm:GH-compact}, this is impossible if $\bm{X}$ is proper.
...
Let $\spc{X}_1,\spc{X}_2,\dots$, let $\spc{X}_\infty$ be compact metric spaces, and let $\eps_n\to\0+$ as $n\to\infty$.
Suppose that either 
...
\begin{thm}{Theorem} Let $\spc{X}_1,\spc{X}_2,\dots$, and let $\spc{X}_\infty$ be nonempty compact metric spaces.
...
Assume that $\spc{X}_\infty$ is nonempty, and $\bar{\spc{X}}_\infty$ is compact.
...
$\GH$ and $\GH'$.
Assume 
$\spc{X}_n\xto{\GH}\spc{X}_\infty$ and $\spc{X}_n\z{\xto{\GH'}}\spc{X}_\infty'$.
...
and $\spc{X}_\infty$ is compact and nonempty, then
$\spc{X}_\infty$ is isometric to $\spc{X}_\o$.
\end{subthm}

\begin{subthm}{thm:ultra-GH:b}
If $\spc{X}_n\GHto \spc{X}_\infty$ 
and $\spc{X}_\infty$ is proper and nonempty, then
