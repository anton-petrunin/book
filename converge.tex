...
one might think that $\spc{Y}+\eps$ is a metric space $(\ushort{\spc{Y}},\dist{}{}{}+\eps\cdot \rho)$, where $\rho$ is a discrete metric on $\ushort{\spc{Y}}$ (that is, $\rho(x,y)=1$ if $x\not=y$). 
...
The Gromov--Hausdorff convergence was first introduced by David Edwards in \cite{edwards}
and rediscovered later by Mikhael Gromov in \cite{gromov-polynomial-growth}.
