...
A cubical complex can be defined as a subcomplex 
of the unit cube in the Euclidean space;
that is, as a subset of faces of a cube $[0,1]^{\times N}$ in $ \RR^N$
...
The union of all cubes in a cubical complex $\spc{K}$ will be called its \emph{underlying space};
we will denoted as $\spc{K}$ or as $\ushort{\spc{K}}$ to emphasise that it is a set, not a complex.
A homeomorphism from $\ushort{\spc{K}}$ to a topological space $\spc{X}$ is called \emph{cubulation of} $\spc{X}$.

The underlying space of a cubical complex $\spc{K}$ will be always considered with the length metric
induced from $\RR^N$.
In particular, with this metric, 
each cube of $\spc{K}$ is isometric to the unit cube of the same dimension.

It is straightforward to construct a triangulation of $\ushort{\spc{K}}$,
so that each simplex is isometric to a Euclidean simplex.
In particular $\ushort{\spc{K}}$ is a polyhedral space.

Further, the link of each vertex admits a triangulation 
such that each simplex is isometric to the right-angled spherical simplex of the same dimension.

\parbf{Cubical analog for a simplicial complex.}
...
For each $k$-dimesional simplex $\<v_{i_0},\dots,v_{i_k}\>$ of $\spc{S}$,
mark the all the $(k+1)$-dimensional faces 
of the unit cube $[0,1]^{N}\subset\RR^N$ 
which are parallel to the coordinate $(k+1)$-plane 
spanned by $e_{i_0},\dots,e_{i_k}$.

Note that the set of all marked faces of $[0,1]^{N}$
forms a cubical complex. 
The obtained cubical complex will be called \index{cubical analog}\emph{cubical analog} of $\spc{S}$
and it will be denoted as $\square_\spc{S}$.

The following proposition follows from the construction above.

\begin{thm}{Proposition}
Let $\spc{S}$ be a simplicial complex
and $\square_{\spc{S}}$ be its cubical analog.
Then link of any vertex of $\square_{\spc{S}}$
...
\begin{thm}{Corollary}\label{cor:flag-cube}
If  $\spc{S}$ is a flag complex then the universal cover $\tilde\square_{\spc{S}}$ of the underlying space of its cubical analog is a $\cCat{}{0}$ space.
\end{thm}


\parit{Proof.}
According to Flag condition (\ref{thm:flag}), 
the link of any cube in $\square_{\spc{S}}$ is a $\cCat{}{1}$ space.

By cone construction (\ref{thm:warp-curv-bound:cbb:a})
$\square_{\spc{S}}$
is loally $\cCat{}{0}$ space.
It remains to apply Hadamard--Cartan theorem 
\ref{thm:hadamard-cartan}.
\qeds

These two corollaries lead to the following statement.
...
\parit{Proof.}
Start with a simplicial complex $\spc{S}$ with a nonsimply connected homotopy sphere as the underlying space.
(Poincare homotopy sphere gives a perfect choice.)
...
Consider the cubical analog $\square_{\spc{S}}$ of $\spc{S}$.

By Corollary~\ref{cor:flag-hom}, $\square_{\spc{S}}$ is homological manifold.

By Corollary~\ref{cor:flag-cube}, the universal cover $\tilde\square_{\spc{S}}$ is a $\cCat{}{0}$ space.

It remains to show that $\tilde\square_{\spc{S}}$ is not simply connected at the infinity.
Fix a vertex in the universal cover $\tilde\square_{\spc{S}}$.
Its space of directions is isometric to $\spc{S}$
which is equipped  with the right-angled spherical metric.
The space $\spc{S}$ admits an embedding $\iota$ with the image formed by a small sphere around a vertex $v$ in $\tilde\square_{\spc{S}}$.

Fix a noncontractable closed curve $\gamma\:\SS^1\to \spc{S}$.
Note that $\iota\circ\gamma$ is not contractable in $\tilde\square_{\spc{S}}\backslash\{v\}$.
It reamins to show that there is a circle 
$\gamma'\:\SS^1\to \tilde\square_{\spc{S}}\backslash\{v\}$
which is homotopic to $\iota\circ\gamma$ in $\tilde\square_{\spc{S}}\backslash\{v\}$ and which lies arbitrary far from $v$.
The later follows from the following claim.
\begin{clm}{}
...
By ??? any polyhedral homological manifold is homotopically equivalent to a smooth manifold.
Therefore we get the following topological corollary.

\begin{thm}{Corollary}
There is a smooth??? compact aspherical manifold which universal cover is not homeomorphic to the Euclidean space. 
\end{thm}

\parbf{Comments.} The Proposition \ref{prop:example-pi_infty}
has number of variations

\parbf{$\cCat{}{0}$ homology manifold.}

Assume that $\spc{S}$ is a flag simplicial complex 
with underlying space homeomorphic to a homolody sphere.
Then the universal cover of $\square_{\spc{S}}$ 
is a $\cCat{}{0}$ space.

that if the ${\spc{S}}$ is a triangulation of homology sphere then the obtained space is homological manifold;
that is 
If ${\spc{S}}$ is flag,
the fundamental group of at infinity of 
$\tilde{\square}_{\spc{S}}$
admits an onto-homomorphism to the fundamental group of ${\spc{S}}$.
Consider a flag triangulation of homology sphere.

\begin{thm}{Lemma}
Assume $\spc{U}\in \cCat{}{1}$ is a homology sphere.
Then for any point $x\in \spc{U}$ the complement
$\spc{U}\backslash\oBall(x,\pi)$ is connected.
\end{thm}

\parit{Proof.}
Since $\spc{U}\in \cCat{}{1}$,
the closed ball $\cBall[x,\rho]$ is contactable
for any $\rho<\pi$.

By Alexander's duality, 
the complement $\spc{U}\backslash\cBall[x,\rho]$
is connected for any $\rho<\pi$.

Passing to the intersection of $\spc{U}\backslash\cBall[x,\rho]$
for $\rho<\pi$, we get the statement.
\qeds

Recall that a noncompact topological space $\spc{X}$ is called \emph{connected at infinity}
If for any compact subset $K\subset \spc{X}$
there is a compact set $K'\supset K$
such that the complement $\spc{X}\backslash K'$
is connected.
A $\spc{X}$ is called \emph{simply connected at infinity}
is compact subset $K\subset \spc{X}$
there is a compact set $K'\supset K$
such that the complement $\spc{X}\backslash K'$
is simply connected.

\begin{thm}{Lemma}
Any contactable homological manifold of dimension $m\ge 2$ is connected at infinity.
\end{thm}

\begin{thm}{Propostion}
Assume $\spc{U}\in\cCat{}{0}$ is a homological manifold
which is simply connected at infinity then 
everypoint has a simply connected punctured 
\end{thm}

\parit{Proof.}
Assume contrary.
Let $p\in \spc{U}$ be a point 
with spce of directions $\Sigma_p$ which is not simply connected.
Let $\tilde\Sigma_p\to \Sigma_p$ be its universal covering.
Consider the induced covering of $\spc{U}$ which is branching at $p$...
