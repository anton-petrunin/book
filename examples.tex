...
However, 
most of the statements below admit straightforward generalizations to \index{polyhedral spaces!locally polyhedral spaces}\emph{locally polyhedral spaces};
that is complete length spaces  any point of which admits a closed neighborhood isometric to a polyhedral space.
The later class of spaces includes in particular the infinite covers of polyhedral spaces.
...
Let $\spc{P}\in\PM{}\kappa$ and $\tau$ be a triangulation of  $\spc{P}$. Then 
...
admit the following characterization discovered by Gromov \cite[p. 122]{gromov:hyp-groups}.
...
it follows that cubical analog
...
Therefore complete locally $\cCat{}{0}$ spaces are \emph{aspherical};
...
if $X$ if for any compact set $K\subset\spc X$
...
otherwise pass to the barycentric subdivision of $\tau$ and apply Exercise~\ref{ex:baricenric-flag}.
...
It follows that $\spc{Q}$ is a piecewise linear manifold with a finite number of singularities at its vertices.
...
This results in a  closed piecewise linear manifold $\spc{M}$ which is homotopically equivalent to $\spc{Q}$.
...
If it would not be the case, there would be a spheroid in $K$ which lies in the interior of $K$ except for the point $v$.
The part of $\partial K$ 
which lies inside of the spheroid can not be two-convex.
