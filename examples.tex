...
\begin{thm}{Corollary}Assume a closed connected subset $K$ of Euclidean space bounded by a smooth hypersurface forms $\cCat{}{0}$ space if
...
The material of this section inspired by work of Samuil Shefel who seems to be very intrigued by survival of metric properties under affine transformation.
...
\begin{thm}{Corollary}
Assume $f\:\DD\to \RR$ is a saddle locally Lipschitz function then the graph
...
is locally bi-Lipschitz to the euclidean metric on $\Delta$.

Set $\Delta^\circ=\Int\Delta$.
Consider the sequence of sets 
\[\Omega_n
=
\set{(x,y,z)\in \EE^3}{|z- f(x,y)|<\tfrac1n,\ (x,y)\in \Delta^\circ}.\]
Note open that each $\Omega_n$ is open, simply connected and two-convex.
...
Assume that for any 2-dimensional subspace $W$ in $\EE^m$
