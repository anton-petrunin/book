...
Given $m\ge 6$ there is a compact locally $\cCat{}{0}$ polyhedral space which is homeomorphic to a closed $m$-dimensional manifold and
...
Given a positive integer $m\ge 6$
...
Fix an $(m-3)$-dimensional homology sphere $\Sigma$ with a triangulation such that $\pi_1\Sigma\ne0$.
According to \cite{kervaire} %it is a good readable paper, but I am sure the existance follows from sometheng written before
an example of that type exists for any $m\ge 6$.

Remove from $\Sigma$ one $(m-3)$-simplex.
Denote the obtained complex by $\Sigma'$.
Since $m\ge 6$, we have $\pi_1\Sigma=\pi_1\Sigma'$.
...
Note that $\spc{S}$ is homotopy equivalent to the double spherical suspension over $\Sigma$ which is a simply connected homology sphere and hence is homotopy equivalent to $\mathbb{S}^{m-1}$.
 (In fact, it is homeomorphic to $\mathbb{S}^{m-1}$ by the Double Suspension Theorem but we do not need it here).  Hence  $\spc{S}\sim\mathbb{S}^{m-1}$.
