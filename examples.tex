...
The classes of $\PM{}{\kappa}$ spaces 
of dimension $m$ and $\le m$ will be denoted 
$\PM{m}{\kappa}$ and $\PM{\le m}{\kappa}$ respectively.
...
form an abstract simplicial complex called the link of $\sigma$ 
which will be denoted  $\Link_\sigma$.
...
carries a natural structure of a spherical polyhedron. %???WHY???
...
\parit{Step.}
Assume that the theorem is proved if $\dim<m$ and let $\dim\spc{P}=m$.
We will prove the if direction. The proof of the only if direction is similar and left to the reader.
...
Equivalently, $\curv\spc{P}\le\kappa$ if and only if any closed local geodesic in $\Link_\sigma$ has length at least $2\cdot\pi$.
...
\parit{Step.}
Assume that the theorem is proved for spaces of $\dim\spc{P}<m$. Let  $\dim\spc{P}=m$.
As before we will only give a proof of the ``if" direction.
...
Denote by $\hat \Omega$ the completion of $\Omega$ equipped with the induced length metric,
Then $\hat\Omega$ has nonpositive curvature 
if and only if $\Omega$ is two-convex.
\end{thm}

\parbf{Remark.}
In the most cases, $\hat\Omega$ is isometric to $K$ with induced intrinsic metric,
but in general we only have locally distance preserving map $\hat\Omega\to K$ which is onto but in general may be not injective. Say consider a polytope with a 
...
%\end{thm}
The following lemma is from \cite{aa:mwb}.

\begin{thm}{Lemma}\label{lem:m-w-b-geodesic-'}
Let $M$ be a Riemannian manifold-with-boundary, and $\gamma$ be a unit-speed geodesic of $M$ with $\gamma(t)=p\in\partial M$. Then:
\begin{subthm}{}
