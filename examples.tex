...
By a \index{triangulation of a $\PM{}{\kappa}$ space}\emph{triangulation of a $\PM{}{\kappa}$ space} we will understand the triangulation as in the definition. 
...
In particular, the isometry class of $\Link_\sigma$ 
completely determined by the space of directions at one point in the interior of $\sigma$ and the dimension of $\sigma$.
...
$\gamma_n\:[0,1]\to \spc{P}$ 
such that $\gamma_n(t)\in \spc{P}^{\circ}$ for any $n$ and $0<t<1$.
\end{itemize}
...
By induction hypothesis we get $\curv\Sigma_p\ge 1$.

Applying Generalized Hadamard--Cartan theorem \ref{thm:hadamard-cartan-for-k=1},
...
\begin{subthm}{}
$\curv_p\spc{P}<\infty$ for any $p\in\spc{P}$;
\end{subthm}
...
\begin{subthm}{}
Any point $p\in\spc{P}$ 
admits a neighborhood $\Omega\ni p$ 
such that any two points $x,y\in\Omega$ 
are joint unique geodesic $[xy]$ in $\spc{P}$. 
\end{subthm}
...
By a \emph{Riemannian manifold-with-boundary} $M$ of a given differentiability class, we mean $M$ has possibly nonempty boundary and has an extension across the boundary to a manifold $\overline{M}$ of the same class with empty boundary, such that  each boundary point of $M$ has a neighborhood $U$ in $\overline{M}$ coordinatized by $\R^n$ and for which $M\cap U$ is coordinatized by a halfspace of $\R^n$. The Riemannian metrics of $\bar M$ and $\partial M$ are  of differentiability class one degree lower. 
...
Let $M$ be a Riemannian manifold-with-boundary.  Let $\gamma$ be a geodesic of $M$ with unit-speed parameter $s$. Then   after  passing to coordinates, 
$\gamma'$  is Lipschitz continuous in $s$.
\end{thm}

\parit{Proof.}
By passing to coordinates, we   take $\overline{M}=\R^m$.  

\begin{clm}{}\label{clm:}
$\gamma$ has an upper bound on pointwise arc-chord curvature.
\end{clm}
Let $p=\gamma(t)\in\partial M$.  
%We may suppose $\gamma$ is not a straight line segment on any neighborhood of $t$.  

First suppose $\gamma(t_1)$ and $\gamma(t_2)$ lie on $\partial M$, where $t_1<t<t_2$ and the $t_i$ are  arbitrarily close to $t$. 
Let $Q$ be the $2$-plane through $\gamma(t_1), p, \gamma(t_2)$.  Let $\tilde\alpha$ be a unit-speed curve in $Q$  whose image is the open portion of the curve $\partial M\cap Q$ 
between $\gamma(t_1)$ and $\gamma(t_2)$,
and  $\alpha$ be a unit-speed curve in $Q$  whose image is 
the open geodesic of $M\cap Q$ between $\gamma(t_1)$ and $\gamma(t_2)$. 
 
The osculating plane of $\gamma$ at $t$ is defined and normal to $\partial M$, i.e. as $t_1,t_2$ approach $t$, $Q$ approaches the $2$-plane that contains $\gamma'(t)$ and is normal to $\partial M$ at $p$.   It follows, assuming   $|t-t_i|$ sufficiently small for $i=1,2$, that  $\tilde\alpha$ has  \emph{geodesic} curvature ???$\le k$.  
  
In the plane $Q$, $\alpha$ is a $C^1$ convex curve that runs along the boundary of the convex hull of $\tilde\alpha$.  Therefore the total turn of  the tangent line of any subarc of  $\alpha$ with endpoints on $\partial M$  is not more than the total absolute turn of the tangent line of the subarc of  $\tilde\alpha$ with the same endpoints.  
 
For subarcs of the same length, the total turn of the tangent line of a subarc of $\alpha$ is not more than for a subarc of a circle $\sigma$ of curvature $k$.  Therefore the angles of a sufficiently fine polygonal curve inscribed in $\alpha$  are no less than the corresponding angles of a polygonal curve with the same sidelengths inscribed in the Euclidean circle $\sigma$ of geodesic curvature $k$.
 By Arm Lemma \ref{lem:arm}, the chord-length of $\alpha$ is not less than the chord-length of a segment of $\sigma$ of length $=\length\alpha$. Therefore $\alpha$ has arc-chord curvature $\le k$.
 
Since the geodesic $\gamma|[t_1,t_2]$ has the same endpoints as $\alpha$ and is not  longer
than $\alpha$, then  $\gamma|[t_1,t_2]$ has arc-chord curvature $\le k$.  Therefore $\gamma$ as pointwise arc-chord curvature $\le k$. 


\begin{clm}{}\label{clm:}
$\gamma'$ is Lipschitz continuous. 
\end{clm}
$\gamma'$ is Lipschitz continuous in the unit-speed parameter inherited from $\,\R^m$ . 

Globalization of arcchord curvature
\ref{subthm:local-global-base-angle}
\qeds
...
closed ball meeting $M$ at a single point. Then the principal curvatures of $M$, as well as  those of $\partial M$ belonging to a normal
...
displacement in $\EE^n$ between corresponding points. Then except at the countably many points where $f''$ fails to exist, we have a differential inequality
...
If $M$ is Euclidean space, there is a beautiful purely synthetic proof in \cite{milka-conv}.
I.e., it proves the following corollary of Buyalo's theorem.

\begin{thm}{Corollary} 
Let $F\subset \EE^m$ be a convex hypersurface equipped 
with the length-metric; 
i.e., $F$ bounds a convex set.
Then $F\in\CBB{m-1}0$.
\end{thm}

\parit{Sketch of Milka's proof.}
We will assume that $F$ is smooth;
the non-smooth case require only minor modifications. 

Denote by $H$ the union of $F$ and the exterior points of $F$.

Let $\alpha\:[0,a]\to F$ be a geodesic in $F$.
Given $t_0\in [0,a]$, 
consider the curve $\alpha_{t_0}\:[0,a]\to H$ defined the following way
\[\alpha_{t_0}(t)
=
\left[
\begin{aligned}
&\alpha(t)&&\text{if}&t\le t_0
\\
&\alpha(t_0)+(t-t_0)\cdot \alpha^+(t_0)&&\text{if}&t> t_0
\end{aligned}
\right.
\]
Note that $\alpha_{t_0}$ is a geodesic in $H$ 
and $\alpha_0$ is a straight segment of the same length as $\alpha$.

Assume $\alpha\:[0,a]\to F$ and $\beta\:[0,b]\to F$
be two geodesics in $F$ which stat at the same point;
say $p=\alpha(0)=\beta(0)$.
...
Note that $[p\hat x\hat y]=\~\triangle (pxy)_F$.
Therefore it is sufficient to show that 
\[\dist{\hat x}{\hat y}{H}\ge \dist{x}{y}{H}.\]

Consider the function 
$d(t,\tau)
=
\dist{\alpha_t(a)}{\beta_\tau(b)}{H}$.
Note that $\partial_t h(t,\tau)\ge 0$
and $\partial_\tau h(t,\tau)\ge 0$
if both points $\alpha_t(a)$ and $\beta_\tau(b)$ lie on the boundary of convex hull of $F\cup\{\alpha_t(a),\beta_\tau(b)\}$.

Thereofore it is sufficient to show that 
there is a continuous nondecreasing function
$\tau\:[0,a]\to[0,b]$
such $\tau(0)=0$ and $\tau(a)=b$ 
such that for any $t$ the points $\alpha_t(a)$ and $\beta_{\tau(t)}(b)$ lie on the boundary of $\Conv(F\cup\{\alpha_t(a),\beta_\tau(b)\})$.
We leave this last statement as an exercise.
\qeds
