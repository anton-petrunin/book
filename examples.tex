...
However, 
most of the statements below admit straightforward generalizations to locally polyhedral spaces;
that is complete length spaces  any point of which admits a closed neighborhood isometric to a polyhedral space.
...
which will be denoted by $\Link_\sigma$.
...
is called \emph{all-right triangulation} if each simplex of the triangulation is isometric to a spherical simplex all of which angles are right.
...
to a spherical simplex all of which angles are right.
...
is formed by all the simplexes  containing $v$. Similarly, $\Star_v$,   an open star of $v$ is the union of all simplices containing $v$ with faces opposite $v$ removed.
...
the local geodesic $\gamma$ has to enter another simplex, say, $\sigma'$
...
but simplices have to be changed to  cubes.
...
it will be denoted as $\spc{Q}$ or as $\ushort{\spc{Q}}$ if we need to emphasize that we are talking about a set, 
not a complex.
...
if $X$ has one end and for any compact set $K\subset\spc X$
there is a bigger compact set $K'\supset K$
such that  $\spc X\backslash K'$ is path connected and  the map $\pi_1( \spc X\backslash K')\to \pi_1( \spc X\backslash K)$ is trivial.

% complement $\spc X\backslash K'$ is simply connected.
...
For any  $m\ge 4$ there is a closed aspherical  $PL$ $m$-dimensional manifold
...
Further, note that 
$\tilde G$ is homeomorphic to the plane since it's a 2-dimensional manifold without boundary which by above is $\cCat{}{0}$ and hence is contractible.
...
Let $\Sigma^{m-1}$ be an $(m-1)$-dmensional smooth homology sphere which is not simply connected and bounds a contractible smooth compact manifold $\spc{W}^m$. For $m\ge 5$ the existence of such $(W, \Sigma)$ follows from \cite{kervaire}. For $m=4$ it well-known that there are many $3$-dimensional homology spheres (e.g. the Brieskorn sphere $\Sigma (2,3,13)$) which bound contractible smooth $4$-manifolds. best ref???

Pick any smooth triangulation $\spc L$ of $W$ and let $\spc{S}$ be the resulting subcomplex which triangulates $\Sigma$.
...
It follows that $\spc{Q}$ is a $PL$-manifold with a finite number of topological singularities at its vertices.


Let us remove a small spherical neighborhood of each vertex.
We obtain a $PL$-manifold with boundary formed by several  copies of $\Sigma$.
To each copy of $\Sigma$, 
glue a copy of  $\spc{W}$ along its boundary.
This results in a  closed $PL$-manifold $\spc{M}$ which is homotopically equivalent to $\spc{Q}$.

Finally, by the Lemma~\ref{lem:example-pi_infty},  
the universal cover $\tilde{\spc{Q}}$ of $\spc{Q}$
is not simply connected at infinity.
The same holds for 
the universal cover $\tilde{\spc{M}}$ of $\spc{M}$ since a homotopy equivalence $f\co \spc Q\to \spc M$ lifts to 
a surjective homotopy equivalence $\tilde f \co \tilde{\spc{Q}}\to \tilde{\spc{M}}$ which is a quasi-isometry and hence maps any compact exhaustion of $ \tilde{\spc{Q}}$ to a compact exhaustion of  $\tilde{\spc{M}}$.
\qeds
...
Given $m\ge 5$ there is a compact locally $\cCat{}{0}$ polyhedral space which is homeomorphic to a closed $m$-dimensional manifold and
...
In the proof we apply the following Lemma, which follows from the disjoint discs property by a theorem of Edwards
...
\parit{Proof of Lemma~\ref{lem:funny-S}.} 
...
\parbf{Remark.}
If $\Omega$ is interior of a polytope $K$
then typically $\Omega^*$ is isometric to $K$  with induced intrinsic metric.
However in general
we only have locally distance preserving map $\Omega^*\to K$ which is onto but in may be not injective. 
...
To prove the converse note that since $\Omega$ and twoconvex,
any vertex $v$ of $K$ admits a neighborhood $U$ such that the intersection $U\cap \Omega$ is formed by a finite collection of simply connected components.
If it would not be the case, there would be a spheroid in $K$ which lies in the interiour of $K$ expept for the point $v$.
The part of $\prtial K$ 
which lieas inside of the spheroid can not be two-convex.
