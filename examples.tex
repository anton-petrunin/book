...
The boundary $\partial A$ is the parabolid described by  $z=x^2+y^2$ in particular it bounds an open convex set in $\EE^3$ with complement $A$.
...
If $\gamma$ is a closed curve that lies in $W\cap K$ and null-homotopic in $K$, then it is null-homotopic in $W\cap K$.
\end{thm}
...
From above the there is a disc in $K$ with boundary $\gamma$ that lies arbitrary close to $\Conv\gamma$.
...
In this section we characterize the subsets in $\EE^m$ with smooth boundary that are $\CAT{0}$ if equipped with the induced length metric. 
...
Then by Proposition~\ref{prop:two-cove+smooth}, there is a triangle $[xyz]$ in $K$ that is null-homotopic in $K$,
...
there is a short map $f\:\Delta\to K$ that preserves $x$, $y$ and $z$.
...
\begin{thm}{Corollary}
Let $f\:\DD\to \RR$ be a Lipschitz function that is saddle in the interior of the closed unit disc $\DD$. 
Then the graph
...
Denote by $\mathbb{S}^2$ the unit sphere centered at $p$
By  rescaling, we can assume that $\mathbb{S}^2$ intersects only the faces of $K$ that contain $p$.
Set $\Theta=\mathbb{S}^2\cap\Omega$.
Note that $\Sigma_pK$ is isomeric to the completion of $\Theta$
and $\Sigma_{\tilde p}\tilde K$ is a completion of a regular metric covering $\tilde\Theta$ of $\Theta$ that is induced by universal metric cover $\tilde \Omega\to \Omega$.
...
describes a construction given by Shefel in \cite{shefel-3D}.
...
and a closed curve $\gamma\:\mathbb{S}^1\to \Pi\cap \Omega'$ 
that is null-homotopic in $\Omega'$,
...
In other words, 
$F'_i$ is the subset of facet $F_i$ that  remains on the boundary of $K'$.
...
\end{enumerate}
(If not, rescale and pass to the boundary of the convex piece cut by $F$  from a small convex ball centered at $x\in F$, taking $h=-10\cdot\distfun{x}{}{}$.)
...
Applying Lemma~\ref{lem:smoothing}, we can find a smooth $(-\tfrac\eps2)$-concave function $f_{\eps}$ that is arbitrarily close to $f+\eps\cdot h$ on $K_\eps$ and defined on a neighborhood of $K_\eps$. 
...
A direction tangent to the boundary will be called concave if there is a short geodesic in this direction that leaves the boundary and goes into interior of $M$.
...
A subset $K$ is called strongly two-convex if any null-homotopic circle $\gamma\:\mathbb{S}^1\to K$ is also null-homotopic in $K\cap \Conv[\gamma(\mathbb{S}^1)]$.
...
In \cite{petrunin-metricmin, petrunin-stadler} it was shown that discs in complete length $\CAT{0}$ spaces 
that do not admit a length-decreasing deformations with fixed boundary are $\CAT{0}$.

Any metric minimizing disc forms a saddle surface as defined above.
There are saddle surfaces that are not metric minimizing globally and it is expected that there saddle surfaces that are not metric minimizing in arbitrary neighborhood of a fixed point.
Constructing such examples could shed light on the Shefel's question.

\parbf{On convex hypersurfaces in Alexandrov space.}
We are not aware of any proof of Theorem~\ref{thm:buyalo} that is not based on the Gauss formula.

Even if the ambient space is Euclidean, we do not know any proof that avoids approximation by simpler spaces;
