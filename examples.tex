...
A cubical complex can be defined 
as a subset of faces of a cube $[0,1]^{\times n}\subset \RR^n$
...
A homeomorphism from $|\spc{K}|$ to a topological space is called \emph{cubulation}.
...
each cube of $K$ is isometric to the unit cube of the same dimension
and the link of each vertex admits a triangulation 
such that each simplex is isometric to the right-angled spherical simplex of the same dimension.

Given a simplicial complex $L$,
let us construct a cubical complex $K_L$ 
such that the links of all the vertices are isomorphic to $L$.

Recall that any simplicial complex $\spc{S}$ can be realized as subcomplex of a simplex, say $\triangle$.
We can assume that the basis in $\RR^n$ 
forms the set of vertices of simplex of $\triangle$.

For each simplex $\sigma$ of $\spc{S}$
consider all the coordinate subspace $L_\sigma$ of $\RR^n$
spanned by vertices of $\sigma$.
Choose all the faces of the cube which are parallel to $L_\sigma$.
All such faces in the cube form a subcomplex which we denote by   $P_L$.

The link of one vertex of the $n$-dimensional cube is isometric to right-angled 
spherical simplex $\triangle^{n-1}$. 
Let us realize the simplex $L$ as a sub-complex of $\triangle^{n-1}$.
Consider all faces of the cube which have
