...
\begin{thm}{Question}
Which of the sets $A$, $B$ and $C$, if equipped with the induced length metric, form a $\cCat{}{0}$ space and why?
\end{thm}
...
and show that any finite union of $V_t$ forms a $\cCat{}{0}$ space.
...
In this section we characterize the subsets in $\EE^m$ with smooth boundary which form $\cCat{}{0}$ spaces. 
...
Base: $W^1=V^1$ is a $\cCat{}{0}$ space as a convex subset in $\EE^m$.

Step: assume that $W^i\in \cCat{}{0}$.
...
$K\in\cCat{}{0}$ if $K$ is \index{strongly two-convex set}\emph{strongly two-convex};
...
By \ref{clm-strong2convex}, $K_\delta\in\cCat{}{0}$.
...
Show that 
$K\in \CBB{}{0}$ if and only if $K$ is convex. 
\end{thm}
...
Note that it is enough to consider only injective triangle $[xyz]$;
...
Show that the completion of $\tilde \Theta$ is a  $\cCat{}{1}$ space.
\end{thm}
...
Then any affine transformation of $K$ also forms $\cCat{}{0}$ space.
\end{thm}
...
By Theorem~\ref{thm:warp-curv-bound:cbb:S}, \ref{eq:curv=<0} follows if 
\[\Sigma_{\tilde p}\tilde K\in\cCat{}{1}.
\eqlbl{eq:curv=<1}\]
...
By Lemma~\ref{lem:poly-shefel}, $\tilde K'\in \cCat{}0$.
...
Show that $K$ is always two-convex and one can choose $\iota$ so that $K$  with the induced length metric is not $\cCat{}{0}$.
\end{thm}
...
\begin{thm}{Question}
Is it true that any closed strongly two-convex set with dense interior bounded by a Lipschitz hypersurface in $\EE^m$ forms a $\cCat{}{0}$ space? 
\end{thm}

\parbf{Metric minimizing surfaces.}
In \cite{petrunin-metricmin} was shown that discs in $\cCat{}{0}$ spaces 
which do not admit a length-decreasing deformations with fixed boundary form $\cCat{}{0}$ spaces.
...
the sythetic argument of Milka in \cite{milka-conv} is very intersting, 
...
The latter problem is one of the first questions in $\CBB{}{}$ spaces which is still open.

\begin{thm}{Question}
Let $\spc{L}\in\CBB{m}{\kappa}$ and its boundary $\partial\spc{L}$ is not empty.
Is it true that $\partial\spc{L}$ equipped with induced length metric forms a $\CBB{m-1}{\kappa}$ space?
\end{thm}

Equivalently one can ask, is it true that convex hypersurface in a $\CBB{m}{\kappa}$ space without boundary forms a $\CBB{m-1}{\kappa}$ space?
