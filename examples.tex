...
A cubical complex can be defined as a subcomplex of unit cube in the Euclidean space;
that is, as a subset of faces of a cube $[0,1]^{\times n}\subset \RR^n$
such that together with each face it contains all its sub-faces.
Each cube face in this collection will be called \emph{cube} in the cubical complex.

Note that according to this definition cubical complex is finite;
that is it contains a finite number of cubes.

The union of all cubes in a cubical complex $\spc{K}$ will be called it underlying space and denoted as $|\spc{K}|$.
A homeomorphism from $|\spc{K}|$ to a topological space $\spc{X}$ is called \emph{cubulation of} $\spc{X}$.

Let us equip $|\spc{K}|$ with the length metric
induced from $\RR^n$.
It is straightforward to construct a triangulation of $|\spc{K}|$,
so that each simplex is isometric to a Euclidean simplex.
In particular $|\spc{K}|$ is a polyhedral space.

Moreover, in this metric, 
...
\parbf{Construction of cubical complex from the given simplicial complex.}
Let $\{v_1,\dots,v_N\}$ be the set of vertexes of $\spc{S}$.
Let $\{e_1,\dots,e_N\}$ be the standard basis in $\RR^N$.
For each $k$-dimesional simplex $\<v_{i_0},\dots,v_{i_k}\>$ of $\spc{S}$,
mark the all the $(k+1)$-dimensional faces 
of the unit cube $[0,1]^{\times n}$ 
which are parallel to the coordinate $(k+1)$-plane 
spanned by $e_{i_0},\dots,e_{i_k}$.

Note that the set of all marked faces of $[0,1]^{\times n}$
form a cubical complex; denote it by $\spc{K}_{\spc{S}}$.

The following proposition follows from the construction above.

\begin{thm}{Proposition}
Let $\spc{S}$ be a simplicial complex
and $\spc{K}_{\spc{S}}$ be the asociated cubical complex.
Assume that $\spc{K}_{\spc{S}}$ equipped with the length metric induced from ambient Euclidean space.
Then link of any vertex of $\spc{K}_{\spc{S}}$
is isometric to the simplicial complex ${\spc{S}}$
equipped with right-angled metric.
\end{thm}

\begin{thm}{Corollary}
If the underlying space of $\spc{S}$ 
is a homology sphere then $\spc{K}_{\spc{S}}$ is a homology manifold.
\end{thm}

\begin{thm}{Corollary}
If  $\spc{S}$ is flag then the universal cover of $\spc{K}_{\spc{S}}$ is a $\cCat{}{0}$ space.
\end{thm}
...
Then the universal cover of $\spc{K}_{\spc{S}}$ 
is a $\cCat{}{0}$ space.
