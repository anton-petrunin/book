...
\begin{thm}{Theorem}\label{thm:product}
Let $\spc{X}$ be a length space.
Then 
\begin{subthm}{thm:product:prod-CBB} 
$\spc{X}\times\spc{Y}\in \CBB{}{0}$ $\iff$ $\spc{X},\spc{Y}\in \CBB{}{0}$.
\end{subthm}
\begin{subthm}{thm:product:prod-CAT} 
$\spc{X}\times\spc{Y}\in \cCat{}{0}$ 
$\iff$ $\spc{X},\spc{Y}\in \cCat{}{0}$.
\end{subthm}
\end{thm}
...
The spaces $\spc{X}$ and $\spc{Y}$ is isometric to the vertical and horizontal subsets in $\spc{X}\times\spc{Y}$.
Note that the product space is a length space if and only if its factors are length space (assuming non of them is empty). 
%???DO we assume that metric space is nonempty???
Whence the $\Rightarrow$ 
in (\ref{SHORT.thm:product:prod-CBB}) 
and (\ref{SHORT.thm:product:prod-CAT}) 
follows. 

\parit{``If'' part in (\ref{SHORT.thm:product:prod-CBB}) and (\ref{SHORT.thm:product:prod-CAT}).} 
Follows from function comparison (theorem \ref{thm:conc} and \ref{thm:function-comp}),
