...
Now assume in addition that $\spc{X}$
...
Note that each $\Omega_n$ is path connected 
...
\begin{thm}{Lemma}
Let $\spc{M}$ and $\spc{N}$ be compact connected manifolds of the same dimension
...
Note that $f$ factors through a cover $\phi$;
...
Start with a simplicial complex $\spc{S}$ with a nonsimply connected homotopy $m$-sphere as the underlying space.
...
Now assume $\spc{S}$ admits a nontrivial covering by a homology sphere.
This covering induce a covering $\tilde{\spc{K}}_v\to\tilde{\spc{K}}$
branching at $v$.

More precisely, 
consider the map $\tilde{\spc{K}}\backslash\{v\}\to \spc{S}$
which sends a point $x$ to the direction of the geodesic $[vx]$ in $\Sigma_v=\spc{S}$.
Consider the induced covering for this map 
and denote by $\tilde{\spc{K}}_v$ its completion.
The space $\tilde{\spc{K}}_v$ obtained from the covering by adding one point???

Note that $\tilde{\spc{K}}_v$ is a homological manifold.
