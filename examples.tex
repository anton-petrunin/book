...
Then the completion $\hat{\Omega}$ of $\Omega$ is a $\cCat{}{\kappa}$ space.
\end{thm}

\parbf{Remark.}
Note that if $\Omega$ is bounded by a simple closed rectifiable curve its completion of $\hat{\Omega}$ is isometric to the closure $\bar{\Omega}$  of $\Omega$ equipped with the length metric. 
In general, the space $\hat{\Omega}$ admits natural length-preserving map in $\bar{\Omega}$, 
but 
...
\parbf{Two-convexity.}
Assume for a subset $A\subset \Lob{m}{\kappa}$
the following condition holds for any open $2$-dimensional halfspace $W$ in $\Lob{m}{\kappa}$.
...
This definition is closely related to the one given by Gromov in \cite[???]{gromov:SaGMC}, see also \cite{panov-petrunin:sweeping}.

Note that according to this definition, if $\kappa\le 0$ then any subset $A\subset \Lob{2}{\kappa}$ is 2-convex
and if $\kappa>0$ it is 2-convex if it does not contain a closed halfspace in its interior.
Therefore Theorem \ref{thm:2-d-cba} can be reformulated as any 2-convex polygon in $\Lob{2}{\kappa}$ equipped with induced intrinsic metric is $\cCat{}{\kappa}$ space.
Similarly one can reformulate Theorem~\ref{thm:2d-bishop}.
...
forms a polyhedral space as defined in \ref{def:poly}.

\begin{thm}{Proposition}
Let $\Omega\subset\Lob{3}{\kappa}$ be an open.
Assume $W_1,W_2,\dots$ is a dense sequence of 2-dimensional halphspaces $\Lob{3}{\kappa}$.
Denote by $\Omega=\Omega_0\subset \Omega_1\subset\dots $ the sequence of open sets
such that $\Omega_{n}$ include all the points in $\Omega_{n-1}$ and all connected components 
of the complement of $\Omega_n\cup W_n$ which do not have point on the boundary line 
...
Then $K$ id 2-convex if and only if $K$ equipped with intrinsic metric has curvature $\le \kappa$ at any point.
\end{thm}
...
that is, there is a tetrahedron $\Delta$,
...
and let $\hat \Omega$ be its completion.
Then $\hat \Omega$ has curvature $\le\kappa$ at every point 
...
is convex in $\Lambda$;
that is, for any two points $x,y\in \Lambda'$ any geodesic
