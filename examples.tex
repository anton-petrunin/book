...
\section{Domains with smooth boundary}\label{sec:smooth-bry}

Let $S$ be an orientated hypersurface in the Euclidean space $\EE^m$.
The orientation can be given by a choice of orthonormal vector field $\nu$ at each point is given. 
Denote by $k_1\le \dots\le k_{m-1}$ the scalar field of principal curvatures at each point of $S$
with respect to $\nu$.

We say that $S$ is \index{convex hypersurface}\emph{convex with respect to $\nu$} if $k_1(p)\ge 0$ at any point and \index{two-convex hypersurface}\emph{two-convex with respect to $\nu$} if $k_2(p)\ge 0$ at any point $p\in S$.
...
\parit{``if''-part.} 
Note that is $S$ is convex then $K$ is locally convex;
...
Note that in this case there is a triangle $[xyz]$ in $\EE^m$ such that its sides lie in $\Omega$ completely but the solid triangle (that is, convex hull $\triangle=\Conv(x,y,z)$) sticks out of $K$.
...
Moving this triangle slightly in the direction $\nu(p)$ provides the needed triangle.

If $K\in\cCat{}{0}$,
then by Reshetnyak majorization theorem,
there is a short map $f$ from  $\triangle$ in $\EE^m$ to $K$ which preserves the sides of the triangle.

Note that the identity map is the only map 
\[f\:\triangle\to\EE^m\]
which satisfies the above property.
...
Note that $K_s'$ forms a convex set in $\EE^m$ and
\[K'=\bigcup_{s\in[-\eps,\eps]}K_s'.\]
Further note that 
\[K'_b\supset K'_a\cap K'_c\]
for any choice $a<b<c$.

Given a finite set of values $s_1<\dots<s_\kay$ in $[-\eps,\eps]$,
consider the unions 
...
\lbl{6,18;$K'_{s_1}$}
\lbl{26,17;$\dots$}
\lbl{48,18;$K'_{s_\kay}$}
\end{lpic}
\end{center}

Note that the sequence $(s_n)$ can be chosen in such a way that 
$Q_\kay$ is arbitrary close to $K'$.

Therefore, in order to prove \ref{K'-is-CAT}, 
it is sufficient to show the following claim.
\begin{clm}{}
$Q_\kay$ is a $\cCat{}{0}$ space for any sequence $(s_n)$ as above.
\end{clm}

This claim is proved by induction.
The base case, $Q_1=K'_{s_1}$ is a $\cCat{}{0}$ space as a convex subset in $\EE^m$.

Further, assume that $Q_i\in \cCat{}{0}$.
Note that 
\[K'_{s_{i+1}}\cap Q_i=K'_{s_{i+1}}\cap K'_{s_{i}}\] 
is a convex set in $\EE^m$ and therefore it forms a convex set in $Q_i$ and in $K'_{s_{i+1}}$.
By Reshetnyak's gluing theorem, $Q_{i+1}\in \cCat{}{0}$.
Hence the claim follows.
\claimqeds

It remains to consider the case $k_2(p)=0$.
Choose a coordinate system $(x_1,\dots,x_n)$ as above 
and fix $\eps>0$ so that a neighborhood of $p$ in $S$ 
forms a graph
\[x_m= f(x_1,\dots,x_{m-1})\]
for some function defined at the open disk $\DD$ of radius $\eps$ in the $(x_1,\dots,x_{m-1})$-coordinate hyperplane centered at the origin.
Fix a smooth strongly convex function $\phi\:\DD\to \RR_+$
such that $\phi(x)\to\infty$ as $x$ approaches the boundary $\DD$.
Note that for $\delta>0$, the graph $S_\delta$
\[x_m= f(x_1,\dots,x_{m-1})-\delta\cdot(x_1^1+\dots+x_{m-1}^2)\]
...
Let us define two-convex open sets in Euclidean space.
In order to generalize Theorem \ref{thm:set-with-smooth-bry:CBA} to nonsmoth hypersurfaces

\parbf{Two-convexity.}
Assume $K\subset \EE^m$ is an subset
...
Note that 
\begin{itemize}
\item Any connected subset in $\EE^2$ is 2-convex.
\item If $K$ has smooth boundary $S$ then it is two-convex if and only if $S$ is two-convex in the sense of definition in Section \ref{sec:smooth-bry}.
\item Intersection of arbitrary collection of two-convex sets $\EE^3$ is two-convex.
\item Interior of two-convex set is a convex two-set.
\end{itemize}

In particular one can define \emph{two-convex hull} $\Conv_2 K$ of a set $K$
as the intersection of all two-convex set containing $K$.
By the last property, two-convex hull of open set is open.
