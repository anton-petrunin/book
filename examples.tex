...
\section{Domains with smooth boundary}
...
We say that $S$ is \index{convex hypersurface}\emph{convex} if $k_1(p)\ge 0$ at any point and \emph{two-convex} if $k_2(p)\ge 0$ at any point $p\in S$.
...
Recall that $k_1(p)\dots k_{m-1}(p)$ denote the principle curvatures of $S$ at $p\in S$ in the nondecreasing order. 
...
If $S$ is not convex then there is a triangle $[pxy]$ in $\EE^m$ such that two sides $[qx]$ and $[qy]$ lie in $\Omega$, but the side $[xy]$ does not completely lie in $K$.
...
Indeed, choose a point $p\in S$ such that the first two principle curvatures  $k_1(p)$ and $k_2(p)$ are below zero.
...
Moreover, since $\eps$ is small,  for any fixed $s\in[-\eps,\eps]$ the function 
...
Finally as $\delta\to0$, a sequence of neighborhoods of $p$ in $K_\delta$ 
converges to a neighborhood of $p$ in $K$.
