...
Assume that the orientation on $S$ is given by a choice of orthonormal vector field $\nu$ at each point. 
Denote by $k_1\le \dots\le k_{m-1}$ the scalar field of principal curvatures at each point of $S$
with respect to $\nu$.

We say that $S$ is \index{convex hypersurface}\emph{convex} if $k_1\ge 0$ at any point and \emph{two-convex} if $k_2\ge 0$ at any point.
...
equipped the the orthonormal vector field $\nu$ which points outside of $K$.
...
\begin{itemize}
 \item Assume a closed connected subset $K$ of Euclidean space bounded by a smooth hypersurface forms $\cCat{}{0}$ space if
equipped with the induced length metric. Then so is any affine transformation of $K$.
\end{itemize}
As you will see in the next section the same holds for sets bounded by arbitrary surface in 3-dimensional Euclidean space, but fails to hold in higher dimensions.
...
Such a triangle can be found in the plane spanned by the normal vector $\nu(p)$ and the first principle direction at a point $p\in S$ where the first principle curvature  is below zero.
...
Since $p$ lies in the interior of $K$, 
\[\mangle\hinge pxy_K=\mangle\hinge pxy_{\EE^m}.\]
By hinge comparison (\ref{angle}) we have
\[\dist{x}{y}{K}\le\dist{x}{y}{\EE^m}.\]
The latter contradicts \ref{eq:xy_K>xy}.

\parit{(\ref{SHORT.thm:set-with-smooth-bry:CBA}) ``only-if'' part.}
...
Choose a small solid triangle in $\Pi$ which contains $p$ it its interior; 
...
by Cartan--Hadamard theorem \ref{thm:hadamard-cartan}, we only need to show that $\curv_p K\le 0$ at any point $p\in K$.
...
Moreover, since $\eps$ is small,  for any fixed $x_1\in[-\eps,\eps]$ the function 
...
Therefore it is sufficient to show the following claim.
\begin{clm}{}
$Q_\kay$ is a $\cCat{}{0}$ space for any sequence $(s_n)$.
\end{clm}

The later shown by induction.
First note that $Q_1=K'_{s_1}$ is a $\cCat{}{0}$ space as a convex subset in $\EE^m$.

Further, assume that $Q_i\in \cCat{}{0}$.
Note that 
\[K'_{s_{i+1}}\cap Q_i=K'_{s_{i+1}}\cap K'_{s_{i}}\] 
is a convex set in $\EE^m$ and therefore in $Q_i$ and in $K'_{s_{i+1}}$.
Therefore Reshetnyak's gluing theorem implies that $Q_{i+1}\in \cCat{}{0}$.
Hence the claim follows.
\claimqeds

It remains to consider the case $k_2(p)=0$.
Choose a coordinate system as before and consider the graph $S_\delta$ of 
\[x_m= f(x_1,\dots,x_{m-1})-\delta\cdot(x_1^2+\dots+x_{m-1}^2).\]
Note that for $\delta>0$, $S_\delta$ the $k_2(x)>0$ at any point of $x\in S$ which is $\eps$-close to $p$. 
In particular if $K_\delta$ is the closed domain bounded by $S_\delta$ then from  
Note that $K'_\delta$ converges to $K'$ as $\delta\to 0$.
