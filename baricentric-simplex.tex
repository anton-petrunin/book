...
although the results in section~\ref{sec:web-general} find some applications for finite dimensional $\CBB{}{}$ spaces as well.
The analogous material for $\CBB{}{}$ spaces is described in Chapter~\ref{chap:dist-maps}.
...
i.e. $\bm{x}=(x^0,x^1,\dots,x^\kay)\in\Delta^\kay$ if $\sum_{i=0}^\kay x^i=1$ and $x^i\ge0$ for all $i$.
...
i.e.,  $\map$ is a left inverse to the restriction $\bm{f}|\Web\bm{f}$.
In particular, 
\[\Web\bm{f}=\Im\map.
\eqlbl{eq:Web=Im}\]
...
From the definition of  of inner web we have
...
for any $\bm{y}$ and $\bm{z}$ sufficiently close to $\bm{x}$, i.e., $\spx{\bm{f}}$ is locally Lipschitz.
...
$\d_{\bm{x}}\sigma$ is is linear for almost all $\bm{x}\in\Delta^m$,
...
thus $\cBall[z,r]\in \cCat{}{\kappa}$ 
and $\Web\dist{\bm{a}}{}{}\subset\cBall[z,r]$;
see \ref{thm:convex-balls:=<r<pi/2} and \ref{thm:cat-bary-web:web=Im(bary)}.
...
i.e., if $\InWeb\dist{\bm{a}}{}{}=\emptyset$, 
then
...
the $\map$ is Lipshitz.
Assume the contrary; i.e., there are sequences $\bm{y}_n, \bm{z}_n\to \bm{x}\in\Dom\map$ such that
