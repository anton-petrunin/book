...
for a given \textit{nondegenerate} array of $\kay+1$ strongly convex functions.
...
\parbf{Barycenters.}
Let us denote by $\Delta^\kay\subset \RR^{\kay+1}$\index{$\Delta^m$ (standard simplex)} 
...
Consider the map $\spx{\bm{f}}\:\Delta^\kay\to \spc{X}$\index{$\spx{\bm{f}}$ (barycentric simplex)} defined by 
\[\spx{\bm{f}}(\bm{x})=\argmin\sum_{i=0}^\kay x^i\cdot f^i,\]
where $\argmin f$\index{$\argmin$ (point of minimum)} denotes a point of minimum of $f$.
...
\parit{Proof of \ref{thm:web}; \ref{SHORT.thm:web:Up-convex}+\ref{SHORT.thm:web:f(web)=min}.}
...
\begin{thm}{Exercise}\label{ex:fixed-point-CAT}
...
Let $\spc{U}$ be a complete length $\CAT\kappa$ space,
let $\bm{a}\z=(a^0,a^1,\dots a^m)$ be a nondegenerate point array 
of radius $<\tfrac{\varpi\kappa}{2}$ in $\spc{U}$,
and let $\sigma=\spx{\bm{a}}\mc\kappa$ be the corresponding $\kappa$-barycentric simplex.
...
and let $B^i=\cBall[a^i,r^i]$ for an array of positive reals $(r^0,r^1,\dots,r^\kay)$.
Assume that
...
with topological dimension 1 and arbitrarily large Hausdorff dimension.
...
Let $\spc{U}$ be a complete length $\CAT\kappa$ space,
and let $\rho\:\spc{U}\to\RR$ be a continuous positive function.
...
Let $\spc{U}$ be a complete length $\CAT\kappa$ space,
and let $\bm{a}\z=(a^0,a^1,\dots, a^\kay)$ be a point array with radius $<\tfrac{\varpi\kappa}{2}$.
...
This exercise generalizes Helly's theorem; compare it to \ref{thm:helly}, \cite[Prop. 5.3]{kleiner} and \cite{ivanov2014}.
