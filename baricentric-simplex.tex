...
\subset 
\spc{X}\] 
will be called the \index{web}\emph{web} on $\bm{f}$.
\end{thm}
...
Define the \index{inner web}\emph{inner web} of $\bm{f}$ 
as 
\[\InWeb\bm{f}
=
...
We say that a function array is \index{nondegenerate function array}\emph{nondegenerate} 
if $\InWeb\bm{f}\not=\emptyset$.
...
is \index{subnormal vector}\emph{subnormal} to $Q$ at point $\bm{v}\in Q$ 
if
\[\<\bm{x},\bm{w}-\bm{v}\>
\df
...
The \index{radius of the point array}\emph{radius of the point array} $\bm{a}$ is defined to be the radius of the set $\{a^0,a^1,\dots,a^\kay\}$;
that is,
...
\emph{vertices of $\kappa$-barycentric simplex}%
\index{vertices of barycentric simplex}.
...
Let $\spc{U}$ be a complete length $\CAT\kappa$ space
and $\bm{a}=(a^0,a^1,\dots a^\kay)$ be a point array with radius $<\tfrac{\varpi\kappa}{2}$.
If $\InWeb(\distfun{\bm{a}}{}{})$ is nonempty, then the point array $\bm{a}$ is called \index{nondegenerate point array}\emph{nondegenerate}.
\end{thm}
...
Therefore (\ref{SHORT.thm:cat-bary-web:Lip}), (\ref{SHORT.thm:cat-bary-web:web=Im(bary)}) and the first statements in (\ref{SHORT.thm:cat-bary-web:web=Im(bary)}) follow from Theorem \ref{thm:web}.
%???CHECK convexity ???
...
The \index{displacement}\emph{displacement} of the composition $\map[2]\circ\map\:\spc{U}\to\spc{U}$ is bounded by $\rho$;
that is,
...
are vertices of a simplex if and only if
$\Omega_{\alpha^0}
\cap
\Omega_{\alpha^1}
\cap\dots\cap
