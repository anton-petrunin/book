...
In this chapter we discuss the constructions introduced by Bruce Kleiner in \cite{kleiner}.
...
$\distfun{\bm{a}^{\without 0}}{}{}|\InWeb(\distfun{\bm{a}}{}{})$ is an open $C^{\frac12}$-embedding in $\RR^\kay$.
Thus there is an inverse of $\distfun{\bm{a}^{\without 0}}{}{}|\InWeb(\distfun{\bm{a}}{}{})$, say $\map\:\RR^\kay\subto\spc{U}$.
...
Let $\spc{U}$ be a complete length $\CAT\kappa$ space
and $\bm{a}=(a^0,a^1,\dots a^\kay)$ be a point array with radius $<\tfrac{\varpi\kappa}{2}$.
If  $\InWeb(\distfun{\bm{a}}{}{})\z\ne\emptyset$, then the point array $\bm{a}$ is called \emph{nondegenerate}\index{nondegenerate point array}.
\end{thm}
...
Further since $\InWeb(\distfun{\bm{a}}{}{})\ne\emptyset$,
the area formula %??? 
implies that $\d_{\bm{x}}(\tau\circ\sigma)$ is surjective on a set of positive masure of points  $\bm{x}\in\Delta^m$.
...
at these points $\Im\d_{\bm{x}}\sigma$ forms a subcone isometric to a Euclidean space in $\T_{\sigma(\bm{x})}$.
...
\section{Webs in finite-dimensional CAT spaces}
