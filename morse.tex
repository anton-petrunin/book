...
\section{Stratified spaces}\label{sec:background}
...
Let $X$ be a  compact $n$-dimensional $MCS$ space.
Then $X_{n-1}=\emptyset$ if and only if $K(X)_{n}=\emptyset$.
\end{thm}
\begin{proof}
 Suppose $X_{n-1}\ne\emptyset$.
...
Since $K(X)_{n}$ contains more than one point $\RR\times X$ also has nontrivial codimension 1 strata.   Let $X_{\le n-2}$ be the union of the strata of $X$ of dimension $\le n-2$.

Since $(\RR\times X)_n$ is an $n$-manifold and  $\RR\times X_{\le n-2}$ is at most $(n-1)$-dimensional there exists $p\in (\RR\times X)_n\setminus (\RR\times X_{\le n-2})$ i.e $p$ belongs to $\RR\times X_n$. But this means that  the pointed conical neighborhood of $p$ is homeomorphic to $\RR^{n+1}$ which contradicts the fact that $p$ lies in the  codimension 1 strata of $K(X)$.
\end{proof}
...
Then $X$ has nonempty codimension 1 strata if and only if $Y$ does.
\end{thm}
...
Let $X_{\le n-2}$ be the union of all strata of dimensions $\le n-2$. Then by the Alexander--Spanier duality we have $\bar H^n_c(X_l,\ZZ_2)=\bar H^n_c(X\setminus X_{\le n-2},\ZZ_2)\cong \bar H^n(X, X_{\le n-2},\ZZ_2)$. 
...
$\phi\: f^{-1}(I^k)\to f^{-1}(f(x))\times I^k$ respecting $f$ (that is such that $f=p_2\circ \phi$ and such that $\phi|K=F_x|K$).
\end{thm}
...
Then there exists an open embedding $\psi\co U\to X$, $\varkappa(\delta)$-close to $i$ and such that  $\psi\equiv\phi$ on $W$ and $\psi\equiv i$ on $U\setminus V$.

(Here $\varkappa$ depends on $W,U,V,X$ but not on $\phi$).
\end{thm}
\begin{proof}
Consider the open embedding $\phi |_{U\setminus \bar{W}}\co U\setminus \bar{W}\to X$. By the deformation theorem of Siebenmann~\cite[Theorem 5.4]{Sieb},  it can be perturbed to an open embedding $\phi_1\co U\setminus \bar{W}\to X$ which is $\varkappa(\delta)$-close to $i$,  coincides with $i$ on some neighborhood of $\partial V$ and is equal to $\phi$ outside some compact subset of  $U\setminus \bar{W}$. Now let

\[
\psi(x)=
\begin{cases}
\phi(x),\text{ for } x\in W\\
\phi_1(x), \text{ for } x\in V\setminus W\\
x, \text{ for } x\in U\setminus V
\end{cases}
\]
...
\[\tilde{X}\setminus \underset{\a\in \frak A\setminus\{\a_1,\a_2\}}  { \cup}\tilde{U}_\a\subset \phi'(U_1^4\cup U_2^4).\]
...
Let $U\Subset V\Subset W\subset X$ be relatively compact  open subsets in an MCS-space $X$.
Let 
...
It's easy to see that $f$ is spherically concave if and only if its  1-homogeneous extension to $C\Sigma$ is concave. In particular, if $f\:\spc{X}\to\RR$ is semi-concave, then $df_p\: \Sigma_pX\to \RR$ is spherically concave.
\end{comment}
...
\[
\dlang \dd_p\phi, \dd_p\psi\drang\le \sdk\kappa{p}{\phi}{\psi}
\]
...
\]
\item \label{der-der} If $h\in DER(\Sigma)$ and $p\in \Sigma$, then $\dd_ph\in DER(\Sigma_p\Sigma)$;
...
for the unit vectors $h,g,p$ in $\mathbb R^n$ we have that $\langle P(h),P(g)\rangle=\langle h,g\rangle-\langle h,p\rangle\cdot \langle h,p\rangle$ where $P$ is the orthogonal projection to $p^\perp$.
Of particular importance in  applications will be generalization of  the implication that if $\langle h,g\rangle<0$ and $\langle h,p\rangle>0$, $\langle g,p\rangle>0$, then $\langle P(h),P(g)\rangle<0$ also.

\begin{proof}
(\ref{der-commute}) is obvious.

To see (\ref{der-der})  let  $h=\sum_\a -a_\a\langle \cdot, {A_\a}\rangle$.
Then by the first variation formula we have
  \[\dd_ph=\sum_\a -a_\a\sin \angle A_\a p \langle \cdot, \Dir{p}{A_\a}\rangle\]
...
Also, by Proposition~\ref{prop:regular=+1} we have that  $\bm{\phi}\:\spc{L}\subto \RR^{\kay+1}$ is regular at $p$ if and only if there exists a point $a_{\kay+2}$ arbitrary close to $p$ such that $\bar {\bm{\phi}}=(\bm{\phi},\distfun{a_{\kay+1}}{}{})$ is a $\kappa$-strutting for $p$. 


Here we think of $\bar {\bm{\phi}}$ as a multi distance function in the natural way as follows.
If $\bm\phi\can F\circ\distfun{\bm{a}}{}{}$ where $\bm a=(a_0,\ldots, a_\kay)$,   then  $\bar{\bm\phi}\can \bar F\circ\distfun{\bar{\bm{a}}}{}{}$ where $\bar{\bm a}=(a_0,\ldots a_{\kay+1})$ and $\bar F(x_0,\ldots x_{\kay+1})=(F(x_0,\ldots x_{\kay}), x_{\kay+1})$. 
...
\[
\dd_p\phi^i(\xi_+)>\eps\qquad \text{ and } \dd_p\phi^i(\xi_-)<-\eps
\]
\end{subthm}


\begin{subthm}{} \label{pos-eps}
 For every $i=1,\ldots \kay$ there exists $\xi^i+\in \Sigma_p\spc{L}$ such that $\dd_p\phi^i(\xi^i_+)> \eps$ and $\dd_p\phi^j(\xi^i_+)=0$ for $j\ne 0, i$.
\end{subthm}
\begin{subthm}{} \label{neg-eps}
 For every $i=1,\ldots \kay$ there exists $\xi^i_-\in \Sigma_p\spc{L}$ such that $\dd_p\phi^i(\xi^i_-) <- \eps$ and $\dd_p\phi^j(\xi^i_-)=0$ for $j\ne 0, i$.
\end{subthm}
\end{thm}
 ...
 \[
\dlang \dd_p\phi^i, \dd_p\phi^j\drang\le \sdk\kappa{p}{\phi^i}{\phi^j}<-\eps
\]
For all $i\ne j$.

Let $\hat A^i\in T_p\spc{L}$ be the vector provided by Lemma~\ref{der-prop} (\ref{der-single-vector}) for $\dd\phi^i$.
By the sum Lemma out assumptions on $|dF|_1$ guarantee that $|\hat A^i|\le 1$ for all $i$.
 On the other hand for $i\ne j$ we have 
 \[
 -\eps>\dlang \dd_p\phi^i, \dd_p\phi^j\drang\ge \dlang \hat A^i, \dd_p\phi^j\drang\ge \dlang  \hat A^i, \hat A^j\drang=\langle  \hat A^i, \hat A^j\rangle
 \]
 ...
 To prove \eqref{incr-decr-all} For any $i>0$ we have
  ...
  \[
-\dd_p\phi^i(\xi_+)=\dlang \hat A^0, \dd_p\phi^i\drang\le \dlang \dd_p\phi^0, \dd_p\phi^i\drang\le \sdk\kappa{p}{\phi^i}{\phi^j}<-\eps
\]
 which means  that $\dd_p\phi^i( \hat A^0)>\eps$. Since $|\hat A^0|\le 1$ by the Sum Lemma (\ref{lem:sum}), we can set $\xi_+=\frac{\hat A^0}{|\hat A^0|}$.
 Next, let $u$ be polar to $\xi_+$. Then $|u|\le 1$ and  for any $i>0$ we have $\dd_p\phi^i(\xi_+)+\dd_p\phi^i(u)\le 0$ which implies that $\dd_p\phi^i(u)< -\eps$ for all $i>0$. Again we can set $\xi_-=\frac{u}{|u|}$. Then $\xi_+$ and $\xi_-$  satisfy \eqref{incr-decr-all}.
 
 
 Let us prove \eqref{pos-eps} and \eqref{neg-eps}. We will prove that for every $i>0$ there exists $v\in \Sigma_p\spc{L}$ such that $\dd_p\phi^i(v)\ge \eps$ and $\dd_p\phi^j(v)=0$ for $j\ne 0, i$ and  $\dd_p\phi^0(v)\le- \eps$.
 This will obviously  imply b). It  will  also imply  c) by interchanging $\phi^0$ and $\phi^i$.
 
To simplify notations let us denote $f^i=\dd_p\phi^i$.
...
Let $u\in \Sigma_v\Sigma_p\spc{L}$ be given by $u= \dd_vf^{j_0}$.
By the same argument as above  $d_vf^{j_0}<0$ and $d_vf^j(v)>0$ for all $j>0,j\ne j_0$.
Let $\beta(t)\in \Sigma_p\spc{L}\subset T_p\spc{L}$ (we identify $ \Sigma_p\spc{L}$ be a curve in the direction of $u$.
Then for small $t>0$ we have that 
...
\item \label{reg-colip} Suppose $\bm{\bar\phi}\can \bar F\circ\distfun{\bm{a}}{}{}=(\phi_0,\phi_1,\ldots \phi_\kay)$ is a $\kappa$-strutting such that $|dF|_1\le 1$ and
...
\begin{comment}
Let $\xi_i^{\pm}\in T_p\spc{L}$ be unit vectors provided by Lemma~\ref{lem-tight-diff}  so that $\dd_p\phi^i(\xi_i^-)<-\eps$, $\dd_x\phi^i(\xi_i^+)>\eps$ and $\dd_x\phi^j(\xi_i^\pm)=0$ for all $j\ne i$. We can assume that $v_i^\pm=\dir{p}{a_i^\pm}$ for some points $a_i^\pm$.

Also, without loss of generality $\phi^0=\distfun{a^0}{}{}$ and let $\xi^+=\dir{p}{a^0}$.

Then $\dd_p(\xi^+)>\eps$ for $i>0$ and  the same is true for all points $x$ near $p$.  
...
\item $(f,h)$ is regular on $\left(\bar{I}^k(f(p),r) \cap B(p,R)\right)\setminus S$.
\end{subthm}
\end{thm}
...
\end{equation}
 ...
 for some $\tilde\e>0$.
 Hence the map $\distfun{\bm{a}}{}{}\:\spc{L}\to\RR^{n+1}$ is tight in a neighbourhood  of $s$ and the restricted map $H=\distfun{\bm{a}^{\without 0}}{}{}\co \spc{L}\to\R^n$ is regular near $s$. Therefore, by the Inverse Function Theorem (~\ref{thm:inverse-function}) $H$ is an L-bilipschitz homeomorphism of the  ball $\oBall(s,R)$ for some $L,R>0$ onto an open subset of $\R^n$.
...
\end{proof}
\begin{thm}{Remark}
By part c) of  the Inverse Function Theorem (~\ref{thm:inverse-function}) it follows that the constants $L$ and $R$ (and hence also $c_1',c_1''$ and $c_1$) depend only on $\tilde\e,\kappa$, $\distfun{\bm{a}}{s}$ and $\dist{p}{q}{}$ (in fact, as mentioned above, $c_1''$ depends only on $\kappa$ and $\dist{p}{q}{}$). 
...
Denote $\Sigma_p^\e=\{\xi\in \Sigma_p \quad |\quad \dd_pf_j(\xi)>\e \text{ for all } j=1,\ldots,k\}$.

It easily follows from the definition that  $f$ is incomplementable at $p$ if and only if $\diam (\Sigma_p^0)\le \pi/2$.
...
If the equality in the middle is violated, then there exists a maximum point $r\in U_v^+\cap \bar{B}(p,\delta/2)$ such that $f_l(r)>v_l$ for some $l$. By applying \ref{incr-decr-all} to $\dd_rh, \dd_r f_j$ in $\Sigma_rX$ we can find a direction $\xi\in \Sigma_rX$ such that $\dd_rh(\xi)>0$ and $\dd_rf_j(\xi)>0$ for all $j\ne l$. This contradicts the fact that $r$ is a point of maximum of $h$ on $ U_v^+\cap \bar{B}(p,\delta/2)$. This proves~\eqref{max}.
...
Let $\xi=\dir{x}{s'}$.  By concavity of $h$ and $\lambda$-concavity of $f_j$'s it is obvious that $\dd_xh(\xi)>0$ and $\dd_xf_j(\xi)>0$ for all $j$. Combined with (\ref{e:grad}) this means that $(f,h)$ is regular at $x$.
...
We argue by reverse induction in $k$. 
If $k>n$, then the set of regular points is empty by Lemma~\ref{dim-ineq-strut} and there is nothing to prove. Suppose $k\le n$ and the theorem has been proved for $k+1$.
...
Then $U=W\setminus S$ and $(f,h)$ is regular on $U$. Therefore the  local fibration theorem holds for $(f,h)$ on $U$ by induction assumption.  
...
We claim that $(f,\tilde h)$ is admissible.
Indeed $(f,\tilde h)=G\circ (f,h)$ where $G\:\R^{k+1}\to\R^{k+1}$ is  given by $G(v,t)=(v, t-g(v))$.
...
a homeomorphism $W\to KF\times \bar{I}^k(f(p),r)$ in an obvious way (by mapping points of $x\in S$ to $\0\times f(x)$ where $\0$ is the vertex of $KF$).
...
\begin{thm}{Corollary}\label{sphere-neighb}
 ...
 \end{thm}
\begin{proof}
By ??? (is this proved somewhere yet?) there is $\eps>0$ such that  $f=\distfun{p}{}{}$ has no critical points in $B(p,2\eps)\setminus\{p\}$.
Therefore by the Fibration Theorem ~\eqref{fibration}, it's a bundle map from $B(p,2\eps)\setminus\{p\}$ to $(0,2\eps)$. This easily yields the result.
\end{proof}
...
$\Omega\subset\spc{L}\setminus\partial\spc{L}$ be an open set
