...
\section{Definition}\label{sec:wp-def}
...
\parbf{General fiber.}
The fiber-independence theorem proved below (\ref{thm:fiber-independence}) gives an explicit formula:
\[
...
A horizontal leaf is weakly convex, but does not need to be convex even if $\spc{B}\warp{f}\spc{F}$ is a geodesic space, since vanishing of the warping function~$f$ allows geodesics to bifurcate into distinct horizontal leaves.
...
Then  the proof that $L=L_{\Sigma}$  proceeds  as in the classical case, for length of an absolutely continuous curve in $\R^2$ \cite[p. 245--247]{goffman}.
