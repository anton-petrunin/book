...
Warped products is a construction that produce 
a new metric space denoted as $\spc{B}\warp f \spc{F}$
from two metric spaces $\spc{B}$, $\spc{F}$ and a function $f\:\spc{B}\to\RR_{\ge0}$;
the space $\spc{B}$ is called base and $\spc{F}$ --- the fiber of the warped product.
