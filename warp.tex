...
and $\gamma_\spc{F}$ is the projection.
...
\eqlbl{eq:length}
\]
where $\int$ is Lebesgue integral, $v_\spc{B}$ and $v_\spc{F}$ is the speeds of $\gamma_\spc{B}$ and $\gamma_\spc{F}$ correspondingly.
...
The corresponding metric space is called \emph{warped product with base $\spc{B}$, fiber $\spc{F}$ and warping function $f$}; it will be denoted by $\spc{B}\warp f\spc{F}$.

The points in $\spc{B}\warp f\spc{F}$ can be described by corresponding pair $(p,\phi)\in\spc{B}\times\spc{F}$; note that if $f(p)=0$ for some $p\in \spc{B}$, then $(p,\phi)=(p,\psi)$ for any $\phi,\psi\in \spc{F}$.

\begin{thm}{Proposition}
The warped product $\spc{\spc{B}}\warp{f}\spc{F}$ satisfies:

\begin{subthm}{horiz-leaf-proj}
The projection $(p,\phi_0)\mapsto p$ is a submetry which restriction to any horizontal leaf $\spc{B}\times\{\phi_0\}$
...
\begin{subthm}{Df>0}If  $f$ achieves its minimum at $p_0$, then the vertical leaf $\{p_0\} \times \spc{F}$, with its length metric, is locally isometrically embedded in $\spc{B}\warp{f}\spc{F}$.
\end{subthm}
...
In this case, set $\beta(t)=(\alpha_{\spc{B}}(t),\alpha_{\spc{F}}(t))\in \spc{B}\times\spc{F}$;
clearly 
\[\length \alpha= \length_f\beta.\]

If $f\circ\alpha_{\spc{B}}(t_0)>0$ for some $t_0$, set $\beta$ to be the concatenation of three curves in $\spc{B}\times\check{\spc{F}}$;
namely: 
(1) the horizontal curve $(\alpha_\spc{B}(t),\check\phi)$ for $t\le t_0$,
(2) a vertical path in form $(s,\check\phi)$ to $(s,\check\psi)$
and 
(3) the horizontal curve $(\alpha_\spc{B}(t),\check\psi)$ for $t\ge t_0$.
By \ref{eq:length}, the $f$-length of the middle path in the concatination is vanishing;
therefore the $f$-length of $\check\alpha$ is the same as length of $\alpha_{\spc{B}}$;
that is,
\[\length_f\alpha \ge 
...
It implies that if $\iota\:A\to \check A$ is an isometry between two subsets
...
Let $\spc{B}$ be a $\Alex\kappa$ length space and the function $f\:\spc{B}\to \RR_\ge$ satisfies $f''+\kappa\cdot f\le 0$;
denote by $Z\subset \spc{B}$ the zero set of~$f$.
Suppose $\spc{F}$ is a $\Alex{\kappa'}$ space.
...
Let $\spc{B}$ be a $\CAT\kappa$ length space and the function $f\:\spc{B}\to \RR_\ge$ satisfies $f''+\kappa\cdot f\ge 0$;
denote by $Z\subset \spc{B}$ the zero set of~$f$.
Suppose $\spc{F}$ is a $\CAT{\kappa'}$  space.
