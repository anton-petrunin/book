...
By \ref{horiz-leaf-proj}, $\length_f\gamma$ is the sum of $\length_f\gamma$ restricted to the nonzero set of $f\circ\gamma_\spc{B}$, and the length of $\gamma_\spc{B}$ restricted to the zero set. 
...
The points in $\spc{B}\warp f\spc{F}$ can be described by corresponding pairs $(p,\phi)\in\spc{B}\times\spc{F}$; note that if $f(p)=0$ for some $p\in \spc{B}$, then $(p,\phi)=(p,\psi)$ for any $\phi,\psi\in \spc{F}$.
...
Let us mention an alternative way to define the $f$-length that works for any Lipschitz curve $\gamma$ in the quotient space $\spc{B}\times\spc{F}/\sim$ where $(p,\phi)\z\sim (p,\psi)$ if $f(p)=0$.
We define $f$-length as a sum of two integrals --- one is for $\length_f\gamma$ as in \ref{eq:length} restricted to the nonzero set of $f\circ\gamma_\spc{B}$, and the the other for length of $\gamma_\spc{B}$ restricted to the zero set.
The two definitions agree by \ref{horiz-leaf-proj}, but with the latter approach this statement follow directly from the definition.
