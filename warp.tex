...
Let $\spc{B}$ and $\spc{F}$ be length spaces and $f,g\:\spc{B}\to \RR_\ge$ be two locally Lipschitz nonnegative  functions.
...
\]
In this way we get \index{$\kappa$-cones}\emph{$\kappa$-cones} over $\spc{F}$, denoted by $\Cone\mc\kappa\spc{F}=[0,\infty)\warp{\sn\kappa} \spc{F}$ for $\kappa\le 0$
...
Let $\spc{B}$ be a complete finite-dimensional $\Alex\kappa$ length space, and the locally Lipschitz function $f\:{\spc{B}}\to \RR_\ge$ satisfy $f''+\kappa\cdot f\le 0$.
If $\spc{B}^{(\dag)}$ is the result of gluing two copies of ${\spc{B}}$ on the closure of the set of boundary points where $f$ is nonvanishing, and $f^\dag\: \spc{B}^{(\dag)}\to\RR_\ge$ is the tautological extension of $f$, suppose $\spc{B}^{(\dag)}$ is $\Alex\kappa$ 
