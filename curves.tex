...
A \emph{curve}\index{curve} in $\spc{X}$ is a continuous map $\alpha\:\II\to \spc{X}$, where $\II$ is a \emph{real interval}\index{real interval} (i.e. an arbitrary convex subset of $\RR$);
...
Hopefully the following theorem is in \cite{kirchheim}CHECK???
...
we have
\[\dist{f(y)}{f(z)}{}=\lVert z-y\rVert_x+o(|y-x|+|z-x|).\]
Moreover if $s(x)=\sup_{w\not=0}\{\lVert w\rVert_x/|w|\}$ then 
...
%???This seems to be sufficient for us, but it is kind of weak def.For example: there is $\spc{L}\in\CBB\infty{}$ and a curve $\alpha\:\RR\to \spc{L}$ such that $\alpha^+(0)=\0$ but \[\liminf_{t\to 0}\frac{\dist{\alpha(0)}{\alpha(t)}{}}{|t|}>0.\]Also this def. is good together with Kuratowski tangent cone, and it is not that good with the standard definition.A.???
...
Acording to Rademacher's theorem, any Lipscitz real-to-real function is differentiable almost everywhere. Here its extension for Lipschitz curves in $\CBB{}{}$-spaces.
...
\begin{thm}{Rademacher's theorem for $\cCat{}{}$ spaces}\label{thm:rademacher-cba}
Let $\spc{U}\in\cCat{}{}$  
