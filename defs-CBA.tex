...
By the triangle inequality, the $z_n$ are approximate midpoints of $p^1$ and $p^2$. %???English???
...
and the \index{natural map}\emph{natural map} $\trig{\tilde x^1}{\tilde x^2}{\tilde x^3}\to \trig{x^1}{x^2}{x^3}$ 
...
Recall that a curve in $\Lob2\kappa$ is \index{concave curve}\emph{(locally) concave with respect to $p$} if (locally) its supergraph with respect to $p$ is a convex subset of $\Lob2\kappa$; see Definition~\ref{def:convex-devel}.
...
\parit{Proof of \ref{thm:alex-patch}.} It only remains to prove (\ref{SHORT.thm:alex-patch:geo-uni})$\Rightarrow$(\ref{SHORT.thm:alex-patch:ccat}).
...
Suppose 
$\spc{U}^1$ and $\spc{U}^2$ are %complete 
...
%closed 
%$\varpi\kappa$-convex sets $\spc{U}^1,\spc{U}^2\subset\spc{W}$ such that
%$\spc{U}^1\cup\spc{U}^2=\spc{W}$ and $\spc{U}^1$, $\spc{U}^2$ are $\CAT\kappa$ spaces.
%Then $\spc{W}$ is a $\CAT\kappa$ space.
Let $\spc{W}$ be a 
length space having two 
...
Geodesic paths of $\Cone\spc{U}^i$ that do not pass through the tip of the cone correspond, project to geodesics of length $<\pi$ in $\spc{U}^i$. 
...
%It follows that  $[0,1]$ is covered by finitely many intervals $\II_i$,  that are open in $[0,1]$, where $\gamma_\infty|_\II_i$ is  a geodesic that is the limit of geodesics $\gamma_n|_\II_i$. Therefore   $\gamma_\infty$ is a local geodesic path with $\length\gamma_n\to\length\gamma_\infty$.
...
\parit{Proof.} By Corollary \ref{cor:geo-hom}, substituting $\bar q=p$ for $q$ and taking  $\alpha^0(t)=p$ and  $\alpha^1(t)=\alpha(t)$ for all $t\in [0,1]$, there is an interval $\II$ such that (a) holds for all $t\in\II$, and either $\II=[0,1]$ or $\II=[0,a)$ for some $a\le 1$.
%Apply Corollary \ref{cor:geo-hom}, setting $\alpha^0(t)=\alpha(0)$ for all $t\in [0,1],$ and $\alpha^1=\alpha$. By \ref{cor:geo-hom-length}  
%S: note substitution
...
In this case, we say that $D$ \index{majorizes}\emph{majorizes} the curve $\alpha=F\circ\tilde \alpha$ under the map $F$.
\end{thm}
...
\parbf{Example.} Let $\spc{Q}$ be a solid quadrangle $[p x z y]$ in $\EE^2$, which is formed by two congruent triangles and is non-convex at $z$ (as in the picture).  
...
\[
\emph{Let $F_{n-1}$ be a majorizing map for the polygon $[x^1x^2\dots x^{n-1}]$,
...
We now use to prove the theorem for general curves.
...
Two majorizations $F\:D\to \spc{U}$ and $F'\:D'\to \spc{U}$ will be called \index{equivalent majorizations}\emph{equivalent} if $F'=F\circ\iota$ for an isometry $\iota\:D\to D'$.
...
By the classical arm lemma \cite{sabitov}, we get $\dist{\tilde x^0}{\tilde x^{n+1}}{}\le \dist{\tilde y^0}{\tilde y^{n+1}}{}$.
Since $ \dist{\tilde y^0}{\tilde y^{n+1}}{}=\dist{x^0}{x^{n+1}}{}$, part (\ref{SHORT.subthm:arm-ineq}) follows.

\parit{\ref{SHORT.subthm:arm-eq}).} Suppose equality holds in (\ref{SHORT.subthm:arm-ineq}).
...
%If $\mangle\hinge xyz=\angk\kappa xyz$, then  the restriction $f|_{([xy]\cup[xz])}$ is distance-preserving.\end{clm}
...
We have  given elementary proofs of the  first-variation statements \ref{lem:strong-angle-cba}, \ref{thm:1st-var-cba} and \ref{cor:both-end-first-var-cba}. Note however that the no-conjugate-point theorem \ref{thm:no-conj-pt} not only provides a proof of these statements, but also extends the statements  
...
Consider a majorization $F$ for quadrangle $[pp^{*}q^{*}q]$.
By Majoriztion theorem \ref{thm:major} and Proposition \ref{prop:majorize-geodesic}, 
the figure $F$ is a solid convex model quadrangle $[\tilde p\tilde p^*\tilde q^*\tilde q]$ in $\Lob2\kappa$ such that 
\begin{align*}
