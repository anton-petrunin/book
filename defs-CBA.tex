...
By the patchwork globalization theorem (\ref{thm:alex-patch}), 
...
$\eps>0$,
and $\gamma_1,\gamma_2\:\mathbb{S}^1\to\spc{U}$ be two closed curves.
Assume 
\begin{subthm}{}
...
Prove that any compact geodesic, locally $\CAT0$ space $\spc{X}$ that is not $\CAT0$ contains a \index{geodesic circle}\emph{geodesic circle};
...
Formulate and prove the analogous statement for $\CAT1$ spaces.
\end{thm}
...
Let $p$ be a point in a complete length $\CAT\kappa$ space $\spc{U}$, and $K\subset \spc{U}$ be a closed $\varpi\kappa$-convex set. 
...
Let us list important sources of examples of $\CAT{}$ spaces.
...
Subsets of \textit{positive reach} in  Riemannian manifolds were studied in this context by 
the first author, Richard Bishop, and
Alexander Lytchak \cite{alexander-bishop,lytchak2004}.
...
In particular, if  $\spc{U}$ is a $\CAT{0}$ space then its \textit{asymptotic cone}\index{asymptotyc cone}   defined as  the ultralimit of its rescalings $\tfrac1n\cdot \spc{U}$ as $n\to \o$ is again  $\CAT{0}$.
...
Among more conceptual examples, let us mention a result of Brian Clarke \cite{clarke}: the \textit{space of Riemannian metrics} on a compact, orientable smooth manifold
