...
Further, any open simply-connected \textit{subset of the plane} equipped with the induced length metric is $\CAT{0}$. 
...
In higher dimensions things getting more complicated;
see \cite[Chapter 4]{alexander-kapovitch-petrunin-CAT} for related questions (open and solved).
...
If  $\spc{X}$ is a $\CAT{0}$ space then its \textit{asymptotic cone}\index{asymptotyc cone}   defined as  the ultarlimit of its rescalings $\tfrac1n\cdot M$ as $n\to \o$ is again  $\CAT{0}$.
Unlike in the case of $\Alex{0}$ spaces in this case the use of ultralimits is necessary even if $\spc{X}$  is a manifold due to the lack of compactness theorem. Asymptotic cones of $\CAT{0}$ spaces and their generalizations play an important role in geometric group theory.
...
by Reshetnyak gluing theorem it is $\CAT0$.
(This observation was used in one of the most beautiful applications of the Reshetnyak gluing theorem is given by Dmitri Burago,  Serge Ferleger,
...
This fact was successfully used by Dmitri Burago, Serge Ferleger, and Alexey Kononenko \cite{burago-ferleger-kononenko1998-2} to bound the number of collisions of balls in $\EE^m$.)
...
By \ref{prop:CAT-olim} and \ref{thm:ultra-GH}, the \textit{ultralimits} and \textit{Gromov--Hausdorff limits} of $\CAT\kappa$ spaces are $\CAT\kappa$.

Further, $\CAT{}$ spaces behave nicely with respect to some natural constructions.
For example, the product of $\CAT{0}$ spaces is a $\CAT{0}$.
Also, the Euclidean cone over $\CAT{1}$ space is a $\CAT{0}$.
