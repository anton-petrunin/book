...
%\section{Sources of examples of $\CAT{}$ spaces}

\subsection{Metric trees}

From a basepoint $\spc{X^0}$ construct a metric tree $\spc{X}$ recursively by attaching $2^n$ intervals of length $2^{-n}$ to each end vertex of $X^{n-1}$. Clearly $\spc{X}$ is  $\CAT\kappa$ for any $\kappa$.
...
\begin{thm}{Question}
Which of the sets $A$, $B$ and $C$, if equipped with the induced length metric, form $\CAT{0}$ spaces?
\end{thm}

\begin{thm}{Answer}
No for $A$,  yes for $B$ and $C$.
\end{thm}

See Subsection 4.1 of \cite{AKP} for further discussion.

\subsection{polyhedra}

\begin{thm}
A Euclidean polyhedral space in which any two points can be connected by a unique geodesic is $\CAT 0$. (Exercise 12.4).
\end{thm}

\subsection{Subsets of the plane}

Consider an open simply connected subset of $\Lob2 0$, equipped with its induced length metric. The completion of the resulting space is $\CAT 0$.  See Theorem 4.4.1 of \cite{AKP}.

\subsection{puff pastry}

Let $(A^1 \dots A^n$ be an array of convex closed sets in $\EE^m$. Glue successive pairs of spaces along  $A^1 \dots A^n$, respectively.  The resulting space is a \emph{puff pastry}.

\begin{thm}
A puff pastry is a $\CAT(0)$ space.
\end{thm}

See Subsection 2.4 of \cite{AKP}.

\subsection{Warped products}
