...
\begin{thm}{Definition}
\label{def:asymptotic-cone} Let $\spc{X}$ be a metric space and $\o$ be a non-principal ultrafilter.

The \emph{asymptotic cone} $\frac{1}{\o}\blow\spc{X}$ 
%$\Cone^\o\spc{X}$ 
of $\spc{X}$  is defined by
\[
%\frac{1}{\o}\blow\spc{X}= \spc{X}_\o \text{\ \ \ \ where \  }
(\frac{1}{n}\blow\spc{X}\,, x)\to (\frac{1}{\o}\blow\spc{X},x_\o).
\]
\end{thm}
%The asymptotic cone is independent of the choice of basepoint $x$.
...
$\Cat{}{0}$ spaces $\spc{U}$ that are cocompact (namely, the union of images of  a compact subset under isometries of $\spc{U}$) and proper have two contrasting alternative behaviors:  if the  asymptotic cone  of $\spc{U}$ is not an $\RR$-tree (hence $\spc{U}$ is not hyperbolic; see Definition \ref{def:delta-hyperbolic} and Exercise \ref{prop:hyp-tree}), then $\spc{U}$ contains an isometric copy of the Euclidean plane.   
  This theorem was first stated by Gromov 
 \cite{gromov:hyp-groups}. In its original version for visibility Riemannian manifolds, the theorem was  proved by  Eberlein \cite{eberlein}.   As a brief but informative glance at the field, we give a related proof following \cite{bridson}. For a detailed discussion of ideas that appear  in the proof, in particular, visibility and the boundary at infinity of $\Cat{}{0}$ spaces, see \cite{BH}. 
...
Gromov's theory of hyperbolic spaces \cite{{gromov:hyp-groups}} captures many global features of negative curvature. The study of these spaces is a broad field associated with hyperbolic groups. For fundamental references, see \cite{ghys-dlHarpe, BH}. 
...
Triangle comparison immediately implies that any $\Cat{}{\kappa}$ space with $\kappa<0$ is  $\delta$-hyperbolic for some $\delta>0$ that depends only on $\kappa$.
...
In a complete $\Cat{}{0}$ space $\spc{U}$, two unit-speed geodesic rays are called equivalent (asymptotic) if the Hausdorff distance between their images is finite.  Given $x,y\in\spc{U}$ and a unit-speed geodesic ray $\gamma_x$ with  $\gamma_x(0)=x$,  there is a unique unit-speed geodesic ray $\gamma_y$ with  $\gamma_y(0)=y$ that is equivalent to $\gamma_x$.  Uniqueness is immediate from convexity of  $\spc{U}$, namely,  the distance between geodesics with the same parameter interval is a convex function of the parameter (see Exercise \ref{CAT-is-convex}).  If $\spc{U}$ is proper,  the selection theorem \ref{thm:selection}  provides a sequence of geodesics $\gamma_n$  from $y$   to $\gamma_x(n)$ that converges pointwise to $\gamma_y$.  (Properness is not required here;  see Exercise \ref{}.) 
...
A $\Cat{}{0}$ space $\spc{U}$ is \emph{uniformly visible}	if far-away geodesics subtend small angles in the following sense:  for any $\eps>0$  there exists $R(\eps)>0$ such that if a geodesic $]pq[$  lies outside a ball $\oBall(x,R(\eps))$, then $\angle\hinge x pq< \eps$.  

If  $\spc{U}$ is uniformly visible, then $\spc{U}$ is $R(\pi/2)$-hyperbolic, since for any triangle $[pqr]$ and $x\in ]pr[$ we have without loss of generality
 $\mangle\hinge xpq \ge \pi/2$.  Thus  the distance from $x$ to  $]pq[$ is $\le R(\pi/2)$.
...
%Let $G$ be the isometry group of $\spc{U}$.
 If $\spc{U}$ is not uniformly visible, then there exist $x\in\spc{U}$ and $c\ne \bar c\in\partial\spc{U}$ such that 
\[\dist{c}{\bar c}{\angle}=\angle\hinge x {c} {\bar c} \le\pi/2.\]  
%\[
%\angle\hinge p {c^1} {c^2}=\sup_{g\in G} \dist{g(c^1)}{g(c^2)}{\angle}\le\pi/2.
%\]
\end{clm}

There exists $\eps>0$ such that for any positive integer $n$, we may take segments $[p_nq_n]$ that lie outside some ball $\oBall(x_n,n)$ and satisfy  $\angle\hinge {x_n} {p_n} {q_n}\ge\eps$.  By cocompactness, we may assume the $x_n$ lie in a compact set.  For a limit point $x$, the segments $[p_nq_n]$  lie outside $\oBall(x,n)$, and by angle semicontinuity \ref{lem:ang.semicont}, satisfy $\angle\hinge {x} {p_n} {q_n}\ge\eps$.  Let $r_n$ be the  nearest point of $[p_nq_n]$ to $x$.  Then $\angle\hinge {r_n}{p_n} {x}\ge\pi/2$ by the first variation formula \ref{cor:both-end-first-var-cba}; an exception may occur if $r_n=p_n$, in which case we relabel $p_n$ and $q_n$.  Since the angle sum of triangle $[x p_n r_n]$ is $\le \pi$, then  by relabeling if necessary when $r_n$ is not an endpoint, we have $\eps/2\le \angle\hinge {x}{p_n} {r_n}\le\pi/2$.  Since $\spc{U}$ is proper, by  the 
selection theorem \ref{thm:selection} and \ref{lem:cat-unique}, we may assume $[xp_n]$ and $[xr_n]$ respectively converge pointwise to geodesic rays in $c$, $\bar c$ $\in$ $\partial \spc{U}$.

For any $y\in\spc{U}$, we have $\angle\hinge {r_n}{x} {y}\to 0$ by angle comparison \ref{cat-hinge}.  Given $\delta>0$, the triangle inequality for angles based at $r_n$ gives $\angle\hinge {r_n}{p_n} {y}\ge\pi/2-\delta$ for $n$ sufficiently large.  Then $\angle\hinge {y}{p_n} {r_n}\le\pi/2+\delta$ by angle comparison for triangle $[yp_nr_n]$.   It follows that $\angle\hinge {y}{c} {\bar c}\le\pi/2$.  Since $y$ was chosen arbitrarily,  $\dist{c}{\bar c}{\angle}\le\pi/2$. 

Now choose $x^n, c^n,\bar c^n$ so that 
\[
\angle\hinge{x^n} {c^n} {\bar c^n}  \to 
\sup \bigl\{\angle\hinge{x^n} {g(c^n)} {g(\bar c^n)} \:g\in G\}
\]
where $G$ is the isometry group of $\spc{U}$.  By cocompactness, we may assume $x^n$ lies in a compact subset of $\spc{U}$.  But then, by properness, the supremum is realized by $\angle\hinge p c {\bar c}$ for some choice of $p, c, {\bar c}$.  The claim follows.

Consider $x\in\spc{U}$ and $c\ne \bar c\in\partial\spc{U}$ as in \ref{clm:realize-angle}. Let $\gamma^1\in c$, $\gamma^2\in\bar c$ be  based at $x$, and $\gamma^3 \in \bar c$  be based at $\gamma_1(t)$.  By adjacent angle comparison \ref{cat-2-sum}, 
\[
\angle\hinge {\gamma^1(t)} {\gamma^1(0)} {\gamma^3(1)} \ge \pi - \angle\hinge {\gamma^1(t)} {c^1} {c^3} \ge \pi -  \angle\hinge x {c} {\bar c}. \eqlbl{arm-flat-plane}
\]
Therefore $\cos\angle\hinge {\gamma^1(t)} {\gamma^1(0)} {\gamma^3(1)} + \cos\angle\hinge {\gamma^1(t)} {\gamma^1(0)} {\gamma^3(1)} \le 0$, and the function $f(t)=\dist{\gamma^1(t)}{\gamma^3(t)}{}$ satisfies  $f^+(0)\ge0$ by the first variation formula \ref{cor:both-end-first-var-cba}. Since $f$ is convex and bounded, $f$ is  constant and the inequalites in \ref{arm-flat-plane} are equalities. It follows from the Arm Lemma \ref{lem:arm} that the convex hull of  $\gamma^1$ and $\gamma^2$ is isometric to a sector in the Euclidean plane.
%the nested regions 
...
there is a subsequence $\Phi_{1,i}$ of $\Phi_{0,i}$ such that  $\Phi_{1,i}(y_1)\to x_1\in \spc{U}$. Proceeding recursively, we obtain a sequence of  successive subsequences $\Phi_{n ,i}$ such that $\Phi_{i,i}(y_n)\to x_n$ for each $n$.  Since $\dist{y_n}{y_m}{}= \dist{\Phi_{i,i}(y_n)}{\Phi_{i,i}(y_m)}{\spc{U}}=\dist{x_n}{y_n}{\spc{U}}$, an isometric embedding of the plane into $\spc{U}$ is given by continuous extension of the map $y_n\mapsto x_n$.
\qeds