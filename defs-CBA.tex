...
In the following form the theorem appears in the book of Martin Bridson and Andr\'e Haefliger.\cite{BH}.
...
Let $\theta(\hat\gamma)$ be the length tranced by the ends of $\hat\gamma_t$;
...
such that $\hat\alpha(0)=\hat p$ and $\map\circ\hat\alpha=\alpha$; in particular $\hat\alpha_t(1)\equiv\alpha(t)$. 
By \ref{clm:loc-iso}, 
...
(\ref{cat-hinge}).
By first variation formula (\ref{cor:both-end-first-var-cba}), it follows that each pair of corresponding angles of triangles $[\tilde x \tilde y \tilde z]$ and $[x y z]$ are equal.
