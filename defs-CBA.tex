...
It remains to prove the converse for \textit{(\ref{SHORT.cat-monoton})}.
...
The following model-space lemma is is extracted from Lemma 2 in \cite{reshetnyak:major}.
...
\parit{Proof of \ref{thm:function-comp}.} Fix a sufficiently short geodesic $[x y]$ in $\oBall(p,\varpi\kappa)$.
...
follows directly from $\varpi\kappa$-convexity of $A^i$.
...
Show that the sum of external angles of any polygon in complete length $\CAT0$ space cannot be smaller than $2\cdot\pi$. 
\end{thm}
...
Show that the sum of external angles of $\beta$ does cannot be smaller than $4\cdot\pi$.
