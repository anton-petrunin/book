...
Section~\ref{sec:cba-def} defines $\cCat{}{\kappa}$ spaces via a (2+2)-point comparison; it concerns only distances 
...
Note that by definition completion of a $\Cat{}{\kappa}$ space is a $\cCat{}{\kappa}$.
From Lemma on uniqueness of geodesics, see \ref{lem:cat-unique} below,
it follows that any $\cCat{}{\kappa}$ space is $\varpi\kappa$-geodesic,
in particular $\cCat{}{\kappa}\subset \Cat{}{\kappa}$.
It often convinient to think of $\Cat{}{\kappa}$ spaces as $\varpi\kappa$-convex (not necesary closed) subset of a $\cCat{}{\kappa}$ space.
...
The next proposition follows directly from Definition \ref{def:ccat} and the definitions of ultralimit and ultrapower (see Section~\ref{ultralimits}).
...
\begin{thm}{Lemma on uniqueness of geodesics}\label{lem:cat-unique} 
...
Then any ball (close or open) of radius $R\le\tfrac{\varpi\kappa}2$ in $\spc{U}$ is $\varpi\kappa$-convex.
\end{thm}
...
\parit{Proof.}
Follows from the uniqueness of geodesics in $\cCat{}{\kappa}$ spaces (\ref{lem:cat-unique}).
\qeds
...
\begin{thm}{Exercise}\label{ex:short-retraction-CBA(1)}
...
Lemma \ref{lem:cat-unique} implies that the geodesic $[x\alpha(s)]$ 
...
\begin{subthm}{}
If $\kappa<0$ then the function 
\[f(x)=\exp(\sqrt{-\kappa}\cdot\bus_\gamma(x))\] 
satisfies
\[f''+\kappa\cdot f\ge 0\]
\end{subthm}
...
According to Corollary~\ref{cor:convex-balls}, we may assume that
\begin{itemize}
...
Suppose 
$\spc{U}_1$ and $\spc{U}_2$ are $\varpi\kappa$-geodesic spaces 
...
we can choose $r>0$ so that the restriction of $\map$ to $\oBall(\hat p,r)$ is distance preserving.
By Lifting globalization, the image  $\Phi(\oBall(\hat p,r))$ coincides with the ball
$\cBall(p,r)$.
This proves the first part of theorem.

To prove the second part, lat us choose $\rho_p$ to be maximal value $\le\tfrac{\varpi\kappa}2$ such that $\cBall(p,\rho_p)\in\Cat{}{\kappa}$.
By Corollary~\ref{cor:convex-balls}, the ball
\[\oBall(q,\rho_p-\dist{p}{q}{})\] 
is convex in $\oBall(p,\rho_p)$.
Therefore  
\[\oBall(q,\rho_p-\dist{p}{q}{})\in\Cat{}{\kappa}\]
