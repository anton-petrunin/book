...
\begin{thm}{Definition}
\label{def:R-tree}
A \emph{metric tree} is a metric space in which:
\begin{subthm}{}
...
\begin{thm}{Proposition}
\label{prop:R-tree}
A metric space $\spc{U}$ is a  metric tree if and only if 
...
\]
\end{thm}
Tbe limit $\spc{X}_\o$ does not depend on the choice of basepoint $x$.

\begin{thm}{Proposition}
Suppose $\spc{U}\in \Cat{}{\kappa}$ where $\kappa <0$. Then the asymptotic cone $\Cone_\o\spc{X}$ is a metric tree.
\end{thm}
...
\begin{thm}{Definition}
\label{def:delta-hyperbolic} Let $\spc{X}$ be a geodesic metric space.  Then $\spc{X}$ is a \emph{$\delta$-hyperbolic space} if for any triangle in $\spc{X}$, any point of any side is within distance $\delta$ from one of the other two sides.
\end{thm}

$\delta$-hyperbolic spaces are only interesting when unbounded.  The canonical example is hyperbolic space.  Triangle comparison immediately implies that any $\Cat{}{\kappa}$ space with $\kappa<0$ is  $\delta$-hyperbolic for some $\delta>0$ that depends only on $\kappa$.
