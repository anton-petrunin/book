...
(This is the CAT analog of Definition \ref{df:cbb1+3} of CBB.)
\end{thm}
...
By Property~\ref{md-diff-eq}, we have $\tilde f''\z=1-\kappa\cdot  \tilde f$.
...
\]
The the angle semicontinuity (\ref{lem:ang.semicont}) implies ``$\le$'' in \ref{eq:2-side-variation}, and ``$\ge$'' holds by the triangle inequality for angles, since angle comparison (\ref{cat-hinge}) gives 
\[
...
Geodesics in the complement of the tip of $\Cone\spc{U}^i$ project to geodesics of length $<\pi$ in $\spc{U}^i$. 
...
These statements will be used in the proof of the Hadamard--Cartan theorem (\ref{thm:hadamard-cartan}) and the lifting globalization theorem (\ref{thm:globalization-lift}), in much the same way as  the exponential map is used in Riemannian geometry.
...
If $\alpha_s$ for some value of $s$ does not converge as $t\to a-$, then condition (\ref{SHORT.cor:geo-hom-cauchy}) holds.  If each $\alpha_s$  converges as $t\to a-$,  
...
If $\tilde \gamma$ does not contain the point $[\tilde x^{k-1}\tilde x^{k+1}] \cap [\tilde x^{k}\tilde x^{n+1}]$, it follows from the induction hypothesis and (i) (ii), (iii) that  $\gamma = F\circ\tilde \gamma$ is a local geodesic of length $<\varpi\kappa$.
...
The base of the induction is Exercise \ref{ex:isometric-majorization}.
It finishes the proof of part \textit{(\ref{SHORT.subthm:arm-eq})}.
\qeds
...
\parit{Proof.} Since $\varpi\kappa=\infty$, the map $\map\:\spc{B}\to\spc{U}$ in Theorem~\ref{thm:globalization-lift} is a \index{metric covering}\emph{metric covering};
