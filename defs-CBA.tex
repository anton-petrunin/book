...
The following theorem was proved by Yurii Reshetnyak \cite{reshetnyak:glue}, assuming $\spc{U}_1$, $\spc{U}_2$ are proper and complete.  In the following form, where $\spc{U}_1$ and $\spc{U}_2$ are not assumed complete, the theorem appears in \cite{BH}.
...
$\spc{U}_1\cup\spc{U}_2=\spc{W}$, $A=\spc{U}_1\cap \spc{U}_2$ is complete and  $\varpi\kappa$-convex in $\spc{W}$, and $\spc{U}_1$, $\spc{U}_2$ are $\CAT\kappa$ spaces.
Then $\spc{W}$ is a $\CAT\kappa$ space.
\end{thm}
...
$\spc{W}$ is $\varpi\kappa$-geodesic if $\kappa\le0$.
\end{clm}
For $p_1\in \spc{U}_1$, $p_2\in \spc{U}_2$, we may choose a sequence $z^i\in A$ such that $|p_1z^i|+|p_2z^i|$
 ...
 In that case, the limit point $z$ of $z^i$ satisfies $|p_1z|+|p_2|=|p_1p_2|$, so the geodesics $[p_1z]$ in $\spc{U}_1$ and $[p_2z]$ in $\spc{U}_2$ together give a geodesic $[p_1p_2]$ in $\spc{U}$.  
 
 Suppose $z^i$ is not a Cauchy sequence.  Then there are subsequences  $x^i$ and $y^i$ satisfying  $\lim|x^iy^i|>0$. Let $m^i$ be the midpoint of $[x^iy^i]$. For any $\epsilon >0$, we may assume (taking subsequences and possibly relabeling $p_1$ and $p_2$)
 \[
 |p_1m^i\ge  |p_1x^i|-\epsilon,  |p_1m^i\ge  |p_1y^i|-\epsilon|.
 \]
 
 Since triangle $\trig{p^1}{px^i}{y^i}$ is thin, the analogous inequalities hold for the Euclidean model triangle  $\trig{\~p^1}{\~x^i}{\~y^i}$.  
 Thus there is a nondegenerate limit triangle $\trig{p}{x}{y}$ in the Euclidean plane satisfying $|px|=|py|\le|pm|$ where $m$ is the midpoint of $[xy]$.  This  contradiction proves the claim.
 
\claimqeds

Finally consider the case $\kappa>0$; by  scaling, take $\kappa=1$.
By Theorem \ref{}, $\Cone\spc{U}_i$ is a $\CAT0$ space for $i=1,2$. Geodesics of $\Cone\spc{U}_i$ that do not pass through the vertex of the cone are carried by projection from the vertex to geodesics of $\spc{U}_i$ of length $<\pi$, by \ref{}. 
  It follows that $\Cone A$ is $\pi$-convex in $\Cone\spc{U}_1$ and $\Cone\spc{U}_2$. By the cone distance formula \ref{}, $\Cone A$ is complete since $A$ is complete.
    Gluing on $\Cone A$ and applying \ref{clm:geod-gluing0 } for $\kappa=0$, we find that 
...
By classical arm lemma (\cite{arm-lemma}) it follows that $\dist{\~x^0}{\~x^{n+1}}{}\le \dist{\~y^0}{\~y^{n+1}}{}$.
