...
\parbf{Example.} Let $\spc{Q}$ be a solid quadrilateral $[p x z y]$ in $\EE^2$ formed by two congruent triangles, which is non-convex at $z$ (as in the picture).  
...
Assume a closed curve $\alpha$ in a  $\CAT0$ space is not a triangle.
Show that exactly one of these statements holds:

\begin{subthm}{}
$\alpha$ is majorized by non-isometric convex regions in $\Lob20$;
\end{subthm}
...
% ??? S: Would  "quadrangle" be  better than "quadrilateral", in analogy to "triangle"?
%We may change quadrilateral to quadrangle everywhere if you feel it is better
