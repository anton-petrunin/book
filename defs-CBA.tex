...
We begin by defining $\Cat{}{\kappa}$ spaces and spaces of curvature $\le\kappa$ via a (2+2)-point comparison that concerns only distances and makes no assumptions on the presence of geodesics (Sectf{sec:cba-def}).  
...
A definition using the Kirszbraun short-map extension property may be found in Section~\ref{sec:4pt}.   % ???   Berg-Nikolaev.
...
\label{def:ccat}
%A complete $\varpi\kappa$-length space $\spc{U}$ 
...
Note that according to Proposition~\ref{prop:submodel},
...
However, it is true that an infinitesimal bound always implies a local bound; that is, if
...
applied to triangle $[p^1 p_n^1 z]$, the $z_n$ are approximate midpoints of $p^1$ and $p^2$:
\[\dist{p^1}{z_n}{},\dist{p^2}{z_n}{}
\to\tfrac12\cdot\dist[{{}}]{p^1}{p^2}{}
\ \text{as}\ n\to\infty.
\eqlbl{eq:to|p1p2|/2}\] 
Apply (2+2)-point comparison (\ref{def:2+2}) to the quadruple $p^1,p^2,z_n,z$. 
For either $p=p^1$ or $p=p^2$, by continuity of the function $\tangle\mc\kappa\{a;b,c\}$ we see that $\angk{\kappa} {p}{z_n}{z}$ is arbitrarily small when $n$ is sufficiently large.  
By 
continuity of $\side\kappa\{\phi;b,c\}
$, \ref{eq:z_n->z} follows.
\qeds
...
\parit{Proof.} 
Choose a sequence of approximate midpoints $z_n$ for $p^1$ and $p^2$, as in \ref{eq:to|p1p2|/2}.
Then $\angk{\kappa} {p^1}{z_n}{p^2}$ and $\angk{\kappa} {p^2}{z_n}{p^1}$ are arbitrarily small when $n$ is sufficiently large.
Let us apply (2+2)-point comparison (\ref{def:2+2}) to the quadruple $p^1,p^2,z_n,z_\kay$ with large $n$ and $\kay$.
For either $p=p^1$ or $p=p^2$,
we conclude that  $\angk{\kappa} {p}{z_n}{z_\kay}$ is arbitrarily small when $n,\kay$ are sufficiently large.  
Therefore $(z_n)$ is a Cauchy sequence.  
Since $\spc{U}$ is complete,  $(z_n)$ converges to a midpoint for $p^1$ and $p^2$. 
Then the existence of a geodesic $[p^1p^2]$ follows from Lemma~\ref{lem:mid>geod}.
\qeds
...
\parit{Proof.} By Theorem \ref{thm:cat-complete}, it is only necessary to verify that $\spc{U}^c$ is a $\varpi\kappa$-length space; this is straightforward.
\qeds
...
\begin{thm}{Definition of $\kappa$-thin triangles%make kappa bold???
}\label{def:k-thin}
...
%\begin{thm}{Theorem (Development criterion)\index{comparison!development comparison}}\label{thm:concave-devel} 
%Suppose $\spc{U}$ is a metric space in which any two points at distance $<\varpi\kappa$ are joined by a geodesic.
%Then  $\spc{U}\in\Cat{}{\kappa}$ if and only if for any $p\in \spc{U}$, and any geodesic $\gamma:[0,T]\to\spc{U}$ 
%for which the triangle $\trig{p}{\gamma(0)}{\gamma(T)}$ has perimeter $<2\cdot\varpi\kappa$,
%the $\kappa$-development $\~\gamma$ in $\Lob2\kappa$ of $\gamma$ with respect to $p$ is locally concave. 
%\end{thm}
...
According to  (\ref{SHORT.thm:alex-patch:ccat})$\Rightarrow$(\ref{SHORT.thm:alex-patch:geo-uni}) of Patchwork globalization  (\ref{thm:alex-patch}), 
any two points in $\oBall(p,R)$ can be joined by a unique geodesic, which depends continuously on its endpoints.
%!!!??? here we have a double redirect --- we refer to part of theorem, which is actually lemma about uniqueness of geodesics.
...
%and two distance preserving maps
%$\iota_0\:\Omega^0\to \spc{N}$ 
%and 
%$\iota_1\:\Omega^1\to\spc{N}$ 
%such that 
%$\dist{\iota_0(x)}{\iota_1(y)}{\spc{N}}=\length\gamma_{x y}$
%for any  pair $x,y\in \Omega^0\times \Omega^1$.
\end{subthm}
\end{thm}
...
%Assume all local geodesics in $\spc{U}$ have length at most $\ell<\varpi\kappa$.
%Then for any path $\alpha\:[0,1]\to \spc{U}$
%and any local geodesic path $\gamma\:[0,1]\to \spc{U}$ 
%such that $\gamma(1)=\alpha(0)$
%there is unique continuous family of local geodesic paths $\gamma_t\:[0,1]\to \spc{U}$
%such that $\gamma_0=\gamma$, $\gamma_t(0)=x$ and $\gamma_t(1)=\alpha(t)$ for all $t$.
%\end{thm}
...
%$p\in\spc{U}$, $R>0$,
%and $\curv_x\spc{U}\le\kappa$ for any $x\in\oBall(p,R)$
%and $\cBall[p,\bar R]$ is complete for any $\bar r<r$.
%Assume 
and $\alpha\:[0,1]\to \spc{U}$ is a path of length $\ell<\varpi\kappa$.
%such that
%\[\dist{p}{\alpha(0)}{}+\dist{p}{\alpha(1)}{}+\ell<2\cdot r.\]
...
for any path $\alpha\:[0,1]\to\spc{U}$ with $\alpha(0)=p$ and $\length\alpha<\varpi\kappa/2$, %???Maybe we want $\length\alpha\le\varpi\kappa/2$???
...
%that is, $\geodpath_{[p z]}\rightrightarrows
%\geodpath_{[p x]}$ as $z\to x$. 
%???where defined?
...
This theorem is proved in \cite{reshetnyak:major};
...
%i.e. respectively, the join of geodesics $[x^1x^2]$, $[x^2,x^3],\dots,[x^{n-1}x^n]$ and the closed curve formed by joining $[x^1x^2],\dots,[x^{n-1}x^n], [x^n x^1]$.
...
\parit{Proof of Reshetnyak majorization theorem (\ref{thm:major}).}
...
%\eqlbl{eq:arm-angle}
%\]
%to get the angle at $\~x^{i, i}$. The vertices $\~x^{0,i-l},\dots, \~x^{i-1, i-1}$ are moved rigidly while the vertices $\~x^{i,i-1},\dots, \~x^{n+1,i-1}$ remain fixed, i.e. $\~x^{j,i- 1}= \~x^{j,i}$ for $i\le j \le n + 1$. 
...
The Hadamard--Cartan theorem was first stated for geodesic spaces by Gromov  \cite[p.119]{gromov:hyp-groups}. 
...
%One may consider the following alternative to definition of $\cCat{}{}$ space which is apparently more standard in the literature. 

%\begin{thm}{Definition}
%\label{def:cat}
%A space $\spc{U}$ 
%is called a \emph{$\Cat{}{\kappa}$ space} (also written $\spc{U}\in \Cat{}{\kappa}$) 
%if every quadruple $p^1,p^2,x^1,x^2$ whose subtriples have perimeter $<2\cdot\varpi\kappa$ satisfies the (2+2)-point comparison (\ref{def:2+2}).  
%\end{thm}

%According to \ref{lem:cat-complete},  $\cCat{}{\kappa}$ space is a $\Cat{}{\kappa}$ space.
%Thus the above definition may be considered as a generalization of definition \ref{def:2+2}.

%On the other hand it is straightforward to check that for any $\spc{U}\in \Cat{}{\kappa}$ we have $\spc{U}^c\in\cCat{}{\kappa}$, 
%where $\spc{U}^c$ denotes completion of space $\spc{U}$.
%Therefore any $\Cat{}{\kappa}$ space can be uniquely presented as a convex everywhere dense subset of a $\cCat{}{\kappa}$ space.

%This observation makes possible to extend directly 
%many results for $\cCat{}{\kappa}$ space to all $\Cat{}{\kappa}$ spaces;
%it includes ???.
%Such a direct generalization does not work if the theorems
%which imply existence some objects in the space.
%One example is Majorazing theorem (\ref{thm:major}).
%However, the proof which we give works for $\Cat{}{\kappa}$ spaces just as well; one only has to check that limit map in the very end has image in the  space (formally it lies in the completion).%???SAY MORE 
%For many theorems as ???, it is not known whether it 
...
\begin{subthm}{CAT-is-convex}
Show that any $\Cat{}{0}$ space is convex.
\end{subthm}
...
%Let $\spc{U}\in\Cat{}{\kappa}$.
%Assume 
%\[\angk\kappa xyp=\angk\kappa xzp\]
%for some $p,x,y\in \spc{L}$ and $z\in\l]xy\r[$.
%Prove that 
%\[\mangle\hinge xyp=\angk\kappa xyp.\]
%\end{thm}
...
\begin{thm}{Definition}
\label{def:R-tree}
An $\RR$-tree is a metric space $\spc{U}$ such that 
$\spc{U}\in\Cat{}{\kappa}$ for every $\kappa\in\RR$.
\end{thm}

The following characterization follows from point-on-side triangle comparison (\ref{cat-monoton}):

\begin{thm}{Proposition}
\label{prop:R-tree}
A geodesic metric space $\spc{U}$ is an $\RR$-tree
if and only 
\begin{subthm}{}
 any two points are joined by a unique geodesic, 
\end{subthm}
\begin{subthm}{}
if $[p x]\cap [x q] = \{x\}$ then $[p x]\cup [x q] = [p q]$.
%
%if two geodesic segments intersect only at an endpoint of each, then their union is a geodesic segment.
\end{subthm}
%is a geodesic metric space in which any two points are connected by a unique simple path.
\end{thm}

\begin{thm}{Definition}
\label{def:asymptotic-cone} Let $\spc{X}$ be a metric space and $\o$ be a non-principal ultrafilter.  Consider the $\o$-limit  
\[
\frac{1}{\o}\blow\spc{X}
\df
\lim_{n\to\o} \frac{1}{n}\blow\spc{X}.\]
Given a basepoint $x\in \spc{X}$, the sequence $\frac{1}{n}\blow x\in \frac{1}{n}\blow\spc{X}$ corresponds to a point $\frac{1}{\o}\blow x\in \frac{1}{\o}\blow\spc{X}$.
The \emph{asymptotic cone} 
of $\spc{X}$  is the metric component of $\frac{1}{\o}\blow\spc{X}$ containing $\frac{1}{\o}\blow x$.
\end{thm}

Since $\dist{(\frac{1}{\o}\blow x)}{(\frac{1}{\o}\blow y)}{} = 0$ for any $y\in \spc{X}$, the asymptotic cone is independent of the choice of basepoint $x$.

%\begin{thm}{Proposition}
%Suppose $\spc{U}\in \Cat{}{\kappa}$ where $\kappa <0$. Then the asymptotic cone $\Cone_\o\spc{U}$ is an $\RR$-tree.
%\end{thm}
%
%\parit{Proof.} 
%Immediate from Proposition  \ref{prop:R-tree}, since  $\Cone_\o\spc{X}\in\Cat{}{\kappa}$ for every $\kappa$ (\ref{prop:CAT^omega}). 
%\qeds 

$\Cat{}{0}$ spaces $\spc{U}$ that are cocompact (namely, the union of images of  a compact subset under isometries of $\spc{U}$) and proper have two contrasting behaviors:  if the  asymptotic cone  of $\spc{U}$ is not an $\RR$-tree, 
%(hence $\spc{U}$ is not hyperbolic; see Definition \ref{def:delta-hyperbolic} and Exercise \ref{prop:hyp-tree})
then $\spc{U}$ contains an isometric copy of the Euclidean plane.  This theorem was first stated by Gromov 
 \cite{gromov:hyp-groups}.  We give a proof following \cite{bridson}.  In its original version for Riemannian manifolds with the visibility property, the theorem was  proved by  Eberlein \cite{eberlein}.   For a detailed discussion of ideas that appear  in the proof, particularly visibility and the boundary at infinity of $\Cat{}{0}$ spaces, see \cite{BH}. 
 Kleiner has examined the supremum of $r$ such that $\spc{U}$ contains an isometric copy of Euclidean $r$-space; in particular, $\sup\{r\}=\LinDim\frac{1}{\o}\blow\spc{X}$ \cite{kleiner}.  

Gromov's theory of hyperbolic spaces \cite{{gromov:hyp-groups}} captures many global features of negative curvature. The study of hyperbolic spaces is a large field associated with hyperbolic groups. For fundamental references, see \cite{ghys-dlHarpe, BH}. 

\begin{thm}{Definition}
\label{def:delta-hyperbolic} Let $\spc{X}$ be a geodesic metric space.  Then $\spc{X}$ is a \emph{$\delta$-hyperbolic space} if for any triangle in $\spc{X}$, any point of any side has distance $\le\delta$ from the union of the other two sides.  $\spc{X}$ is \emph{hyperbolic} if $\spc{X}$ is $\delta$-hyperbolic for some $\delta$.
\end{thm}

Triangle comparison implies that any $\Cat{}{\kappa}$ space with $\kappa<0$ is  $\delta$-hyperbolic for some $\delta>0$ that depends only on $\kappa$.

\begin{thm}{Exercise}
\label{prop:hyp-tree}
The asymptotic cone of a hyperbolic space is an $\RR$-tree.
\end{thm}  

\begin{thm}{Flat planes theorem}\label{thm:flat-planes}
Let $\spc{U}$ be a proper, cocompact $\Cat{}{0}$ space.  Then  $\spc{U}$ is hyperbolic if and only if $\spc{U}$ does not contain a subspace isometric to the Euclidean plane.\end{thm}

In a complete $\Cat{}{0}$ space $\spc{U}$, two geodesic rays $\gamma,\bar\gamma$ are called equivalent (asymptotic) if $\dist{\gamma(t)}{\bar \gamma(t)}{}<c$ for some $c$ and all $t$.  
Given $x,y\in\spc{U}$ and a geodesic ray $\gamma_x$ with  $\gamma_x(0)=x$,  there is a unique geodesic ray $\gamma_y$ with  $\gamma_y(0)=y$ that is equivalent to $\gamma_x$ 
(Exercise \ref{exercise:CAT(0)-infinity}). 
Here uniqueness of $\gamma_y$ is immediate from convexity of  $\spc{U}$, according to which the distance between geodesics with the same parameter interval is a convex function of the parameter (see Exercise \ref{CAT-is-convex}).    In Theorem \ref{thm:flat-planes}, since 
$\spc{U}$ is assumed to be proper, existence of $\gamma_y$ follows directly by applying the selection theorem \ref{thm:selection}, together with  uniqueness and continuity of geodesics (\ref{lem:cat-unique}), to  a sequence of geodesics $\gamma_n$  from $y$   to $\gamma_x(n)$. 

We denote the set of equivalence classes of geodesic rays by $\partial\spc{U}$.  
   
For $c,\bar c\in\partial\spc{U}$,  write $\angle\hinge x {c} {\bar c}$ for the angle at $x$ between the representatives of $c$ and $\bar c$ with initial point $x$.  We endow $\partial\spc{U}$ with the  \emph{angular metric} $\dist{*}{*}{\angle}$, where
	\[\dist{c}{\bar c}{\angle}=\sup_{x\in\spc{U}}\angle\hinge x {c} {\bar c}.\]
	
A $\Cat{}{0}$ space $\spc{U}$ is \emph{uniformly visible}	if distant geodesics subtend small angles in the following sense:  for any $\eps>0$  there exists $R(\eps)>0$ such that whenever a geodesic $]pq[$  lies outside a ball $\oBall(x,R(\eps))$, then $\angle\hinge x pq< \eps$.  

If  $\spc{U}$ is uniformly visible, then $\spc{U}$ is $R(\pi/2)$-hyperbolic, since for any nonlinear triangle $[pqr]$ and any $x\in ]pr[$, we have  $\mangle\hinge xpq \ge \pi/2$ by adjacent angle comparison \ref{cat-2-sum}.  Thus  the distance from $x$ to  $]pq[$ is $\le R(\pi/2)$.
 
\parit{Proof of Flat Planes Theorem \ref{thm:flat-planes}.}
Clearly if $\spc{U}$ contains  a flat plane then $\spc{U}$ is not hyperbolic. Thus it suffices to prove that if $\spc{U}$ is not hyperbolic, and  hence not uniformly visible, then $\spc{U}$ contains  a flat plane.

\begin{clm}{}\label{clm:realize-angle}
%Let $G$ be the isometry group of $\spc{U}$.
 If $\spc{U}$ is not uniformly visible, then there exist $z\in\spc{U}$ and $c\ne \bar c\in\partial\spc{U}$ such that 
\[\dist{c}{\bar c}{\angle}=\angle\hinge z {c} {\bar c} \le\pi/2.\]  
%\[
%\angle\hinge p {c^1} {c^2}=\sup_{g\in G} \dist{g(c^1)}{g(c^2)}{\angle}\le\pi/2.
%\]
\end{clm}

There exists $\eps>0$ such that for any positive integer $n$, we may take segments $[p_nq_n]$ that lie outside some ball $\oBall(x_n,n)$ and satisfy  $\angle\hinge {x_n} {p_n} {q_n}\ge\eps$.  By cocompactness, we may assume the $x_n$ lie in a compact set.  For a limit point $x$ of $(x_n)_{n=1}^\infty$, the segments $[p_nq_n]$  lie outside $\oBall(x,n)$, and by angle semicontinuity \ref{lem:ang.semicont}, satisfy $\angle\hinge {x} {p_n} {q_n}\ge\eps$.  Let $r_n$ be the nearest point of $[p_nq_n]$ to $x$.  Then $\angle\hinge {r_n}{p_n} {x}\ge\pi/2$ by the first variation formula \ref{cor:both-end-first-var-cba}; here if $r_n=p_n$, we relabel $p_n$ and $q_n$.  Since the angle sum of triangle $[x p_n r_n]$ is $\le \pi$, then  by relabeling if necessary when $r_n$ is not an endpoint, we have $\eps/2\le \angle\hinge {x}{p_n} {r_n}\le\pi/2$.  Since $\spc{U}$ is proper, we may assume $[xp_n]$ and $[xr_n]$ respectively  converge pointwise to geodesic rays in $c$, $\bar c$ $\in$ $\partial \spc{U}$.

For any $y\in\spc{U}$, we have $\angle\hinge {r_n}{x} {y}\to 0$ by angle comparison \ref{cat-hinge}.  Given $\delta>0$, since $\angle\hinge {r_n}{p_n} {x}\ge\pi/2$ then  the triangle inequality for angles based at $r_n$  implies $\angle\hinge {r_n}{p_n} {y}\ge\pi/2-\delta$ for $n$ sufficiently large.  Then $\angle\hinge {y}{p_n} {r_n}\le\pi/2+\delta$ by angle comparison for triangle $[yp_nr_n]$.   It follows that $\angle\hinge {y}{c} {\bar c}\le\pi/2$.  Since $y$ was chosen arbitrarily,  $\dist{c}{\bar c}{\angle}\le\pi/2$. 

Now choose $z^n\in\spc{U}$ and $c^n,\bar c^n\in\partial\spc{U}$ so that 
\[
\angle\hinge{z^n} {c^n} {\bar c^n}  \to 
\sup \bigl\{\angle\hinge{z^n} {g(c^n)} {g(\bar c^n)} \:g\in G\}
\]
where $G$ is the isometry group of $\spc{U}$.  By cocompactness, we may assume $z^n$ lies in a compact subset of $\spc{U}$.  But then, by properness, the supremum is realized by $\angle\hinge z c {\bar c}$ for some choice of $z, c, {\bar c}$.  The claim follows.

Let $z\in\spc{U}$ and $c\ne \bar c\in\partial\spc{U}$ satisfy claim \ref{clm:realize-angle}. Let $\gamma^1\in c$, $\gamma^2\in\bar c$ satisfy $\gamma^1(0)=\gamma^2(0)=z$, and $\gamma^3 \in \bar c$  satisfy $\gamma^3(0)=\gamma_1(t)$.  By adjacent angle comparison \ref{cat-2-sum}, 
\[
\angle\hinge {\gamma^1(t)} {\gamma^1(0)} {\gamma^3(1)} \ge \pi - \angle\hinge {\gamma^1(t)} {c} {\bar c} \ge \pi -  \angle\hinge z {c} {\bar c}. \eqlbl{arm-flat-plane}
\]
Therefore $\cos\angle\hinge {\gamma^1(t)} {\gamma^1(0)} {\gamma^3(1)} + \cos\angle\hinge {z} {c} {\bar c} \le 0$, and the function $f(t)=\dist{\gamma^2(t)}{\gamma^3(t)}{}$ satisfies  $f^+(0)\ge0$ by the first variation formula \ref{cor:both-end-first-var-cba}. Since $f$ is convex and bounded, $f$ is  constant and the  inequalities in \ref{arm-flat-plane} are equalities. It follows from  Arm Lemma \ref{subthm:arm-eq} applied to  the polygon $[\gamma^2(t) \,x \,\gamma^1(t)\, \gamma^3(t)]$  as $t\to\infty$, that the convex hull of  $\gamma^1$ and $\gamma^2$ is isometric to a sector in the Euclidean plane.
%the nested regions 
%$$\Conv(\gamma^1(t),x,\gamma^2(t),\gamma^3(t))$$ 
%is the  image under a distance-preserving map of the region  bounded by a parallelogram with  adjacent sides of length $t$ lying along the sides of a fixed planar sector.  
%contain, for sufficiently large  $t$, the  image under a distance-preserving map of the region  bounded by a parallelogram with  adjacent sides of length $n$ lying along the sides of a fixed planar sector.  ]

Therefore $\spc{U}$ contains, for any positive integer $i$, the isometric image $\Phi_i(\oBall(0,i))$ of a ball of radius $i$ in the Euclidean plane. Since $\spc{U}$ is cocompact, we may assume  the sequence $\Phi_i(0)$ lies in a compact set. Let $\Phi_{0,i}$ be a  subsequence such that  $\Phi_{0,i}(0)\to x_0\in\spc{U}$.
Consider a dense sequence $y_n$, $n\ge 0$, $y_0=0$, in the plane.  Since $\spc{U}$ is proper, 
there is a subsequence $\Phi_{1,i}$ of $\Phi_{0,i}$ such that  $\Phi_{1,i}(y_1)$ converges to some $x_1\in \spc{U}$. Proceeding recursively, we obtain  
successive subsequences $\Phi_{n ,i}$ such that $\Phi_{i,i}(y_n)\to x_n$ for each $n$.  Then $\dist{y_n}{y_m}{}= \dist{\Phi_{i,i}(y_n)}{\Phi_{i,i}(y_m)}{\spc{U}}=\dist{x_n}{x_m}{\spc{U}}$.  Since the $y_n$ are dense in the plane, an isometric embedding of the plane into $\spc{U}$ is given by continuous extension of the map $y_n\mapsto x_n$.
\qeds

\begin{thm}{Exercise}
\label{exercise:CAT(0)-infinity}
Let $\spc{U}$ be a complete  $\Cat{}{0}$ space.
Given $x,y\in\spc{U}$ and a geodesic ray $\gamma_x$ with  $\gamma_x(0)=x$, show that there is a unique geodesic ray $\gamma_y$ with  $\gamma_y(0)=y$ that is equivalent to $\gamma_x$.  
\end{thm}  