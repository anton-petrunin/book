...
Section~\ref{sec:cba-def} defines $\CAT\kappa$ spaces via a (2+2)-point comparison; 
...
Moreover, a complete metric space $\spc{U}$ is $\CAT\kappa$ space if and only if its ultrapower $\spc{U}^\o$ is $\CAT\kappa$.
...
If $\spc{U}$ is a complete length $\CAT\kappa$ space then $\curv\spc{U}\le\kappa$; this fact follows from Proposition~\ref{prop:inherit-bound}.
...
That is, if $\spc{U}$ is a complete length locally $\CAT\kappa$ space, then for any $p$ there is $r>0$ such that $\oBall[p,r]\in\cCat{}{\kappa}$ (Corollary \ref{cor:k-for-k}).
