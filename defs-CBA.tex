...
\begin{thm}{Definition}
\label{def:R-tree}
An  \emph{$\RR$-tree} is a metric space in which:
\begin{subthm}{}
...
\begin{thm}{Proposition}
\label{prop:R-tree}
A metric space $\spc{U}$ is an $\RR$-tree if and only if 
$\spc{U}\in\Cat{}{\kappa}$ for every $\kappa\in\RR$.
\end{thm}

\parit{Proof.} Both conditions are equivalent to the condition that angles of  triangles take values in $\{0,\pi\}$. 
\qeds 

\begin{thm}{Definition}
\label{def:asymptotic-cone} Let $\spc{X}$ be a metric space and $\o$ be a non-principal ultrafilter.
The \emph{asymptotic cone} $\Cone_\o\spc{X}$ of $\spc{X}$  is defined by
\[
\Cone_\o\spc{X}= \spc{X}_\o \text{\ \ \ \ where \  }
(\frac{1}{n}\cdot\spc{X}\,, x)\to (\spc{X}_\o,x_\o).
\]
\end{thm}
...
Gromov's theory of $\delta$-hyperbolic spaces captures many global features of negative curvature. 
...
\begin{thm}{Exercise}
Show that  a metric space is hyperbolic if and only if its asymptotic cone is an $\RR$-tree.\end{thm}

The following dichotomy appears in  $\Cat{}{0}$ spaces $\spc{U}$ that are cocompact (namely, the union of images of  a compact subset under isometries of $\spc{U}$) and proper:  if the  asymptotic cone  is not an $\RR$-tree, then $\spc{U}$ contains an isometric copy of the Euclidean plane.   
For a detailed discussion of aspects of the proof, and of visibility and the boundary at infinity of $\Cat{}{0}$ spaces, see \cite{BH}.   The theorem was first proved by  Eberlein in the Riemannian case \cite{}, in its equivalent formulation for visibility manifolds.  

We give a short proof following \cite{bridson}. In the setting of general $\Cat{}{\kappa}$ spaces, with  properness and cocompactness not assumed, the dichotomy of the theorem is examined in \cite{kleiner}, see  \ref{thm:dim-infty-CBA}.
   
\begin{thm}{Flat planes theorem}\label{thm:flat-planes}
Suppose a proper $\Cat{}{0}$ space $\spc{U}$ is cocompact.  Then $\spc{U}$ is hyperbolic if and only if $\spc{U}$ does not contain a subspace isometric to the Euclidean plane.\end{thm}

Say two geodesic rays are equivalent (asymptotic) if the Hausdorff distance between their images is finite.  Given $p,q\in\spc{U}$ and a unit-speed geodesic ray $\gamma_p$ with initial point $\gamma_p(0)=p$, there is a unique unit-speed geodesic ray $\gamma_q$ with  initial point $q$ that is equivalent to $\gamma_p$.  ??? We denote this set of equivalence classes by $\partial\spc{U}$.  
...
It follows that $\spc{U}$ is $R(\pi/2)$-hyperbolic, since for any triangle $[pqr]$ and $x\in ]pq[$ we have without loss of generality
 $\mangle\hinge xpr \ge \pi/2$, hence the distance from $x$ to  $]pr[$ is $\le R(\pi/2)$.
...
For any $x\in\spc{U}$, $\eps>0$ and positive integer $n$, we may take segments $[p^nq^n]$ lying outside $\oBall(x,n)$ and such that  $\angle\hinge x {p^n} {q^n}\ge\eps$.  Let $r^n$ be the  nearest point of $[p^nq^n]$ to $x$.  Then $\angle\hinge {r^n}{p^n} {x}\ge\pi/2$ by the first variation formula \ref{cor:both-end-first-var-cba}, and we may assume  $\eps/2\le \angle\hinge {x}{p^n} {r^n}\le\pi/2$.  Since $\spc{U}$ is proper, by passing to subsequences we may assume $[xp^n]$ and $[xr^n]$ respectively converge pointwise to geodesic rays in $c$, $\bar c$ $\in$ $\partial \spc{U}$, by recursive application of the 
selection theorem \ref{} and diagonalization.

For any $y\in\spc{U}$, we have $\angle\hinge {r^n}{x} {y}\to 0$ by angle comparison \ref{cat-hinge}.  Given $\delta>0$, the triangle inequality for angles based at $r_n$ gives $\angle\hinge {r^n}{p^n} {y}\ge\pi/2-\delta$ for $n$ sufficiently large.  Then $\angle\hinge {y}{p^n} {r^n}\le\pi/2+\delta$ by angle comparison for triangle $[yp^nr^n]$.   It follows that $\angle\hinge {y}{c} {\bar c}\le\pi/2$.  Since $y$ was chosen arbitrarily,  $\dist{c}{\bar c}{\angle}\le\pi/2$. 
...
Consider $x\in\spc{U}$ and $c\ne \bar c\in\partial\spc{U}$ as in \ref{clm:realize-angle}. Let $\gamma^1\in c$, $\gamma^2\in\bar c$ be  based at $x$, and $\gamma^3 \in \bar c$  bebased at $\gamma_1(t)$.  By adjacent angle comparison \ref{cat-2-sum}, 
\[
...
there is a subsequence $\Phi_{1,i}$ of $\Phi_{0,i}$ with $\Phi_{1,i}(y_1)\to x_1\in \spc{U}$. Proceeding recursively, we obtain a sequence of  successive subsequences $\Phi_{n ,i}$ such that $\Phi_{i,i}(y_n)\to x_n$ for each $n$.  Since $\dist{x_n}{x_m}{\spc{U}}= \dist{y_n}{y_m}{}$, an isometric embedding of the plane into $\spc{U}$ is given by continuous extension of the map $y_n\mapsto x_n$.
\qeds