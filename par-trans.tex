...
Then for any two points $x,y\in \left] p q \right[$, we have
$\T_x\iso \T_y$.
\end{thm}
...
If $p\in \spc{L}$ is a then for any geodesic $[pq]$ and any point $\bar p\in \left[ p q \right[$ we have $\Sigma_{\bar p}\ge \Sigma_p$.
...
Sinse $p$ is Euclidean, $\mathbb{S}^{m-1}\iso\Sigma_p$.
From \ref{cor:simicont-Sigma}, $\mathbb{S}^{m-1}\ge\Sigma_x$ for any $x\in \spc{L}$.

Let $x\in \left] p q \right[$. 
Take a sequence of points $p_n\in \left] p q \right[$ such that $p_n\to p$ as $n\to\infty$.
According to \ref{thm:parallel}, $\Sigma_x\iso\Sigma_{p_n}$. 
Thus, from \ref{cor:simicont-Sigma}, $\Sigma_x\ge\Sigma_p$.
That is, $\mathbb{S}^{m-1}\ge \Sigma_x\ge\mathbb{S}^{m-1}$ and
 according to \ref{lem:>=-isometry}, $\Sigma_x\iso\mathbb{S}^{m-1}$.
\qeds
...
According to ???, for any $x\in \left] p q \right[$
...
Given two points $x,y\in \left] p q \right[$,
...
\[\left|\ddir{y}{\gexp\mc0_{x}v}^\perp\right|\le \Const\cdot|v|.\]
...
\cdot\left[|v|^2-|\Pi_{x,y}(v)|^2+\dist[2]{\Pi_{x,y}(v)}{w}{}\right]
+o(\eps^2).\]}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{multline*}
...
\le
\\
\le \dist{x}{y}{}+\frac{\eps^2}{2\cdot\dist[{{}}]{x}{y}{}}\cdot\left[|v|^2-|\Pi_{x,y}(v)|^2
+\dist[2]{\Pi_{x,y}(v)}{w}{}\right]+o(\eps^2).
\end{multline*}
...
\[|\Phi_n(v)|\le\left(1+\frac{\Const}{n}\right)\cdot|v|.\]
\end{thm}
...
Thus 
\[
\left|\ddir{y}{\circ\gexp\mc\kappa_{x}v}^\perp\right|
=
\left|\ddir{y}{x'}^\perp\right|
=
\dist[{{}}]{x'}{y}{}\cdot\sin\alpha=O(|v|).\]

\parit{(\ref{SHORT.noncontracting}).} 
Note that from above we also have
\[\left|\ddir{y}{\circ\gexp\mc\kappa_x v}^\top\right|
=
...
\cdot 
\dist[{{}}]{u}{v}{}+o(\eps)\leqno\text{\ref{eq:clm:prop-of-Pi-2}}\mc\kappa\]
}%
\[\dist{\hat u_\eps^\perp}{\hat v_\eps^\perp}{}
\ge
...
\[\left(1+O(\dist{x}{y}{})\right)\cdot\dist[{{}}]{\hat v_\eps}{\hat w_\eps}{}
\ge
...
\dist{p}{q}{}
+
\tfrac{\eps^2}2\cdot\left\{\left(\frac{1}{\dist{p}{x}{}}+
\frac{n}{\dist{x}{y}{}}\right)\cdot|v_0|^2
+\left(\frac{1}{\dist{y}{q}{}}-\frac{n}{\dist{x}{y}{}}\right)\cdot|v_n|^2\right\}
+o(\eps^2).
\end{align*}
Thus,
\[\left(
\frac{1}{\dist{p}{x}{}}
+\frac{n}{\dist{x}{y}{}}\right)\cdot|v_0|^2
+\left(\frac{1}{\dist{y}{q}{}}
-\frac{n}{\dist{x}{y}{}}
\right)\cdot|v_n|^2\ge 0.\]
...
Let us remind that if $x\in \left] p q \right[$ 
...
Set $\tilde p=\tau\mc\kappa\cdot\ddir x p$ and $\tilde q=\tau\mc\kappa\cdot\ddir x q$.
Clearly $\tilde p$ and $\tilde q$ are uniquely defined, 
both $\tilde p$ and $\tilde q$ lie in the fiber over $\0\in \T_x^\perp$ 
and $\dist{\tilde p}{\tilde q}{}=\dist{p}{q}{}$.


\begin{thm}{???Theorem}\label{thm:2nd-var}
Let $\o$ be a ???,
$\spc{L}$ be an $m$-dimensional complete length $\Alex\kappa$ space,
$[pq]$ be a geodesic in $\spc{L}$ and $x\in \left] p q \right[$.
Set $\tilde p=\tau\mc\kappa\ddir{x}p$ and $\tilde q=\tau\mc\kappa\ddir {x} q$, so $\tilde p,\tilde q\in(\T_x)\mc\kappa$.

Then there is two norm-preserving noncontracting maps 
$\map_p\:\T_p\to\T_{\tilde p}$ and $\map_q\:\T_q\to\T_{\tilde q}$, 
...
Let $\o$ be a ???,
$\spc{L}$ be an $m$-dimensional complete length $\Alex\kappa$ space,
$[pq]$ be a geodesic in $\spc{L}$ and $x,y$ be two distinct points $\left] p q \right[$.

???

Set $\tilde p=???\log_{\kappa,x} p$ and $\tilde q=???\log_{\kappa,x} q$.
...
\le
\dist{\exp_{\tilde p}\eps\cdot\map_p v}{\exp_{\tilde q}\eps\cdot\map_q w}{}
+o(\eps^2)\]
for $\o$-almost all $\eps$.
\end{thm}
