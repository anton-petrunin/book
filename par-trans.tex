...
Thus passing to a partial limit $\Phi_n\to\Upsilon_{x,y}$ as $n\to\infty$, we get the required map. It only remains to prove claims \ref{clm:prop-of-Pi} and \ref{clm:prop-of-Phi}.\qeds
...
It is enough to show that for any $u,v\in \T_x^\perp$, we have%
...
i.e. if $K=\Cone\Sigma$ then $(K)\mc\kappa=\Cone\mc\kappa\Sigma$.
We will denote by $\tau\mc\kappa\:K\to(K)\mc\kappa$ the \textbf{tau}tological submap;
i.e. the point with polar coordinates $(x,\xi)\in\RR\times\Sigma$ mapped to the point with the same polar coordinates in $(K)\mc\kappa$.
