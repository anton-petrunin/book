...
\parbf{\ref{ex:equality-alexlemma}.} Without loss of generality, we can assume that the points $x,v,w,y$ appear on the geodesic $[xy]$ in the same order.
...
\parbf{\ref{ex:convexity-CAT0}.} %???+PIC
...
\parit{Comment.} These conditions imply that the natural map can be extended to a distance preseving map to the solid model triangle.
...
\parbf{\ref{ex:bishop}.}
This exercise was suggested by Richard Bishop who past away recently.
We failed to find an argument.
...
\parbf{\ref{ex:not-flat}.} The construction is forgotten now; maybe it was wrong. %???
...
\parbf{\ref{ex:riemannian-kirszbraun-equality}.}
...
\parbf{\ref{ex:isbell}.}
The Kuratowsky embedding is an isometric embedding of $\spc{X}$ into the space of bounded functions $\spc{X}$ equipped with a metric induced by sup-norm.
