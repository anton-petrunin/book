...
%V: Exercise asks for a compact space so I changed plane to a square
...
%???V: Is it oappropriate to call this Shrafutdinov retraction in the case of CAT spaces?
...
It is sufficient to construct a metric on the set of points $\{p$, $x^1$, $x^2$, $x^3$, $x^4\}$ that does not satisfy (4+1)-point comparison but does satisfy all (3+1)-point comparisons.
...
\parit{Remark.} There are examples of 6-point metric spaces that satisfy all (5+1)-point comparisons, but do not admit embedding into a complete length $\Alex{0}$ space \cite{lebedeva-petrunin-zolotov}.
...
\parbf{\ref{ex:flat-in-CBB}.} Consider vectors $v^i=\ddir{x^0}{x^i}\in\T_{x^0}$.
