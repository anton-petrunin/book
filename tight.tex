...
A point $p\in \spc{L}$ belongs to some distance chart if and only if $\rank_p\z=m$.
\end{thm}
...
that
\[\text{if}\quad \dd_p x^i(u)\ge 0\quad \text{for each}\quad i\quad\text{then}\quad \dd_p y(u)\le 0.
\eqlbl{1-st order}\]
...
which will be called  \index{radial bisector}\emph{radial bisector of radius $R$ between $x$ and $y$}.
...
Let us denote by $\sigma_w$ the $f$-radial curve for curvature $-1$ initiated at $w\in\spc{L}\setminus\{x,y\}$.
...
Inequality \ref{eq:|p p'|} implies that we can choose $\eps=\eps(\delta,\RR)>0$ so small that 
