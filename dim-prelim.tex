...
For  topological dimension  this holds for a pair of Pontryagin surfaces constructed in \cite{pontyagin-surface}.
...
and $\T_p\spc{X}$ denotes the tangent space of $\spc{X}$ at $p$ (defined in Section~\ref{sec:tangent-space+directions}).%V: This makes sense only if angles are defined. The definition needs to be modified.
\end{thm}
...
the norms of the corresponding metric spaces $\spc{X}$ and $\spc{Y}$
have an isometric copy of $\EE^2$ but
...
One could modify the definition of linear dimension by taking an arbitrary $n$-dimensional Banach space instead of a Euclidean $n$-space.%V:This again makes no sense if we want angles to exist.
