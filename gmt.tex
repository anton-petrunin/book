...
although these in general are not uniquely determined by $B$ as a set.)
...
In other words, 
\[|L|=\lip L;\] 
i.e., $|L|$ is the best Lipschitz constant of $L$.
...
A finite uinon of $n$-rectifiable sets is $n$-rectifiable.
\end{thm}
...
such that the restriction $\d_xf|V$ is \emph{$\eps$-linear}\index{$\eps$-linear function};
i.e., there is a linear function $\ell\:V\to\RR$ such that 
...
\parit{Proof of \ref{Zk-for-zajicek}.}
Set $h^i=h_{V,\ell^i}$.
