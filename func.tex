...
A curve is called \index{Lipschitz curve}\emph{Lipschitz} or \index{locally Lipschitz curve}\emph{locally Lipschitz} if $\alpha$ is Lipschitz or locally Lipschitz. 
...
on differentiability almost everywhere of Lipschitz maps between smooth manifolds (see \cite{burago-burago-ivanov}, 5.5.2) was proved by Bernd Kirchheim \cite{kirchheim}. %CHECK??? 
%S: Note expanded sentence.
...
will be called its \index{differential of metric}\emph{differential of the induced metric} at $x$, or \emph{metric differential} at $x$.
%*S: note addition
...
%S: Note I changed "concave" to "convex".
...
are called \index{convex/concave function} \index{$\lambda$-convex/concave function} $\lambda$-convex and $\lambda$-concave respectively.

$0$-convex and $0$-concave subfunctions will also be called \emph{convex} and \emph{concave} respectively.

If $f$ is $\lambda$-convex for some $\lambda>0$, $f$ will be called \index{convex/concave function!strongly convex/strongly concave function}\emph{strongly convex};
...
then $f$ is called \index{convex/concave function!semiconvex/semiconcave function}\emph{semiconvex (respectively semiconcave}.{\end{thm}
