...
while convex ones are used  mainly in $\CAT{}$ spaces. %V: That's a strange statement IMO. what's used is concavity (respectively convexity) of distance functions.
...
(Continuous partially defined maps could be defined via closed sets; namely, one may require that inverse images of closed sets are closed.
...
\begin{subthm}{}
We say that $\map$ is \index{locally Lipschitz map}\emph{locally Lipschitz} 
...
and $\Lip\in\RR$.
\begin{subthm}{}
The map $\map$ is called \index{$\Lip$-co-Lipshitz map}\emph{$\Lip$-co-Lipshitz} if 
\[\map(\oBall(x,\Lip\cdot R)_{\spc{X}})\supset\oBall(\map(x),R)_{\spc{Y}}\]
for any $x\in \spc{X}$ and $R>0$.
\end{subthm}

\begin{subthm}{}
The map $\map$ is called \index{co-Lipshitz map}\emph{co-Lipshitz} if it is $\Lip$-co-Lipshitz
...
For that reason they also called $\Lip$-open \cite{burago-gromov-perelman}.
Also, be aware that some authors
refer to our $\Lip$-co-Lipschitz maps
as  $\tfrac1\Lip$-co-Lipschitz. %REF???.
...
Note that any continuous function can be presented as the difference of two continuous functions bounded below by $1$.
Hence the result.
\qeds
...
\begin{subthm}{}
The spaces $X$ and $Y$ are called \index{isometric spaces}\emph{isometric} (briefly $X\iso Y$)
...
let us equip it with the Hausdorff  metric. %V:  Hausdorff distance between subsets of a metric space has not been defined in the book, just Hausdorff convergence. So this should be changed.
...
%V: In the following proposition I changed all closed balls to open ones and all strict inequalities to non strict ones. That's because for general submetry image of a closed ball of radius $r$ need not be equal to the closed ball of radius $r$. It's true for proper spaces but we are not assuming properness here.
...
the curve is called \index{Lipschitz curve}\emph{Lipschitz} or \index{locally Lipschitz curve}\emph{locally Lipschitz} if the map $\alpha\:\II\to \spc{X}$~is.
...
The following generalization of Rademacher's theorem was proved by Bernd Kirchheim \cite{kirchheim}. %CHECK??? 
...
will be called the \index{differential of metric}\emph{differential of the induced metric} at $x$.
...
The following notion will be mostly used for geodesic spaces.
...
$0$-convex and $0$-concave subfunctions will be also called \index{convex function}\emph{convex} and \index{concave function}\emph{concave} respectively.

If $f$ is $\lambda$-convex for some $\lambda>0$ it will be called \index{strongly convex function}\emph{strongly convex};
correspondingly if $f$ is $\lambda$-concave for some $\lambda<0$ it will be called \index{strongly concave function}\emph{strongly concave}.
...
%Further, we give the meaning  a certain type of second order ordinary differential inequalities in the context of metric spaces. %V: I don't understand what this sentence is trying to say so I commented it out.
