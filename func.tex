...
a submap \index{$\map\:\spc{X}\subto \spc{Y}$ (subfunction)}$\map\:\spc{X}\subto \spc{Y}$ is a map defined on a subset $\Dom\map\subset \spc{X}$.

A submap $\map\:\spc{X}\subto \spc{Y}$ is \index{continuous submap}\emph{continuous} if the inverse image of any open set is open.
Since $\Dom\map=\map^{-1}(\spc{Y})$, the domain $\Dom\map$ \index{$\Dom\map$ (domain)} of a continuous submap is open.
...
\begin{subthm}{}
We say that $\map$ is \index{Lipschitz map}\emph{Lipschitz} if it is $\Lip$-Lipschitz for a constant~$\Lip$.
...
Let $\spc{X}$ be a metric space, and let $f\:\spc{X}\to\RR$ be a continuous function.
...
Let $\spc{X}$ and $\spc{Y}$ be metric spaces, and let $\map\:\spc{X}\to \spc{Y}$ be a map.
\begin{subthm}{}
...
Let $\spc{X}$ be a length space, and let $\sigma\:\spc{X}\to \spc{Y}$ be a submetry.
...
A curve is called \index{Lipschitz map!Lipschitz curve}\emph{Lipschitz} or \emph{locally Lipschitz} if $\alpha$ is Lipschitz or locally Lipschitz, respectively. 
...
Let $\spc{X}$ be a metric space, and let $\alpha\:\II \to \spc{X}$ be a locally Lipschitz
curve. 
...
A curve $\alpha\:\II\to\spc{X}$ is \index{unit-speed curve}\emph{unit-speed}
if 
...
Let $\spc{X}$ be a metric space, and let $f\:\RR^n \subto \spc{X}$ be $1$-Lipschitz. 
...
Inequalities  \ref{eq:sec:conv-real*} are understood in the sense of distributions.
...
\begin{thm}{Proposition}
Let $\II\subset\RR$ be an open interval, and let $y_n\:\II\to\RR$ be a sequence of solutions of one of the inequalities in \ref{eq:sec:conv-real*}.
...
For $t_0\in \Dom y$, let us define the \emph{right} (respectively, \emph{left}) \emph{derivative} $y^+(t_0)$ ($y^-(t_0)$) at $t_0$ by
...
Let $\II\subset\RR$ be an open interval, and let $y\:\II\to\RR$ be a solution of an inequality in \ref{eq:sec:conv-real*}.
...
Let $\II$ be an open real interval, and let $y\:\II\to\RR$ be a locally Lipschitz function.
...
Suppose $y''\le \lambda-\kappa\cdot  y$.
Let $t_0\in\II$, and let $\bar y$ be a solution of the ordinary differential equation $\bar y''=\lambda-\kappa\cdot  \bar y$ 
...
(Similarly, by reversing inequalities, for $y''\ge \lambda-\kappa\cdot  y$.) 
\end{subthm}
\end{thm}


Note that Theorem ~\ref{y''=<1-ky} implies that $y$ satisfies $y''\ge \lambda$ ($y''\le \lambda$)  on an interval $\II\subset\RR$  if and only if $y(t)-\frac{\lambda}{2}\cdot t^2$ is convex (respectively, concave) on $\II$.
...
\parit{Proof.} Set $\hat f_n(t)=f_n(t)-\tfrac{\lambda}{2}\cdot t^2$ and $\hat f_\infty(t)=f_\infty(t)-\tfrac\lambda2\cdot t^2$.
...
If $f$ is $\lambda$-convex for $\lambda>0$, then $f$ will be called \index{strongly convex function}\emph{strongly convex};
correspondingly, if $f$ is $\lambda$-concave for $\lambda<0$, then $f$ will be called \index{strongly concave function}\emph{strongly concave}.
