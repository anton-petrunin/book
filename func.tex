...
The same is holds for upper and lower semicontinuous functions $f\:\spc{X}\subto \RR$ since they are  continuous functions for a special topology on $\RR$.
...
Note $\sigma\:\spc{L}\to\spc{M}$ is a submetry if it is 1-Lipschitz and 1-co-Lipschitz.
...
\parit{Proof.}
Denote by $[x]$ the projection of $x\in \spc{L}$ in $\spc{L}/G$.
We need to show that the map $x\mapsto[ x]$ is $1$-Lipschitz and $1$-co-Lipschitz.
...
or, equivalently, $\dist{[x]}{[y]}{\spc{L}/G}< r$.
...
Note that Theorem ~\ref{y''=<1-ky} implies that $y$ satisfies $y''\ge \lambda$ ($y''\le \lambda$)  on an interval $\II\subset\RR$  if and only if $y(t)-\frac{\lambda}{2}t^2$ is convex (concave) on $\II$.
