...
In this paper, 
$\II$ always denotes some finite closed  interval.
...
An \emph{admissible curve} for the triple $(\spc{B},f,\spc{F})$ is a curve $\gamma:\II\to (\spc{B}\times \spc{F})/\!\sim$.  We write $\gamma=(\gamma_\spc{B},\gamma_\spc{F})$ for $\gamma_\spc{B}:\II\to \spc{B}$   
and $\gamma_\spc{F}:\II^+\to \spc{F}$, where  $\II^0=(f\circ\gamma_\spc{B})^{-1}(0)$, $\II^+=\II-\II^0$.  
Set  $\II^+=\sqcup_{i=1,2,\ldots} \,\II^i$, where the $\II^i$ are maximal open subintervals of $\II^+$. 
We further assume $\gamma_\spc{B}$ and   $\gamma_\spc{F}|\II^i$ to be Lipschitz, where   $\gamma_\spc{F}|\II^i$, $i=1,\ldots$, have a uniform Lipschitz constant. 
(Our class of admissible curves does not satisfy the concatenation property of  length structures in the sense of \cite{BBI}.) 
Set
\[\length\gamma\  \ =\ \ \int _{\II}\,\sqrt{
v_B^2+ (f\circ\gamma_\spc{B})^2\cdot v_F^2}\,,
\eqlbl{eq:length}\]
where $\int$ is Lebesgue integral, $v_\spc{B}$ is the speed of $\gamma_B$, $v_\spc{F}|\II_+$ is  the speed of $\gamma_\spc{F}|\II_+$ and $v_\spc{F}|\II_0=0$.   
Then the integrand is defined  almost everywhere on $\II$  and bounded.  

Equivalently,  
\[
\length\gamma\  \ =\ \  \sum_i \int _{\II_i}\sqrt{
v_\spc{B}^2+ (f\circ\gamma_\spc{B})^2\cdot v_\spc{F}^2}  \ \ +\ \  \length\, (\gamma_\spc{B}|\II_0).
\]
   Here the first term
 is defined, 
...
Suppose $\beta^*:\II\to \spc{F}^*$ is a pregeodesic in $\spc{F}^*$ such that  $\beta^*$ and $\gamma_\spc{F}$ have the same  speed, i.e. $v_{\spc{F}^*}=v_\spc{F}$ where $v_{\spc{F}}^*$ is the speed of $\beta^*$.  Then $(\gamma_\spc{B},\beta^*)$ is a geodesic in $\spc{B}\times_f\spc{F}^*$.
\end{subthm}
\end{thm}

\parit{Proof.} 
\ref{SHORT.dist-fiber-indep})
Let $\gamma_i:\II\to \spc{W}$ be admissible curves with endpoints $(p,\varphi)$, $(q,\psi)$,  where $\length\gamma_i\to |(p,\varphi)\, (q,\psi) |$.

Suppose $f\circ(\gamma_i)_{\spc{B}}>0$.  Set $L_i=\length\, (\gamma_i)_\spc{F}$ and $v_i=\speed\,(\gamma_i)_\spc{F}$. Let $\beta^*_i$ be curves in $\spc{F}^*$  with endpoints $\varphi^*$, $\psi^*$ and lengths $L_i^*\to |\varphi^*\,\psi^*|=|\varphi\,\psi |$. Without loss of generality, $L_i^*\le L_i+\epsilon_i$, $\epsilon_i\to 0$.  Define $\gamma_i^*:\II\to \spc{W}^*$ by setting $(\gamma_i^*)_\spc{B}=(\gamma_i)_\spc{B}$, and  letting \,$(\gamma_i^*)_{\spc{F}^*}$\,  be \,$\beta_i^*$\, reparametrized with speed $(L_i^*/L_i)\cdot v_i$. By the length formula  \ref{eq:length},  $|(p,\varphi)\, (q,\psi) |=\lim\length(\gamma_i )\ge \limsup(\length\gamma_i^*)$. Hence  $|(p,\varphi)\, (q,\psi) |\ge |(p,\varphi^*)\, (q,\psi^*)|$.  
...
Let $\beta:\II\to \spc{F}$ be a curve with endpoints $\varphi$, $\psi$ and  length $L'<L$. Then the length of $\gamma$ is reduced  by replacing $\gamma_\spc{F}$ with the reparametrization of $\beta$  with speed $(L'/L)\cdot v$.  This contradiction gives \ref{SHORT.fiber-proj}).
...
Let $\gamma=(\gamma_\spc{B},\gamma_\spc{F}):\II\to \spc{W}=\spc{B}\times_f \RR$ be a geodesic  joining $(p,\phi)$ and $(q,\psi)$.  Suppose $\II_0\ne\emptyset$ where $\II_0=(f\circ\gamma_\spc{B})^{-1}(0)$.  


\begin{subthm}{f=0-fiber-projection} The  restriction of $\gamma_F$ to any maximal subinterval $\II_i$ of $\II-\II_0$   is constant. 
 If $\II_i$ Êhas no common endpoint with $\II$, the constant can be changed to any other
...
$\gamma_\spc{B}$ consists of two geodesics of $\spc{B}$ that intersect on the maximal subinterval $[t_0,t_1]$ of $\II$ having endpoints in $\II_0$. 
\end{subthm}
\end{thm}
...
By the length formula \ref{eq:length}, any curve in $\spc{B}$ joining $p$ and  $q$ and passing through $Z=f^{-1}(0)$ is the projection of a curve in $\spc{B}\times_f\spc{F}$ of the same length joining $(p,\phi)$ and $(q,\psi)$, and such that the projection to $\spc{F}$ on each interval $\II_i$ is constant.  
...
%Let  $a\in (f\circ\gamma_\spc{B})^{-1}(0)$. For a nontrivial closed subinterval $\,\II$ of $\,\II$ with endpoint $a$, we may choose  $v_\spc{F}$ satisfying (ii*) to have constant value $L/|\II|$ on $\,\II$,  and to vanish on the interior of $\,\II-\II$.   Then  %%% notation for length??
%
%\begin{align*}
%\int\limits_\II \sqrt{v_\spc{B}^2(t) + f^2(\gamma_\spc{B}(t))\cdot v_\spc{F}^2(t)} \cdot \d t 
%&
%\le \int\limits_\II v_\spc{B}(t) \cdot \d t + 
%\int\limits_\II f(\gamma_\spc{B}(t))\cdot v_\spc{F}(t)\cdot \d t\\
%&
...
%\end{align*}
%By continuity of $f$, the second term approaches $0$ as $|\II|\to 0$. Therefore claim (\ref{SHORT.f=0-dist}) follows from (\ref{wp-distance}).
%
...
Let $\gamma=(\gamma_\spc{B},\gamma_\spc{F}):\II\to B\times_f\spc{F}$ be a geodesic with speed $v$.  Then $v_\spc{B}$ and an extension $\overline{v}_\spc{F}$
of $v_\spc{F}$ are defined and
Lipschitz
 continuous for all $t\in \II$, and 
...
\parit{Proof.} 
Suppose $\II_0\ne\emptyset$ where $\II_0=(f\circ\gamma_\spc{B})^{-1}(0)$.  
By Proposition \ref{f=0-fiber-projection},  $v_\spc{F}:\II-\II_0\to \spc{F}$ satisfies $v_\spc{F}=0$. In this case, $v_F$ may be extended to all of $\II$ by setting 
...
$v_\spc{B}^2 + f^2v_\spc{F}^2 =v^2$ almost everywhere. Thus it suffices to verify \ref{eq:clairaut-fiber} almost everywhere, namely, to show that for any closed subintervals $\II_1,\II_2$ of $\II$ of the same length,
$$\int\limits_{\II_1 }f^2\cdot v_\spc{F} = \int_{\II_2 } f^2\cdot v_\spc{F}.$$

By Schwarz's inequality, the energy of $\gamma$, 
$$\int\limits_\II({v_\spc{B}}^2+f^2\cdot {v_\spc{F}}^2),$$ is a minimum among all curves
$\~\gamma$ connecting the endpoints of $\gamma$. Without loss of
generality we may assume $\II_1$ and $\II_2$ do not overlap. For $|u|$
...
$$w_u =
\begin{cases}
v_\spc{F} \quad &\text{\ on\ } \II- (\II_1\cup \II_2)\\
v_\spc{F}+u \quad &\text{\ on\ } \II_1\\
v_\spc{F}-u \quad &\text{\ on\ } \II_2.
\end{cases}
$$
...
\begin{align*}
\int\limits_\II(v&^2+f^2\cdot w_u^2) 
= \int\limits_{\II -J_1\cup \II_2} (v^2+f^2\cdot w^2) +\\
&+\int\limits_{\II_1}(v^2+f^2\cdot w_u^2 + 2\cdot u\cdot f^2\cdot w + u^2\cdot f^2) +\int_{\II_2}v^2+f^2\cdot w_u^2 - 2\cdot u\cdot f^2\cdot w
+ u^2\cdot f^2).
\end{align*}
At $u=0$,  the minimum occurs and calculating the
derivative gives
$$\int\limits_{\II_1} 2\cdot f^2\cdot w - \int\limits_{\II_2} 2\cdot f^2\cdot w = 0.$$ 
The claim follows.  

\begin{clm}{}\label{clm:clairaut} $v_\spc{F}$ is defined and continuous on $\II$ and satisfies \ref{eq:clairaut-fiber}.
\end{clm}

Set $c=c(\gamma)$.
Since $c/(f\circ \gamma_\spc{B})^{2}$ is defined and continuous on $\II$, then $v_ \spc{F}$ has a continuous extension $\overline{v}_\spc{F}$ to all of $\II$ by claim \ref{clairaut-ae}. It follows that the arclength function  $s(t)$ of   $\gamma_\spc{F}$ is obtained by integrating the continuous function $\overline{v}_\spc{F}$, and so $ds/dt=\overline{v}_\spc{F}$. Since $\gamma_\spc{F}$ is a pregeodesic, $ds/dt$ is the speed of $\gamma_\spc{F}$, i.e. $v_\spc{F}=ds/dt=\overline{v}_\spc{F}$.  The claim follows.

\begin{clm}{}\label{clm:v_B} $v_\spc{B}$ is defined and continuous on $\II$ and satisfies \ref{eq:clairaut-base}.
\end{clm}


It suffices to assume $0$ is in interior $\II$, and show that $v_\spc{B}$ is defined and continuous at $t=0$. 
...
\eqlbl{eq:length}\]
where the supremum is taken over all partitions  \,$t_0<\ldots<t_n$\, of $\II$.  Here, letting $\overline t_i$ be a minimum point of $(f\circ\gamma_\spc{B})|[t_{i-1}\,t_i]$\,, 
\[
d_i=
\begin{cases}
...
\section{Definitions I}\label{sec:wp-def}
...
\section{Basic properties I}

[AKP: In this version, 
we need Lemma \ref{lem:induced-length1}.  Also we don't have a length formula for general Lipschitz curves in the warped product.]
...
\begin{thm}{Lemma}
\label{lem:induced-length1}
Let $\gamma=(\gamma_ \spc{B},\gamma_ \spc{F})$ be a Lipschitz curve in $ \spc{B}\times_f\spc{F}$ such that  $f\circ\gamma_ \spc{B}> 0$;  so $\gamma_F$ is defined for all $t\in [0,1]$ and $\length_f\gamma$ is defined.  Then ${\length}_f\gamma$  is   equal to $\length\gamma$ as defined by the metric of $\spc{B}\times_f \spc{F}$.
\end{thm}
\parit{Proof.}
By \ref{horiz-leaf-proj} and \ref{vert-leaf-proj}, 
$\gamma_\spc{B}$ and $\gamma_\spc{F}$ are Lipschitz; equivalently,   $(\gamma_\spc{B},\gamma_\spc{F})$ belongs to the class of curves in $\spc{B}\times\spc{F}$ for which $\length_f$ is defined. Let us call these curves \emph{admissible}.  Admissible curves are closed under restriction, concatenation and linear reparametrization.
Moreover, 
%??? local uniform continuity of $f$ implies that 
$\length_f$  is a lower semi-continuous functional on admissible curves with respect to pointwise converge (???), that is,  if \,$\gamma_i\to\gamma$\, then \,$\liminf\,(\length_f\gamma_i) \ge\length_f\gamma$. 
 In this case the two lengths agree, as is shown  in \cite{BBI}. \qeds
...
\begin{subthm}{F-proj}
If $f\circ\gamma_\spc{B}>0$, then $\gamma_\spc{F}$ is a reparametrized geodesic of $\spc{F}$.
\end{subthm}

\begin{subthm}{f=0-F-proj}Suppose $\gamma_\spc{B}$ intersects the zero set $Z$of $f$. Then $\gamma_F$ is constant on any maximal subinterval $\II$ of $[0,1]$ such that  $(f\circ\gamma_B)|\II>0$. 
 If $\II$ contains neither $0$ nor $1$, then this  constant  may be chosen arbitrarily and the resulting curve will still be a geodesic of $\spc{B}\times_f\spc{F}$  joining $x$ and $y$.
 \end{subthm}
...
Let $\beta:[0,1]\to \spc{F}$ be a Lipschitz curve with endpoints $\varphi$, $\psi$ and  $\length\beta <\length\gamma_\spc{F}$. By Lemma \ref{lem:induced-length1} and formula  \ref{wp-length} for $\length_f$, a shorter join than  $\gamma$ of $x$ and $y$ may be obtained by replacing $\gamma_\spc{F}$ with a reparametrization of $\beta$  having speed proportional to and less than $\speed\gamma_{\spc{F}}$.  This contradiction gives (\ref{SHORT.F-proj}).

\ref{SHORT.f=0-F-proj}) Suppose $\gamma_\spc{B}(t_0)\in Z$.
Let $\gamma_1$  be the curve in $\spc{B}\times\spc{F}$ defined on $[0,t_0]$, whose projection to $\spc{B}$ is $\gamma_\spc{B }|[0,t_0]$ and whose projection to $\spc{F}$  takes constant value $\phi$.  By  \ref{wp-length}, $\gamma_1$ has image in $\spc{B}\times_f\spc{F}$ of length equal to   $\length\gamma_\spc{B }|[0,t_0]$. Similarly, there is
...
Then there is a Lipschitz curve in $\spc{B}\times\spc{F}$ defined on $[0,1]$ that joins $(p,\phi)$ and $(q,\psi)$ and has  the same length as $\gamma_\spc{B}$.  Indeed  if $t_0>0$, then by \ref{wp-length} it suffices to take a  reparametrization of $\gamma_1$ whose projection to $\spc{B}$ is  constant on $[t_0-\epsilon, t_0]$, on which interval the projection to $\spc{F}$ moves from constantly $\phi$, to constantly $\psi$, followed by $\gamma_2$.  If $t_0=0$, we make a similar adjustment on $[0,\epsilon]$. Claim
(\ref{SHORT.f=0-distance}) follows.

\ref{SHORT.f=0-geo}) It follows also that  $\gamma_\spc{B}$ minimizes length of curves in $\spc{B}$ from $p$ to $q$ that intersect $Z$. 
...
\section{Definitions II}\label{sec:wp-def}

First we define  $\spc{B}\times_f\spc{F}$ for length spaces $\spc{B}$ and $\spc{F}$.  Then we expand the definition to allow arbitrary metric spaces $\spc{F}$.

Consider the topological space  $(\spc{B}\times \spc{F})/\!\sim$, where the elements of $\{p\}\times \spc{F}$ are identified if $f( p )=0$.  We denote this class by $\overline p$, or by  any of its representatives $(p,\varphi)$, $\phi\in \spc{F}$.

An \emph{admissible curve} for the triple $(\spc{B},f,\spc{F})$ is a curve $\gamma:\II\to (\spc{B}\times \spc{F})/\!\sim$.  We write $\gamma=(\gamma_\spc{B},\gamma_\spc{F})$ where $\gamma_\spc{B}:\II\to \spc{B}$, $\gamma_\spc{F}:\II^+\to \spc{F}$,   $\II^0=(f\circ\gamma_\spc{B})^{-1}(0)$, $\II^+=\II-\II^0$.  
Setting $\II^+=\sqcup_{i=1,2,\ldots} \,\II^i$, where the $\II^i$ are maximal open subintervals of $\II^+$, we assume $\gamma_\spc{B}$ and   $\gamma_\spc{F}|\II^i$ to be Lipschitz, where   $\gamma_\spc{F}|\II^i$, $i=1,\ldots$, have a uniform Lipschitz constant. 
Set
\[\length_f\gamma\  \ =\ \ \int _{\II}\,\sqrt{
v_B^2+ (f\circ\gamma_\spc{B})^2\cdot v_F^2}\,,
\eqlbl{eq:length}\]
where $\int$ is Lebesgue integral, $v_\spc{B}$ is the speed of $\gamma_B$, $v_\spc{F}|\II_+$ is  the speed of $\gamma_\spc{F}|\II_+$ and $v_\spc{F}|\II_0=0$.   
Then the integrand is defined  almost everywhere on $\II$  and bounded.  
...
\]
%The first term
% is independent of enumeration  because the summands are positive.  
 
  
Then the   \emph{warped product} \,$\spc{B}\times_f\spc{F}$\, is the corresponding intrinsic  space, where distance is given by infimum of lengths of admissible curves joining two given points.  Since $\length_f$ on admissible curves gives a lower semi-continuous (???) length structure in the sense of  \cite{BBI}, then $\length_f\gamma$ equals $\length\gamma$ as defined by the warped product metric.
...
[Same as Definitions I after this.]

\section{Basic properties II}

[AKP:  In this version, Lemma \ref{lem:induced-length1} from version 1 is deleted,  and Proposition \ref{f=0-F-proj:2} from version 1 is obvious. The rest is the same as version 1.]
...
\begin{subthm}{F-proj:2}
If $f\circ\gamma_\spc{B}>0$, then $\gamma_\spc{F}$ is a reparametrized geodesic of $\spc{F}$.
\end{subthm}

\begin{subthm}{f=0-F-proj:2}Suppose $\gamma_\spc{B}$ intersects the zero set $Z$of $f$. Then $\gamma_F$ is constant on any maximal subinterval $\II$ of $[0,1]$ such that  $(f\circ\gamma_B)|\II>0$. 
 If $\II$ contains neither $0$ nor $1$, then this  constant  may be chosen arbitrarily and the resulting curve will still be a geodesic of $\spc{B}\times_f\spc{F}$  joining $x$ and $y$.
 \end{subthm}
 
\begin{subthm}{f=0-geo:2}
Suppose $\gamma_\spc{B}$ intersects $Z$.  Then $\gamma_\spc{B}$ minimizes length of curves in $\spc{B}$ joining $p$ and $q$ that intersect $Z$, and either is a geodesic of $\spc{B}$, or intersects $Z$ at a single point $p_0$ and consists of two geodesics of $\spc{B}$ joined at $p_0$.
\end{subthm}
\end{thm}
\parit{Proof.}
\ref{SHORT.F-proj:2}) Suppose $\length\gamma_\spc{F}>|\varphi\,\psi|$.  
Let $\beta:[0,1]\to \spc{F}$ be a Lipschitz curve with endpoints $\varphi$, $\psi$ and  $\length\beta <\length\gamma_\spc{F}$. By Lemma \ref{lem:induced-length1} and formula  \ref{wp-length} for $\length_f$, a shorter join than  $\gamma$ of $x$ and $y$ may be obtained by replacing $\gamma_\spc{F}$ with a reparametrization of $\beta$  having speed proportional to and less than $\speed\gamma_{\spc{F}}$.  This contradiction gives (\ref{SHORT.F-proj}).

\ref{SHORT.f=0-F-proj:2}) By the length formula \ref{eq:length}, any curve in $\spc{B}$ joining $p$ and  $q$ and passing through $Z=f^{-1}(0)$ is the projection of a curve in $\spc{B}\times_f\spc{F}$ of the same length joining $(p,\phi)$ and $(q,\psi)$, and such that the projection to $\spc{F}$ on each subinterval $\II_i$ of $\II$ is constant.  
 Hence (\ref{SHORT.f=0-F-proj:2}).  

\ref{SHORT.f=0-geo:2})
It follows also that  $\gamma_\spc{B}$ minimizes length of curves in $\spc{B}$ from $p$ to $q$ that intersect $Z$.  If $t_0=t_1$ in  (\ref{SHORT.f=0-base-projection}), it follows that $\gamma |[0,t_0]$ and $\gamma |[t_0,1]$ are geodesics and the claim holds. Suppose $t_0<t_1$.  Then $\gamma |[0,t_1]$ is  a geodesic, since otherwise  by the triangle inequality there is a curve from $p$ to $q$ passing through $\gamma(t_1)\in Z$ that is shorter than $\gamma$.  Similarly,  
$\gamma |[t_0,1]$ is a geodesic.
\qeds
...
one can use the cosine rule in $\Lob2\kappa$:
\[
\dist{(p,x)}{(q,y)}{} 
=
...
The $1$-cone $\Cone\mc1\spc{F}$ is also called the \emph{spherical suspension} over $\spc{F}$,  denoted by $\Susp\spc{F}$.
That is,
\[
...
if and only if $A$ is $\varpi\kappa$-convex in $\spc{F}$.
\end{thm}
