...
Warped product is a construction which produces 
...
Many important construction, such as direct product, cone, spherical suspension and joins
will be defined using warped products.

\section{Definitions}\label{sec:wp-def}
Consider length spaces $\spc{B}$ and  $\spc{F}$,  
and a locally uniformly continuous  function
$f:\spc{B}\to\RR_{\ge 0}$. 
...
defined as the  infimum of $f$-lengths of such curves $\gamma$ that connect the points.
That is
\[\dist{x}{y}{f}
\df
\inf
...
it will be denoted by $\spc{B}\times_f \spc{F}$.  An equivalence class $p\times \spc{F}$ will be denoted by any of its representatives.

Given a path $\gamma$ in $ \spc{B}\times_f\spc{F}$ defined on $[0,1]$, we write $\gamma=(\gamma_\spc{B},\gamma_\spc{F})$ where $\gamma_B$ is defined on $[0,1]$ and  $\gamma_\spc{F}$ is defined for all $t\in [0,1]$ such that $f(\gamma_B(t))> 0$. There may be no continuous extension of $\gamma_\spc{F}$ to $[0,1]$.

If a path $\gamma$ in $ \spc{B}\times_f\spc{F}$ is Lipschitz and $f\circ\gamma_B> 0$, then $\length\gamma$ as defined by the metric of $\spc{B}\times_f \spc{F}$  is equal to   ${\length}_f\gamma$.  Indeed, the definition of distance in $\spc{B}\times_f \spc{F}$ implies that projection to $\spc{B}$ is distance nonincreasing, and projection to $\spc{F}$ is locally Lipschitz where defined.  Therefore 
$\gamma_\spc{B}$ and $\gamma_\spc{F}$ are Lipschitz; equivalently,   $(\gamma_\spc{B},\gamma_\spc{F})$ belongs to the class of curves in $\spc{B}\times\spc{F}$ for which $\length_f$ is defined.  Call these curves  \emph{admissible} for $\length_f$. Admissible curves are closed under restriction, concatenation and linear reparametrization.
Moreover, local uniform continuity of $f$ implies that $\length_f$  is a lower semi-continuous functional on admissible curves with respect to pointwise convergence, that is,  if \,$\gamma_i\to\gamma$\, then \,$\liminf\,(\length\gamma_i) \ge\length\gamma$. 
It follows as in \cite{BBI} that the two lengths agree. 
...
According to Fiber-independence theorem (\ref{thm:fiber-independence}), the resulting space does not depend on the choice of $\spc{F}'$.
...
Suppose $\gamma_\spc{B}$ intersects the zero set $Z$of $f$. Then $\gamma_F$ is constant on any maximal subinterval $\JJ$ of $[0,1]$ such that  $(f\circ\gamma_B)|\JJ>0$. 
 If $\JJ\cap\{0,1\}=\emptyset$, then this  constant  may be chosen arbitrarily and the resulting curve will still be a geodesic of $\spc{B}\times_f\spc{F}$  joining $x$ and $y$.
 \end{subthm}
...
Let $\beta:[0,1]\to \spc{F}$ be a Lipschitz curve with endpoints $\varphi$, $\psi$ and  $\length\beta <\length\gamma_\spc{F}$. By the length formula  \ref{wp-length}, a shorter join than  $\gamma$ of $x$ and $y$ may be obtained by replacing $\gamma_\spc{F}$ with a reparametrization of $\beta$  with speed proportional to and less than $\speed\gamma_{\spc{F}}$.  This contradiction gives (\ref{SHORT.F-proj}).

\ref{SHORT.f=0-geo}) Suppose $\gamma_\spc{B}(t_0)\in Z$.
By \ref{wp-length}, the curve $\gamma_1$ in $\spc{B}\times\spc{F}$ defined on $[0,t_0]$, whose projection to $\spc{B}$ is $\gamma_\spc{B }|[0,t_0]$ and projection to $\spc{F}$  takes constant value $\phi$, has image in $\spc{B}\times_f\spc{F}$ of length equal to   $\length\gamma_\spc{B }|[0,t_0]$. Similarly, there is
$\gamma_2$  defined on $[t_0,1]$, whose projection to $\spc{F}$ takes constant value  $\psi$ and  whose image in $\spc{B}\times_f\spc{F}$ has length equal to  $\length\gamma_\spc{B}|[t_0,1]$.
 ...
 If $t_0>0$, then there is a Lipschitz curve in $\spc{B}\times\spc{F}$ defined on $[0,1]$, 
joining $(p,\phi)$ and $(q,\psi)$ and of the same length as $\gamma_\spc{B}$.  Indeed by \ref{wp-length} it suffices to reparametrize $\gamma_1$ so that its projection to $\spc{B}$ is constant on $[t_0-\epsilon, t_0]$, on which interval the projection to $\spc{F}$ moves from $\phi$ to $\psi$, followed by $\gamma_2$.  (Similarly on $[0,\epsilon]$ if $t_0=0$.) Claim
