...
A unit-speed geodesic $\gamma\:\RR_{\ge0}\to \spc{X}$ is called a \index{ray}\emph{ray}. %???maybe better half-line???
...
when a confusion is possible we call the latter a \index{constant-speed geodesic}\emph{constant-speed geodesic}.
...
\parbf{Triangles.}
For a triple of points $p,q,r\in \spc{X}$, a choice of triple of geodesics $([q r], [r p], [p q])$ will be called a \index{triangle}\emph{triangle}, and we will use the short notation 
...
The value $\dist{p}{q}{}+\dist{q}{r}{}+\dist{r}{p}{}$ will be called the \index{perimeter}\emph{perimeter of triple} $p$, $q$, $r$.

If for  $p,q,r\in \spc{X}$,
$\trig{\tilde p}{\tilde q}{\tilde r}=\modtrig\kappa(p q r)$ is defined 
and $\dist{p}{q}{},\dist{p}{r}{}>0$, the angle measure of 
$\trig{\tilde p}{\tilde q}{\tilde r}$ at $\tilde  p$ will be called the \index{model angle}\emph{model angle} of triple $p$, $q$, $r$, and will be denoted by
$\angk\kappa p q r$%
...
\end{align*}
Note that the area of model triangle with perimeter $\ell$ can not exceed the area of hemisphere with equator length $\ell$;
in particular,
\begin{align*}
\area\modtrig\kappa(pxy)&\le \ell^2, 
&
\area\modtrig\Kappa(pxy)&\le \ell^2.
\end{align*}
...
and it is more natural to consider the \index{angle!upper angle}\emph{upper angle}  defined as
