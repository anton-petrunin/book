...
will be called \emph{perimeter of triangle} $\trig p q r$;
...
in particular the angle can be calculated from the  cosine law:
\[\cos\angk{0}{p}{x}{y}
=
...
\parbf{Product space.}
Given two metric spaces $\spc{X}$ and $\spc{Y}$, the \index{product space}\emph{product space} 
$\spc{X}\times\spc{Y}$ is defined as the set of all pairs $(p,q)$ where $p\in\spc{X}$ and $q\in \spc{Y}$, 
with the metric defined by 
\[\dist{(p^1,q^1)}{(p^2,q^2)}{\spc{X}\times\spc{Y}}=\sqrt{\dist[2]{p^1}{p^2}{\spc{X}}+\dist[2]{q^1}{q^2}{\spc{Y}}}.\]
...
The point in the cone ${\spc{Y}}$ formed by the equivalence class of $0\times\spc{X}$ is called the \index{tip of the cone}\emph{tip of the cone} and is denoted by $0$ or $0_{\spc{Y}}$.
