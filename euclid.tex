...
A unit-speed geodesic $\gamma\:\RR_{\ge0}\to \spc{X}$ is called a \index{half-line}\emph{half-line}. %???maybe better half-line???
...
To define $s(t,\tau)$, consider three half-lines $\tilde \gamma^1$, $\tilde \gamma^2$, $\tilde \gamma^3$ on a Euclidean plane starting at one point, such that $\mangle(\tilde \gamma^1,\tilde \gamma^2)\z=\alpha^{12}+\eps$, $\mangle(\tilde \gamma^2,\tilde \gamma^3)\z=\alpha^{23}+\eps$ and $\mangle(\tilde \gamma^1,\tilde \gamma^3)\z=\alpha^{12}+\alpha^{23}+2\cdot \eps$.
...
Recall that the speed of curve is defined in \ref{thm:speed}.
