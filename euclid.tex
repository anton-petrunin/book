...
A piecewise geodesic curve is called \index{polygonal line}\emph{polygonal line};
...
More precisely: suppose that geodesic $[xy]$ is the union of two sides $[px]$ and $[py]$ of hinges $\hinge pxz$ and $\hinge pyz$, then 
...
A vector in $\T'_p$ that corresponds to the geodesic path $\geod_{[pq]}$ is called \index{logarithm}\emph{logarithm of $[pq]$} and denoted by $\ddir pq$.\index{$\ddir {p}{q}$}
...
briefly $\alpha^+(0)\z=v$\index{$\alpha^+$}, if for some (and therefore any) sequence of vectors $v_n\in\T'_p$ such that $v_n\to v$ as $n\to\infty$,
