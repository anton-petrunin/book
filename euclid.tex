...
A globally isometric map $\gamma\:\II\to \spc{X}$ is called a \index{unit-speed geodesic}\emph{unit-speed geodesic}%
...
A unit-speed geodesic $\gamma\:\RR_{\ge0}\to \spc{X}$ is called a \index{ray}\emph{half-line}. %???maybe better half-line???
...
The image of $\geod_{[p q]}$ will be denoted by $[p q]$\index{$[{*}{*}]$} and called a \index{geodesic}\emph{geodesic}.
The term \index{geodesic}\emph{geodesic} will also be used for  a linear reparametrization of a unit-speed geodesic;
when a confusion is possible we will call the latter a \index{pregeodesic}\emph{pregeodesic}.
...
If $\II=[0,1]$, then $\gamma$ is called a \index{local geodesic path}\emph{local geodesic path}.
...
will be called the \index{perimeter of triangle}\emph{perimeter of triangle} $\trig p q r$;
...
The value $\dist{p}{q}{}+\dist{q}{r}{}+\dist{r}{p}{}$ will be called the \index{perimeter}\emph{perimeter of the triple} $p$, $q$, $r$.
...
Given a hinge $\hinge p x y$, we define its \index{angle}\emph{angle} to be \index{$\mangle$!$\mangle\hinge{{*}}{{*}}{{*}}$}
\[\mangle\hinge p x y
\df
...
where the right hand side is defined in section~\ref{model}.  %on page~\pageref{page:model-side}.%V: I don't like using hard page references. They might change in formatting.
...
\end{align*}
Note that for $ \kappa\ge 0$ a triangle of perimeter $l$ in  $\Lob2\kappa$ lies is a ball of radius $2l$ which easily implies that  $\area\modtrig\kappa(pxy)\le 100\ell^2$.
...
The elements of $\Sigma'_p$ are called \index{geodesic directions}\emph{geodesic directions} at $p$.
...
The completion of $\Sigma'_p$ is called the \index{space of directions}\emph{space of directions} at $p$ and is denoted by $\Sigma_p$ or $\Sigma_p\spc{X}$.
...
we say respectively that $\alpha$ is \index{differentiable!right differentiable}\emph{right} or \index{differentiable!left differentiable}\emph{left} or \index{differentiable!both-sided differentiable}\emph{both-sided differentiable} at $t_0$.
...
\[|\alpha^+(t)|=|\alpha^-(t)|=\tfrac12\cdot\dist{\alpha^+(t)}{\alpha^-(t)}{\T_{\alpha(t)}},\] then we say that $\alpha$ is \index{differentiable curve}\emph{differentiable} at $t$.
\end{thm}
...
Suppose that $\speed_t\alpha$, $\alpha^+(t)$ and $\alpha^-(t)$ are defined. %V: I don't think $\speed_t$ has been defined.
...
Take a geodesic $\gamma$ starting at $p$ that lies in $\Dom f$.
...
The last part follows from the definitions of differential and ultradifferential. %V: was ultradifferential defined?
\qeds
...
and given a hinge $\hinge p x y$  it is more natural to consider the \index{angle!upper angle}\emph{upper angle}  defined as
