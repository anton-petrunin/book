...
Let $\spc{X}$ be a metric space and $\II\subset \R$\index{$\II$ (interval)} be an interval. 
...
A vector in $\T'_p$ that corresponds to the geodesic path $\geod_{[pq]}$ is called \index{logarithm}\emph{logarithm of $[pq]$} and denoted by $\ddir pq$.\index{$\ddir {p}{q}$ (logarithm)}
