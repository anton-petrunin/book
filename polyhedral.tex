...
Each such tree completely described by its combinatorics and the lengths of its $2\cdot n-3$ edges;
the trees in $\spc{T}_n$ with fixed combinatorics can be identified with a convex subset in $\RR^{2\cdot n-3}$; where lengths of edges are the ccordinates. 
Consider the legth metric on $\spc{T}_n$ such that each subset with fixed combinatorics isometric to the corresponding subset in $\RR^{2\cdot n-3}$.
