...
and let $\sigma$ be a simplex in its triangulation~$\tau$.
...
$\Lob{}\kappa$ space, and let $\tau$ be a triangulation of $\spc{P}$. Then 
...
\parit{Proof.}
The only-if part is evident by 
the generalized Hadamard--Cartan theorem (\ref{thm:hadamard-cartan-gen}), 
Exercise~\ref{ex:geod-circle}, and
Theorem~\ref{thm:warp-curv-bound:cbb}.
...
Let $\spc{P}$ be a piecewise
$\Lob{}\kappa$ space, and let $\tau$ be a triangulation of $\spc{P}$.
Then $\spc{P}$ is $\Alex\kappa$ if and only if the following conditions hold.

\begin{subthm}{thm:poly-CBB:pure} $\tau$ is \index{pure triangulation}\emph{pure}; 
...
Finally, assuming that $m$ is dimension of $\tau$,
condition \ref{SHORT.thm:poly-CBB:pure} follows from the remaining conditions.
...
\parit{Step.}
Assume that the theorem is proved for polyhedral spaces  of dimension less than $m$.
Suppose  $\dim\spc{P}=m$.
...
If $m=2$, then \ref{clm:curv+diam} follows from \ref{SHORT.thm:poly-CBB:2pi}.
...
Assume that a spherical polyhedral space $\spc{P}$
admits a triangulation $\tau$ such that all edge lengths of all simplexes in $\tau$ are at least $\tfrac\pi2$.
...
but in fact, it is hard to check even in simple cases.
