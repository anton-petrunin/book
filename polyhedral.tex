...
\emph{$\Lob{}\kappa$-polyhedral space} (briefly $\spc{P}\in \PM{}\kappa$, which stands for \emph{piecewise} $\Lob{}\kappa$)
...
Assume  $\spc{P}\in\PM{}\kappa$ and $\dim \spc{P}\ge 2$. 
Show that 
if $\spc{P}$ is a complete length $\CAT{\kappa'}$ space, then $\kappa'\ge \kappa$ and $\spc{P}$ is $\CAT\kappa$.
\end{thm}
...
that are parallel to the coordinate $(k+1)$-plane 
...
then $\spc{Q}$ is locally $\CAT0$ 
and therefore its universal cover $\tilde{\spc{Q}}$ is $\CAT0$.
\end{thm}
...
and any loop in $\spc X\backslash K'$
...
Note that $\gamma$ forms a 1-dimensional subcompelex of $\spc{S}$ that is a closed local geodesic.
Denote by $G$ the subcomplex of $\spc{Q}$ that corresponds to $\gamma$.
...
The latter can only fail if $\gamma$ contains two vertices, say $\xi$ to $\zeta$ in $\spc{S}$, that are joined by an edge not in $\gamma$; 
...
since $\tilde G$ is 
a 2-dimensional manifold without boundary.
From above, $\tilde G$ is $\CAT0$ and hence is contractible.
...
Let $\Sigma^{m-1}$ be an $(m-1)$-dmensional smooth homology sphere that is not simply connected and bounds a contractible smooth compact $m$-dimensional manifold $\spc{W}$. 
...
\parbf{Generalized flag condition.}
It would be interesting to get an analog of the flag condition for other simplices (not necessary right-angled).
...
\parbf{More exotic constructions.}
...
A similar construction was used by Michael Davis, Tadeusz Janu\-szkie\-wicz and Jean-Fran\c{c}ois Lafont \cite{davis-januszkiewicz-lafont}
to construct a closed smooth 4-dimensional manifold $M$ with universal cover $\tilde M$ diffeomorphic to $\RR^4$ such that 
(1) $M$ admits a polyhedral metric that is locally $\CAT0$, 
but (2) $M$ does not admit a Riemannian metric with nonpositive sectional curvature.
