...
Let $\trig{\tilde x^1}{\tilde x^2}{\tilde x^3}$ be a trianglc in $\Lob2{\kappa}$.  Let $\tilde p^1,\tilde p^2,\tilde p^3$ be points  in $\Lob2{\kappa}$ such that, for any permutation $\{i,j,k\}$ of $\{1,2,3\}$, we have
\begin{enumerate}[(i)]
...
Choose $p\in \,{]}x^1x^2{[}\,$.  Applying the  short map extension property with $V_3=\{x^1,x^2,x^3\}$, $V_4=\{x^1,x^2,x^3,p\}$ and the map $f(x^i)=\tilde x^i$, we obtain point-on-side comparison (\ref{point-on-side}).
...
contradicting $\Alex\kappa$ comparison (\ref{df:cbb1+3}).
\qeds
...
The inequality in the above exercise was discovered by  Karl-Theodor Sturm \cite{sturm}.
...
Assume that a point array $(a^0,a^1,\dots,a^\kay)$ in $\spc{L}$
 is \index{strutting}\emph{$\kappa$-strutting} (Definition {\ref{def:strut-I})
...
%the result above 
Helly's theorem
 is equivalent to the following statement: if a convex bounded set is closed in the ordinary topology then it is compact in the weak topology.
...
%It is sufficient to show that $(y_n)$ is Cauchy; 
...
Let $z_n$ be midpoint of $[y'_ny''_n]$. 
...
So we may assume that $(\alpha_1,\alpha_2,\dots,\alpha_n)\in\Delta^{n-1}$
...
Prove that there is a subset $Q\subset\spc{L}$ isometric to a convex set in $\EE^m$ containing all the points $x^i$.
\end{thm}
...
It worthwhile to know these results even though they are far from Alexandrov geometry.

The following exercise gives an analog of the finite$\bm{+}$one lemma (\ref{lem:kirsz-neg:new}),  discovered by the second author and Stephan Stadler \cite{perunin-stadler}.
...
We say that a compact space $\spc{X}$ is a \index{Kirszbrun source}\emph{Kirszbraun source} if for an arbitrary complete metric space $\spc Z$ and subset $Q\subset\spc X$, any short map $Q\to \spc{Z}$ can be extended to a short map $\spc{X}\to\spc{Z}$.
