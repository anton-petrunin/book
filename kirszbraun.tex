...
The (2$n$+2)-point comparison (\ref{CBA-n-point}) gives a necessary condition for \ref{open:n-point-CBA} and it is not known whether it is sufficient.

\begin{thm}{Open problem}
Let $\spc{F}$ be a finite metric space which satisfies (2$n$+2)-point comparison (\ref{CBA-n-point}) 
for some fixed for $\kappa$
and any choice of the point array.

Is it true that $\spc{F}$ admits a distance preserving map into a $\cCat{}{\kappa}$ space?
\end{thm} 
...
and all the remaining distances are are equal $1$.
