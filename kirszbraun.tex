...
\eqlbl{eq:old-iii}\]
Condition \ref{eq:main-overlap} holds if and only if 
...
given a half-space $H$ in $\Lob{}{\kappa}$, consider the map $\Lob{}{\kappa}\to H$ 
...
and all the remaining distances are equal to $1$.
For which values of $s$ does the space $\spc{F}_s$ admit a distance-preserving map into:
\begin{subthm}{}
...
Let $\{K_\alpha\}_{\alpha\in \IndexSet}$ be an arbitrary collection of closed bounded convex subsets in a complete length $\CAT0$ space $\spc{U}$.
...
let $Q\subset \spc{L}$ be arbitrary subset
and let $f\: Q\to\spc{U}$ be a short map.
...
\begin{thm}{Kirszbraun's theorem for nonpositive curvature bound}
\label{thm:kirsz}
...
Let $\spc{X}$ and $\spc{Y}$ be metric spaces, let $A\subset \spc{X}$, and let $f\:A\to \spc{Y}$ be a short map.
