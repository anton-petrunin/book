...
\parbf{Example.}  A sinusoidally $K$-convex function $f:[a,b]\to [0,\infty)$ must be continuous on $(a,b)$.  However, a sinusoidally $\kappa$-convex Jacobi length $f:[a,b]\to [0,\infty)$  in $\spc{U}\in\Cat{}{\kappa}$ may be discontinuous at the endpoints. Consider the complement $\spc{U}$  in  $ \EE^2$ of an open disk, and  a 
...
\]
%If  there is no sequence $t_i\to t^+$ such that $\ell_i\ge\ell$, then $g_i(1)$ is undefined, and we define $f_N(1)$ by \,\ref{eq:jacobi-split}, i.e. $f_N(1)^2=f(1)^2-f_T(1)^2$.
\end{thm}
...
%\begin{thm}{Lemma}\label{lem:model-jacobi}
%For any $p\in\Lob{}\kappa$ and any triangle $\trig {\~x}{\~y}{\~z}$ in $\oBall(p,\tfrac{\varpi\kappa}{2})$, the line-of-sight map from $\~x$ to $\geodpath_{[\~y\~z]}$ satisfies Jacobi splitting.  Moreover,   for each $t\in [0,1)$ the corresponding  normal Jacobi length  is a $(\kappa\cdot\ell^2)$-sinusoid.
