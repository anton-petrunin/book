...
It is used in \cite{???} to prove a sharp Alexandrov curvature bound above for Riemannian manifolds-with-boundary (\ref{thm:example-mnflds-with-bry:CBA}), and in \cite{} to obtain sharp curvature bounds for subspaces of spaces with $\curv\le\kappa$ (\ref{sec:gauss-equation}).
...
%\parit{($\Rightarrow$)} 
Suppose $\curv \spc{U}$ is locally $\CAT\kappa$. 
