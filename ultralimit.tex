...
%(In principle, we could avoid 
%selling our souls 
%to the set-theoretical devil, but in this case 
%we must say ``pass to convergent subsequence'' too many times.)
%V:  I am not sure I like that kind of metaphor in a math book.
%A: then remove it, I think it is okey to say that it is not essential in a way. 
...
The resulting space is called the \index{Stone--\v{C}ech compactification}\emph{Stone--\v{C}ech compactification} of $\NN$; it is usually denoted by $\beta\NN$.
...
Define an  \index{ultralimit}\emph{$\o$-limit} of $(x_n)$ to be a point $x_\o$ 
...
\parit{Proof.}
Let $(\spc{X}_n)$ be a sequence of metric spaces such that $\spc{X}_n\to\spc{X}_\o$ as $n\to\o$.
...
Let us choose a double sequence $(x_{n,m})$, $x_{n,m}\in \spc{X}_n$, such that for any fixed $m$ we have $x_{n,m}\to x_m$ as $n\to\o$.
...
Note that $z_\o$ is a midpoint for $x_\o$ and $y_\o$ in $\spc{X}^\o$.
...
as 
$n\to\o$.
In particular, for any $\eps>0$, the point $z_n$ is an $\eps$-midpoint for $x$ and $y$ for $\o$-almost all $n$.
...
Recall that a family of submaps between metric spaces $\{f_\alpha\co \spc{X}\subto\spc{Y}\}_{\alpha\in\mathcal A}$ is called \index{equicontinuous family}\emph{equicontinuous} if for any $\eps>0$ there is $\delta>0$ such that for any $p,q\in\spc{X}$ with $\dist{p}{q}{}<\delta$ and any $\alpha\in\mathcal A$ we have $\dist{f(p)}{f(q)}{}<\eps$.
...
In particular, if the $f_n$ are uniformly bounded and equicontinuous, then its $\o$-limit $f_\o$ is defined, bounded and uniformly continuous.
\end{thm}
...
the functions $f_n$ converge to the \it{ $\o$-differential} of $f$ at $p$.
