...
\parbf{Ultrapower.} If all the metric spaces in the sequence are identical $\spc{X}_n\z=\spc{X}$, 
its $\o$-limit 
$\lim_{n\to\o}\spc{X}_n$
...
Note that the proof above together with Lemma~\ref{lem:X-X^w} implies the following:
...
the sequence $\ell_n\z=n\cdot\dist{x}{x_n}{\spc{X}}$ is bounded.
