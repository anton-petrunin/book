...
Also, if $\spc{X}=\RR$ we say that \[\lim_{n\to\o} x_n=+\infty (-\infty)\] if 

\[
\o\set{n\in\NN}{\dist{x_\o}{x_n}{}>\eps (<-\eps)}=1\quad\text{ for any } \eps>0
\]
%V: I added the case of infinite limits this because it's used later on.
...
and takes a value in $[0,\infty]$. %V: I expanded the definition of ultralimits to include the case of limits equal to infinity.
...
The space $\spc{X}_\o$ is called the \index{ultralimit space}\emph{$\o$-limit} of $\spc{X}_n$.
...
Fix a Cauchy sequence $x_{m}\in \spc{X}_\o$. %earlier Cauchy's sequences are referred to as sequences that converge in themselves. I think we should stick to fixed terminology. I prefer calling them  Cauchy sequences. I don't think sequences that converge in themselves is standard.
...
For example $\RR^\o$ has pairs of points at distance $\infty$ from each other;
...
%V: I am not sure why this is a corollary. It's easy to prove but I don't see why it's a consequence of the previous statement.
\end{thm}

\section{Ultratangent space} 

Recall that we assume that $\o$ is a once for all fixed choice of a selective ultrafilter on $\NN$.
...
The metric component of $x^\o$ in $\o\cdot\spc{X}$ is called the ultratangent space of $\spc{X}$ at $x$ and it is denoted as $\T^\o_x\spc{X}$.
...
Thus $\dist{v_\o}{w_\o}{}=\dist{v}{w}{}$; that is, $\iota$ is a global isometry on $\T_p'$.

Since $\T_p'$ is dense in $\T_p$,
we can extend $\iota$ to a global isometry $\T_p\to \T^\o_p$.%V: we sometimes call distance preserving maps isometries or global isometries and sometimes just distance preserving maps. I think consistent terminology should be used.
\qeds
...
\qeds
\end{comment}
%V: I commented out this corollary because it's already proved in Observation 4.3.2
...
In particular, if the $f_n$ are uniformly bounded and equicontinuous, then the $\o$-limit is defined.%V: I think you need equicontinuity not just continuity to get a well defined $\o$-limit.
\end{thm}
...
the functions $f_n$ converge to $\o$-differential of $f$ at $p$.
