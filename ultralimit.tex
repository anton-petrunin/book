...
to the set-theoretical devil but then we had to say ``pass to convergent subsequence'' too many times.)
...
Fix a Cauchy sequence $x_m\in \spc{X}_\o$.
Passing to a subsequence we can assume that $\dist{x_m}{x_{m-1}}{\spc{X}_\o}<\tfrac1{2^m}$.

Let us choose double sequence $x_{n,m}\in \spc{X}_n$ such that $x_{n,m}\to x_m$ for each $m$ as $n\to\o$.

Set 
\[S_{m}=\set{n\in\NN}{\dist{x_{n,m}}{x_{n,m-1}}{}<\tfrac1{2^m}}.\]
Note that for each $m$ the set $S_{m}$ contains $\o$-almost all elements of $\NN$.
We assume that $S_1=\NN$.
...
If $\spc{X}$ is a length space then for any pair of points $x$ and $y$ there is a sequence of $\tfrac1n$-midpoints $z_n$;
its $\o$-limit $z_\o$ is a midpoint of $x$ and $y$ in $\spc{X}^\o$.
