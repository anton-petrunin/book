...
In this section we address when the metric of the tangent space is a length metric.
...
In particular, $\mangle$ is the length-metric of $\Sigma_p{\spc{L}}$ in $\mangle_{\HH\times\RR}$.
...
According to \ref{thm:buyalo}, any convex hypersurface in a Euclidean space, equipped with the length-metric, is non-negatively curved.
...
thus concavity of $\dd_p f$ is meaningless. 
...
In finite dimensional case $\dd_p f$ is concave (see ???).
...
\end{itemize}
Note that 
\[\dist{\tilde p}{\tilde w}{}=\tfrac{1}{2}\cdot\sqrt{|u|^2+2\cdot\<u,v\>+|v|^2}.\]
...
we have $\dist{u_t}{v_t}{}=t\dist{\tilde u}{\tilde v}{}+o(t)$ 
and $\dist{u_t}{v_t'}{}=t\dist{\tilde u}{\tilde v'}{}+o(t)$ 
...
%By Theorem~\ref{thm:tan-is}, it is also clear that if $\T^\o_p$ is an Alexandrov space with upper or lower curvature bound, then differential $\dd^\o_p f$ of semiconcave locally Lipschitz function $f$ is concave.
