...
By comparison,
\begin{align*}
\angk\kappa x{\alpha(s)}{\gamma_n(s-t)}&\le2\cdot\pi-\angk\kappa x{p_n}{\gamma_n(s-t)} - \angk\kappa x{\alpha(s)}{p_n}\le
\\
...
\[\dist{\gamma_n(s-t)}{\alpha(s)}{}<10\cdot\delta_n\cdot (s-t)\]
if $s-t$ is a sufficiently small and positive.
That is, $\alpha^+(t)$ is defined; see \ref{def:right-derivative}.
\qeds
...
\[\liminf_{t\to t_0+} \frac{\distfun{q_n}\circ\alpha(t)-\distfun{q_n}\circ\alpha(t_0)}{t-t_0}\to -1\]
as $n\to\infty$.
...
By comparison,
\begin{align*}
\angk\kappa x{\alpha(s)}{\gamma_n(s-t)}&\le\angk\kappa x{\alpha(s)}{q_n}<
\\
&<\delta_n. 
\end{align*}
Therefore \ref{eq:ang+dist} implies that
...
Therefore, by the standard Rademacher theorem, $h_t'(s)$ is defined for all $t\in T$ and almost all $s\in\II$.

Set
\[w^+(s)\df\limsup_{\substack{t\in T\\t\to s-}} \{h'_t(s)\}.\]
Let us show that
\[w^+(s)\ae1.\eqlbl{eq:w+=1}\]
Not that once it is proved, Lemma \ref{lem:CBB-diff-curve} implies the proposition in the $\Alex{}$ case.
...
That is, for any $\eps>0$, we have that $w^+(s)\ge1-\eps$ almost everywhere.
Therefore \ref{eq:w+=1} follows.
...
After that Lemma \ref{lem:CBA-diff-curve} implies the proposition in the $\CAT{}$ case.
\qeds
...
The proof is a reduction to the 1-dimensional case (Proposition~\ref{prop:Rademacher-dim=1}) by standard arguments \cite{kirchheim,margulis-mostow}.

\parit{Proof.}
Suppose that $f\:\EE^m\subto \spc{Z}$.
Without loss of generality we may assume that $\Dom f$ is bonded and $f$ is Lipschitz.

By Proposition~\ref{prop:Rademacher-dim=1}, for any fixed vector $v$ the value $\dd_x(v)$ is defined for almost all $x\in\Dom f$.
