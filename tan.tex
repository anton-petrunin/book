...
\parit{Proof.}
Recall that the tangent space $\T_p$ can be considered as a subspace of the ultratangent space $\T_p^\o$.
...
It remains to observe that $\dd_pf=(\dd_p^\o f)|_{\T_p}$.
\qeds
...
Let $\spc{X}$ be a complete length $\Alex\kappa$ or $\CAT\kappa$  space
and $f\:\spc{X}\subto\RR$ be a 
locally Lipschitz 
and 
...
\parit{Proof.} According to thr definition of gradient,
\begin{align*}
 |\nabla_p f|\cdot(1-\eps)
&<
(\dd_p f)(v)
...
\label{lem:grad-lip}
Let $\spc{L}$ be a complete length $\Alex{}$ space,   
$f\:\spc{L}\subto\RR$ satisfy $f''+\kappa\cdot f\le \lambda$ for some $\kappa,\lambda\in\RR$, 
$[p q]\subset \Dom f$ 
and $\ell=\dist{p}{q}{}$.
Then
\[\<\dir pq,\nabla_p f\>\ge
...
In this section we collect a number of corollaries of the following lemma.
...
Let $\spc{L}$ be a complete length $\Alex{}$ space 
and $f\:\spc{L}\subto\RR$ be a locally Lipschitz semiconcave subfunction.
...
will be called the \emph{linear subcone}\index{linear subspace} of $\T_p$.
\end{thm}
