...
\begin{thm}{Theorem}
Let $\spc{U}$ be a complete length $\CAT{}$ space and $p\in \spc{U}$.
Then $\T_p\spc{U}$ is a length space.
\end{thm}

\parit{Proof.}
Assume $\spc{U}$ is $\CAT\kappa$.
...
Fix a pair of positive small numbers $\zeta \ll \delta\ll \eps$.
Let $x_\eps\in[px]$ and $y_\eps\in [py]$ denote the points such that 
$\dist{p}{x_\eps}{}=\dist{p}{y_\eps}{}=\delta$.
Let $z_\eps$ denotes $\zeta$-midpoint of $x_\eps$ and $y_\eps$.

Since $\delta\ll \eps$  we can assume that 
\[\angk\kappa p{x_\eps}{y_\eps}<\mangle \hinge pxy+\eps.\]
By since $\zeta \ll \delta$, by Alexandrov lemma, we have
\[\angk\kappa p{x_\eps}{z_\eps}+\angk\kappa p{y_\eps}{z_\eps}< \angk\kappa p{x_\eps}{y_\eps}+\eps.\]
By construction
\[\angk\kappa p{x_\eps}{z_\eps}=\angk\kappa p{y_\eps}{z_\eps}.\]
Applying angle comparison \ref{cat-hinge}, we get \ref{eq:midpoint}.
\qeds
...
Let $\spc{L}$ be a proper length $\Alex{\kappa}$ space and $p,q\in \spc{L}$ be distinct points. 
Show that 
\[(\d_q\dist{p}{}{})(v)=-\sup\set{\<\xi,v\>}{\xi\in\Dir{q}{p}}.\]
In particular, if there is unique geodesic $[p q]$, then
\[(\d_q\dist{p}{}{})(v)=-\<\dir q p,v\>.\]
The latter holds if $q\in \Str(p)$ and if $p\in \Str(q)$.
\end{thm}
