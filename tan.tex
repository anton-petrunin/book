...
In this section we will prove an extension of Rademacher theorem for $\Alex{}$ and $\CAT{}$ spaces \ref{thm:Rademacher-CBB+CBA} proved by Alexander Lytchak \cite{lytchak:diff}.
The proof essentially use another generalization of Rademacher theorem (\ref{thm:Rademacher-md}) with metric differential; in the 1-dimensional case,  the latter equivalent to the theorem about speed (\ref{thm:speed}). 
...
For each $n$, choose $q_n\in \Str(x,p_n)$ sufficiently close $\alpha(t_n)$ so that the inequality 
...
Fix a countable set of unit vectors $\{v_i\}$ in $\EE^m$.
...
It follows that $\Im\dd_xf$ is $\Alex{0}$ or $\CAT0$ if the space $\spc{Z}$ is $\Alex{}$ or $\CAT{}$ correspondingly.
It remains to apply Exercise \ref{mink+alex=euclid} or \ref{mink+CAT=euclid} if the space $\spc{Z}$ is $\Alex{}$ or $\CAT{}$ correspondingly.
\qeds
