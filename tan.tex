...
the $\CBB{}{}$ and $\cCat{}{}$ spaces have \index{space with defined angles}\emph{defined angles};
that is the angle $\mangle\hinge pxy$ is defined for any hinge $\hinge pxy$ in the space.

Fix a metric space  $\spc{X}$ is a with defined angles and a point $p\in \spc{X}$. 

Consider the set $\mathfrak{S}_p$ 
of all unit-speed geodesics  which start at $p$.
...
\begin{thm}{Exercise}
Assume $\spc{L}\in\CBB{}{}$,  and $[px]$, $[py]$ be two geodesics starting at the point $p\in\spc{L}$ which correspond to the same geodesic direction $\xi\in \Sigma'_p$.
Show that 
...
Note that if this condition in the Definition~\ref{def:right-derivative} holds for one choice of the sequences $(v_n)$ and $(\gamma_n)$ then it holds for any other choice of that type.
...
The differential $\d_pf\:\T_p\to\RR$ is Lipschitz.
Moreover, the Lipschitz constant of $\d_pf\:\T_p\to\RR$ does not exceed the Lipschitz constant of $f$ in a neighborhood of $p$. 
\end{subthm}
...
Recall that we assume that $\o$ is a once for all fixed choice of selective ultrafilter.
...
it corresponds to a point $x^n\in \o\cdot\spc{X}$.
Note that if $x\ne y$ then 
...
Thus, in the estimate \ref{eq:lem:ohta*}, instead of $\dir p{w_t}$, one should take $\dir p{w'_t}$ for some point $w_t'\in \Str(p)$ near $w_t$ (i.e. $\dist{w_t}{w_t'}{}=o(t)$)
\end{enumerate}
\qedsf

\parbf{Ultradifferential.}
Given a function $f\:\spc{L}\to\RR$, consider sequence of functions $f_n\:n\cdot\spc{L}\to\RR$, defined by 
\[f_n(n\cdot x)=n\cdot(f(x)-f(p)),\]
