...
Then $\T_p\spc{U}$ is a length space for any $p\in \spc{U}$.
\end{thm}
...
and let $S\subset \HH\times \RR$ be the graph of $f$ 
...
The proofs of both lemmas are similar.
...
For each $n$, choose $q_n\in \Str(x)$ sufficiently close to $\alpha(t_n)$ such that the inequality 
...
It remains to apply exercises \ref{mink+alex=euclid} or \ref{mink+CAT=euclid} if the space $\spc{Z}$ is $\Alex{}$ or $\CAT{}$, respectively.
\qeds
...
Let $\spc{L}$ be a length $\Alex{\kappa}$ space, and let $p,q\in \spc{L}$ be distinct points. 
...
let $f\:\spc{U}\subto\RR$ be a locally Lipschitz semiconcave subfunction,  
and let $p\in \Dom f$.
...
As it was shown in Halbeisen's example (Section \ref{halbeisen}),  
...
its gradient field is sketched in the figure.
...
\begin{thm}{Exercise}\label{ex:|grad|=1}
Let $p$ be a point in a complete length $\Alex{}$ space.
Show that 
\[|\nabla_x\distfun p|=1\]
...
Let $\spc{Z}$ be a complete length space,
let $f\:\spc{Z}\subto\RR$ be locally Lipschitz and semiconcave, 
and let $p\in \Dom f$.
Suppose that $\spc{Z}$ is either $\Alex{}$ or $\CAT{}$.
...
Suppose that $\spc{Z}$ is either $\Alex{}$ or $\CAT{}$.
Then $\phi\circ f$ is semiconcave and  $\nabla_x(\phi\circ f)\z=\phi^+(f(x))\cdot\nabla_x f$ for any $x\in\Dom f$.
\end{thm}

The following inequalities describe an important property of the \textit{gradient
vector field}.
...
\begin{subthm}{} if $\kappa=1$ and $\lambda=0$, we have
\[\<\dir pq,\nabla_p f\>\ge
...
\label{cor:grad-lip}
Let $\spc{Z}$ be a complete length space, 
let $f\:\spc{Z}\subto\RR$ be locally Lipschitz and $\lambda$-concave, 
and let $[p q]\subset \Dom f$.
Suppose that $\spc{Z}$ is either $\Alex{}$ or $\CAT{}$.
Then
\[
\<\dir p q,\nabla_p f\>
...
Let $\spc{Z}$ be a complete length space, 
let $f,g\:\spc{Z}\subto\RR$, 
and let $p\in\Dom f\cap\Dom g$.
Suppose that $\spc{Z}$ is either $\Alex{}$ or $\CAT{}$.
...
For example, consider $\spc{Z}_n\z=\RR$, $f_n(x)=-|x|$, and $x_n\to 0+$.
 ...
 From the convergence of gradient curves (proved in \ref{ultr-lim-g-curve}), 
...
Let $\spc{L}$ be a complete length $\Alex{}$ space, and let $p\in \spc{L}$.
...
has a unique polar vector such that $|u^*|=|u|$ which is $u^*=(-v_1, \ldots, -v_{n-1}, v_n)$.
...
Let $\spc{L}$ be a complete length $\Alex\kappa$ space, and let $a,b,p$
...
Let $\spc{L}$ be a complete length $\Alex{\kappa}$ space, let $p\in \spc{L}$, and let $u,v\in\T_p$.
...
Let $\spc{L}$  be a complete length $\Alex{\kappa}$ space,
and let $p\in \Str(x_1,\dots,x_n)$.
Then there is a subcone $E\subset \T_p$ that is isometric to a Euclidean space such that $\ddir p{x_i}\in E$ for every $i$.
\end{thm}

\parit{Proof.} 
By the definition of $\Str(x_1,\dots,x_n)$ (\ref{def:straight}), $\ddir{p}{x_i}\in \Lin_p$ for each $i$.
...
Let $u$ and $v$ be such that $u+\tfrac{1}{2}\cdot x=0$ and $v+\tfrac{1}{2}\cdot y=0$.
