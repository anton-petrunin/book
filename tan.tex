...
Let $z$ denote the midpoint for $\bar x$ and $\bar y$.
...
By
% Buyalo's theorem
the theorem of Buyalo \cite ?, sharpened by the authors in \cite ?,
%%\ref{thm:buyalo} and AKP
...
\parit{Proof.} According to the definition of gradient,
\begin{align*}
 |\nabla_p f|\cdot(1-\eps)
&<
(\dd_p f)(v)
...
The following corollary states that the gradient vector field is monotonic in  a sense similar to the definition of \index{monotone operator}\emph{monotone operators}; see for example \cite{phelps}.
...
\parit{Proof.} By Proposition~\ref{prop:opposite}, both $v$ and $w$ satisfy the condition in corollary~\ref{cor:polar}. 
...
Note that for any $v\in\Lin_p$ there is a line $\ell$ that contains $v$ and $\0$.  Therefore by \ref{cor:splitting}, $\Lin_p$ is isometric to a Hilbert space.
\qeds
