...
The inequality \ref{eq:inverse-product-axiom} might be strict for some spaces (see Exercise \ref{ex:schroeder-foetch}).
...
Perelman proved this implication, by combining an idea of C.~Plaut with the technique of gradient flow (see \cite{perelman-petrunin:qg}).
...
We can choose this arreay on such a way that 
...
\begin{wrapfigure}[5]{r}{66mm}
\begin{lpic}[t(-10mm),b(0mm),r(0mm),l(0mm)]{pics/two-balls(0.50)}
\lbl[r b]{22,26;$\bar p$}
\lbl[r b]{109,26;$p$}
\lbl[l t]{15,18;$\bar R$}
\lbl[l t]{101,18;$R$}
...
According to Plaut's theorem (\ref{thm:almost.geod}),
...
We prove implications (\ref{SHORT.LinDim+}) $\Rightarrow$ (\ref{SHORT.LinDim}) $\Rightarrow$ (\ref{SHORT.thm:dim-infty:rank}) $\Rightarrow$ (\ref{SHORT.pack}) $\Rightarrow$ (\ref{SHORT.LinDim+}) $\Rightarrow$ (\ref{SHORT.TopDim}) $\Rightarrow$ (\ref{SHORT.pack}).
...
$\spc{X}\times\spc{Y}$ has isometric copy of $\EE^n$.
(Compare with \cite{schroeder-foetch}.)
\end{thm}

\begin{thm}{Exercise}\label{exer:banach}
Suppose $V$ is an m-dimensional Banach space.
Then
\begin{enumerate}[(a)]
\item $V \in\cCat{}{}$  iff $V$ is isometric to $\EE^m$;
\item $V \in\CBB{}{}$   iff $V$ is isometric to $\EE^m$;
\end{enumerate}
\end{thm}
...
(Compare with \ref{thm:tan4finite}.)
\end{thm}
