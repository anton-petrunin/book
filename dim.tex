...
We say that a point array $(a^0,a^1,\dots,a^\kay)$ in $\spc{L}$
 is \index{strut}\emph{$\kappa$-strutting} for a point $p\in\spc{L}$ if $\angk\kappa p {a^i}{a^j}>\tfrac\pi2$ for all $i\not=j$.
\end{thm} 
...
\le\tfrac{11}{\eps^2}\cdot|\distfun{\bm{a}}{p}-\bm{x}| holds.
\eqlbl{co-lip}\]
...
%As the main dimension-like invariant, we will use  the linear dimension $\LinDim$; 
%see Definition~\ref{def:lin-dim}, in other words by default {}\emph{dimension} means {}\emph{linear dimension}.
%*S: Note move to start of chapter. 
...
of all points $p\in \spc{L}$ such that there is a distance-preserving cone embedding $\EE^m\hookrightarrow \T_p$.
...
%*S. this should be"and the third author"
...
Assume there is a distance-preserving cone embedding $\iota\:\EE^{m}\hookrightarrow \T_p\spc{L}$ (see Section ??).
Then either
\begin{subthm}{}
 $\Im\iota=\T_p\spc{L}$, or
\end{subthm}

\begin{subthm}{} there is a point $p'$ arbitrarily close to $p$ such that there is a distance-preserving cone embedding $\iota':\EE^{m+1}\hookrightarrow \T_{p'}\spc{L}$.
\end{subthm}
\end{thm}
...
that is, an array of directions $\zeta^1,\zeta^2,\dots,\zeta^n\in \iota(\mathbb{S}^{m-1})$ such that $n=\pack_\eps \mathbb{S}^{m-1}$ and $\mangle(\zeta^i,\zeta^j)>\eps$ for any $i\not=j$.

Choose an array of points $x,z^1,z^2,\dots,z^n\in\spc{L}$ such that
...
Let $n=\pack_{\eps}\oBall(p,R)$ and $x^1,\dots, x^n\in\oBall(p,R)$ be a maximal $\eps$-packing;
that is, $\dist{x^i}{x^j}{}>\eps$ for all $i\not=j$.
Without loss of generality we may assume that $x^1,x^2,\dots, x^n\in \Str(\bar p)$.
Thus, for each $i$ there is a unique geodesic $[\bar p x^i]$ (see \ref{thm:almost.geod}).
Choose a factor $s>0$ so that $\bar r>s\cdot(\dist{p}{\bar p}{}+R)$, where ??.
%S. need to define \bar r
...
%*S. This comment moved to end of proof 
...
is called a \index{distance chart}\emph{distant chart},
the restrictions $x^i=\distfun{a^i}{}{}|_{\oBall(p,R)}$ are called \index{coordinate of the distant chart}\emph{coordinates}
and the restriction $y=\distfun{b}{}{}|_{\oBall(p,R)}$ is called \index{strut of the distant chart} the \emph{strut} of the distace chart.
\end{thm}
...
Note that distance map $\distfun{\bm{a}}{}{}$ is Lipschitz
and its restriction $\distfun{\bm{a}}{}{}|_{\oBall(p,R)}$ is open;
the later follows from the right-inverse theorem (\ref{thm:right-inverse-function:open-map}).
...
depends continuously from  $\kappa$, $\dist{q}{a^i}{}$, $\dist{q}{a^j}{}$ and $\dist{a^i}{a^j}{}$.
\qeds
...
and {}\emph{upper} and  \index{box counting dimensions}\emph{lower box counting dimensions}
(also known as \index{Minkowski dimension}\emph{Minkowski dimensions}), 
...
\HausDim\Omega,
\]
where $\TopDim$ and $\HausDim$ denotes  topological dimension (\ref{def:TopDim}) and Hausdorff dimension (\ref{def:HausDim}) respectively.
...
but we have to apply existence of distance chart (\ref{thm:inverse-function}) 
