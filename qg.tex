...
Alexandrov defined quasigeodesics as curves that ``turn'' right and left simultaneously.
...
From \ref{thm:d_q dist_p(v)=-<dri p q, v>}, we have $(\dd_p\distfun{q}{}{})(x)\z=-\<\xi,x\>$.
...
\parbf{Remark.}
The inequality (\ref{SHORT.k-convex-angle}$'$) states that in a ``triangle'' with a quasigeodesic as one side and two geodesics as the other two other sides, one has usual comparison for the angle adjacent to the quasigeodesic.
Note however, that it gives no controle on the angle between the geodesic sides in this triangle.
...
We will construct a map $\map\:\T_{p_\infty}\to K_\o$ that satisfies the following properties:
\begin{enumerate}[(i)]
...
According to Corollary~\ref{cor:S>Sigma}, $\Sigma_{p_n}\le \mathbb{S}^{m-1}$ for each $n$;
thereofre $\Sigma_\o\le \mathbb{S}^{m-1}$.
...
(\dd_{p_\infty}\distfun{q_\infty}{}{})(x)&\equiv-\<\xi_\infty,x\>,
&(\dd_{p_n}\distfun{q_n}{}{})(x)&\equiv-\<{\xi_n},x\>.
...
\[
\begin{aligned}
\mangle\left({\map(\xi_\infty)},\map(\beta_\infty^\pm(t_0))\right)
&\ge
\mangle\left({\xi_\infty},{\beta_\infty^\pm(t_0)}\right),
\\
\mangle\left({\map(\beta_\infty^+(t_0))},{\map(\beta_\infty^-(t_0))}\right)
&\ge
\mangle\left({\beta_\infty^+(t_0)},{\beta_\infty^-(t_0)}\right).
\end{aligned}
...
First, let us define a class of \emph{probation curves} that includes all curves satisfying condition (\ref{SHORT.qg+concave:strong}) for the functions with controlled concavity.
Thus, it will be sufficient to show that any probation curve $\gamma$ is a $\kappa$-quasigeodesic.

\begin{thm}{Definition}
A curve $\gamma$ in an $m$-dimensional complete length $\Alex\kappa$ space $\spc{L}$ is called \emph{probation curve}\index{probation curve} if for any $\lambda\in \RR$ and any subfunction $f\:\spc{L}\subto\RR$ of controlled concavity type $(\lambda,\kappa)$, we have that $f\circ\gamma$ is $\lambda$-concave.
\end{thm}
...
For the canonical convergence $n*\spc{L}\xto{\tau_p} \T_p$ defined in \ref{thm:approx4tan}, 
we have
...
If in addition 
if $f$ is native, 
then subfunction $f+\tfrac{1}{2\cdot s}\cdot\distfun[2]{y}{}{}$ is native, see \ref{thm:native-operations}.
Thus, if $x$ is a foot point of $y\in\Dom f_s$
then, from the existence of supporting vector, we have 
$\dd_y f+\tfrac{1}{2\cdot s}\cdot\dd_y\distfun[2]{x}{}{}=0$.
...
\le
f_s\circ\gamma(t_0)
+ \Const\cdot\tau+\tfrac{1}{2}\cdot\left[\lambda-O(s)\right]\cdot\tau^2.
...
(we can assume $x\not=y$, otherwise \ref{eq:lamb-conc-1} follows since $f_s\le f$, $\dd_x f\equiv0$ and $\dist[2]{y}{y_\tau}{}\le\tau$).
...
Indeed, for any $p\in \spc{L}$, the function $\tfrac{1}{2}\cdot\distfun[2]{p}{}{}$ is $\left[1+O(r^2)\right]$-concave in $\oBall(p,r)$.
Thus, from \ref{eq:lamb-conc-4} and \ref{thm:prop-gexp:func}, for all small $\tau$,  we have
\[\begin{aligned}
\dist[2]{x_\tau}{y_\tau}{}
&\le \dist[2]{x}{y_\tau}{}+2\cdot\tau\cdot\dist[{{}}]{x}{y_\tau}{}\cdot\cos(\alpha+\beta_\tau)+\left[1+O(s^2)\right]\cdot\tau^2,
\\
\dist[2]{x}{y_\tau}{}
&\le \dist[2]{x}{y}{}-2\cdot\tau\cdot\dist[{{}}]{x}{y}{}\cdot\cos\alpha+\left[1+O(s^2)\right]\cdot\tau^2
\end{aligned}
...
f(x_\tau)
&\le
f(x)+(\dd_x f)(\xi_\tau)\cdot \tau+\tfrac\lambda2\cdot\tau^2
=
\\
&=
...
First we construct $\kappa$-convex curves using the radial curves, which are \emph{$\kappa$-monotonic} in the sense of the following definition.
Second we construct pre-quasigeodesics using $\kappa$-convex curves.
...
\[\beta\:[a,\infty)\to \spc{L},\ \ \beta(t)=\left[
\begin{matrix}
...
\[\text{the function}\ \ t\mapsto\tangle\mc\kappa(\beta|^t_a,p)\ \ \text{is non-increasing for}\ \  t\ge b.\eqlbl{eq:ext-mono-1}\]
...
we have $\cos\phi=-(\dd_q\distfun{p}{}{})(v)$,
...
Note that for any $p$, 
\[(\distfun{p}{}{}\circ\alpha)^+(0)=-\cos\left(\lim_{t\to0+}\tangle\mc\kappa(\alpha|_0^t,p)\right).\]
...
\[\beta_{\xi,\eps}(t)=\alpha_{v_n}(t-n\cdot\eps),\ \ \text{where}\ \ n={\lfloor t/\eps\rfloor}.\]
...
Passing to a partial limit, $\beta_{\xi,\eps}\to\beta_{\xi}$ as $\eps\to 0$ we get a curve $\beta_{\xi}$ that is $\kappa$-monotonic with respect to any value of parameter.
...
then its joint
\[\gamma\:[a,\infty)\to \spc{L},\ \ \gamma(t)=\left[
\begin{matrix}
...
such that 
\[\mu_\gamma(a,b) <\eps[\mangle(\xi,\nu)+b-a],\ \ b-a<\eps,\ \ \text{and}\ \ \mangle(\xi,\nu)<\eps,\]
where 
...
\[\mu_\gamma(a,a+\tfrac{s}{3}) <\Const\cdot\left[\mangle(\xi,\nu)\right]^2.\]
...
\[\psi(s)< 10 (\phi(s)+s)^2\ \ \ \text{and}\ \ \ \phi(s)+s\le\eps.\]
...
\parit{Proof.} Note that $\Sigma_x\in$ is $(m-1)$-dimensional (see \ref{thm:tan4finite}). 
...
Let $\Theta\subset \spc{L}$ be a maximal open set that satisfies the following property:
...
For $\gamma_\eps\:[0,a]\to \spc{L}$, applying Milka's lemma, we can find a vector $w$ that is polar to $\gamma_\eps^-(a)$ and such that $|w|=|\gamma_\eps^-(a)|$ (if $a=0$, set $w=v$). 
Further:
\begin{itemize}
...
Then extend $\gamma_\eps$ behind $a$ as $\gamma_\eps(a+t)=\bar\gamma(t)$, clearly $\gamma_\eps([a,a+\eps])\subset \spc{L}\setminus\Theta_\eps$.
\end{itemize}
...
If $\Theta\not=\spc{L}$, then there is $p\in \spc{L}\setminus \Theta$ and $r>0$ such that for any $x\in\oBall(p,\tfrac{r}{6})\setminus \Theta$ and $\xi\in \Sigma_x$ there is a quasigeodesic $\gamma\:[0,\tfrac{r}{6}]\to \spc{L}$ with  $\gamma^+(0)=\xi$.
\end{clm}


First let us list the properties of $p\in \spc{L}$ and $r>0$ that are sufficient for the key claim to hold.

Set $\mathfrak C=\spc{L}\setminus \Theta$. 
...
such that the enthropy $\mu_{\gamma_\eps}$ vanish on $(0,\tfrac{r}{6})\setminus \Upsilon_\eps$ and for each $i$ we have
...
any further. 
Applying Milka's lemma (\ref{lem:milka}), we can find a vector $w\in\T_p$ that is polar to $\gamma^-_\eps(a)$ and such that $|w|=|\gamma^-_\eps(a)|$.
...
<
\eps\cdot\left[\tfrac{r}{6}+\sum_i\mangle(\xi_i,\nu_i)\right].\]
...
\mangle(\xi_i,\nu_i)
<
2\cdot\left[\mangle(\dir{x_i}{q_j},\xi_i)-\mangle(\dir{x_i}{q_j},\nu_i)\right]
...
\le
\\
&\le 10\cdot\left[\cos\mangle(\dir{x_i}{q_j},\nu_i)-\cos\mangle(\dir{x_i}{q_j},\xi_i)\right]
\le
\\
&\le 20\cdot\left[\<\dir{x_i}{q_j},\gamma^-(a_i)\>-\<\dir{y_i}{q_j},\gamma^-(b_i)\>+\lambda (b_i-a_i)\right]
=
\\
...
\parit{Proof.} Set $X=E\cap \Sigma_p$.
...
Otherwise, if $\Sigma_\xi X=\emptyset$ and $X$ contains at least two points, choose $\xi^*$ to be closest point in $X\setminus\xi$ from $\xi$. 
...
$\trig{\tilde \xi}{\tilde \eta}{\tilde \xi^*}
=
\modtrig1(\xi\eta\xi^*)$, 
we have $\mangle\tilde \xi\le \tfrac\pi2$ 
and $\mangle\tilde \xi^*\le \tfrac\pi2$.
Hence 
\begin{align*}
\mangle(\xi,\eta)+\mangle(\eta,\xi^*)
&=\dist{\tilde \xi}{\tilde \eta}{}+\dist{\tilde \eta}{\tilde \xi^*}{}
\le\pi.
\end{align*}
\qedsf
...
Set
\[V(r)=\int\limits_\spc{L}\vol_m\oBall(x,r)\cdot\dd_x\vol_m,\]
...
\[\sum_{i=1}^{n-1}\left(\pi-\mangle\hinge{a_i}{a_{i+1}}{a_{i-1}}\right).\]
Further, variation of turn for a curve can be defined as lower limit of turns of broken geodesics that converge to a given curve (you can choose in which sense).
...
In this case variation of turn can be estimated by the \emph{total turn to a point}\index{turn to the point $p$}, which defined as follows:
\[\sum_{i=1}^{n-1}\max\left\{0,\pi-\angk\kappa{a_i}{a_{i-1}}{p}-\angk\kappa{a_i}{a_{i+1}}{p}\right\}.\]
...
This class of curves should be interesting in the metric spaces that has ``a lot'' of semiconcave functions.
...
\begin{thm}{Exercise}
Construct a three-dimensional compact length $\Alex0$ space $\spc{L}$ with nonempty boundary and a quasigeodesic $\gamma$ in $\spc{L}$ that is not a projection of any quasigeodesic in the doubling of $\spc{L}$.
