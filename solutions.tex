...
Let $\spc{X}_0=[0,1]$ and $\spc{Y}_0=\Mid(\spc{X}_0)=\{0,1/2,1\}$.

Given a metric space $(\spc{Z},d)$ define  new metric space $P_k(\spc{Z})$ as follows. For every pair of distinct  points
$p,q\in \spc{Z}$ attach to $Z$ a countable collection of intervals of $\{I_i\}_{i\ge 1}$ of length $\frac{\dist{p}{q}{}}{2^ki}$ where one end of each $I_i$ is glued to $p$ and the other to $q$.
Then  $P_k(\spc{Z})$ carries a naturally defined   inner metric which is complete if $(\spc{Z},d)$ is complete.

Let $\spc{X}_1=P_1(\spc{Y}_0)$ and $\spc{Y}_1=\Mid(\spc{X}_1)$. Proceeding by induction we define $\spc{X}_k=P_k(\spc{Y}_{k-1})$ and $\spc{Y}_k=\Mid(\spc{X}_k)$. %The metric on each $\spc{Y}_k$ is taken to be restriction of the natural inner metric on $\spc{X}_k$.
Note that by construction the inclusion $\spc{Y}_k\subset \spc{Y}_{k+1}$ is distance preserving. Let $\spc{Y}_\infty=\cup_{k\ge 1}\spc{Y}_k$ with the obvious metric and let $\spc{Y}=\bar {\spc{Y}}_\infty$ be its metric completion. Then $\spc{Y}$ is a length space since it satisfies the almost midpoint property. But it is not hard to see that no two distinct points in $\spc{Y}$ can be connected by a shortest geodesic. \qeds
