...
Given a real number $\kappa$, the \index{plane!$\kappa$-plane}\emph{model $\kappa$-\hspace{0pt}plane} will be a complete simply connected 2-\hspace{0pt}dimensional Riemannian manifold of constant curvature~$\kappa$.
...
are respectively nondecreasing (in fact, decreasing, if $a\z\notin\{|b\z-c|, b+c\}$%??? V: I am confused by the caveat. the comparison angle is not defined in this case
) and nonincreasing (in fact, increasing, if $\phi\notin\{0,\pi\}$ ).%??? V:  don't understand this caveat either.
\end{subthm}
...
\parit{Proof.} Applying scaling, we may assume that $\kappa=1$ and thus $\varpi\kappa=\pi$ and $\Lob2{\kappa}=\mathbb{S}^2$.
