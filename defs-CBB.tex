...
\parit{Proof; (\ref{SHORT.2-sum}).} Since $z\in \l]x y\r[$, we have $\angk\kappa z x y=\pi$. 
...
but meets the following condition: for any 3 points $p,x,y\in \spc{X}$ there is a geodesic $[x y]$ such that for any $z\in \l]x y\r[$
...
We obtain a convex curvelinear triangle formed by a curve $\~\gamma\:[0,T]\to\Lob2{\kappa}$ --- the limit of broken line $\~x_0\~x_1\dots \~x_n$ 
and two geodesics $[\~p'\,\~\gamma(0)]$,
$[\~p'\,\~\gamma(T)]$.
