...
\begin{thm}{Examples}
By Rauch comparison, any Riemannian manifold with induced metric is locally $\Alex{\kappa}$ if and only if its sectional curvature of is bounded below by $\kappa$.

By Toponogov comparison, any complete Riemannian manifold of sectional curvature bounded below by $\kappa$ is globally $\Alex{\kappa}$. This will follow in more generality from the globalization theorem  below \eqref{thm:glob}.

Any convex set in a $\Lob m{\kappa}$ is $\Alex{\kappa}$.

Any convex hypersurface in $\Lob m{\kappa}$, equipped with the induced length metric, is $\Alex{\kappa}$.
Morevoer, the same holds in complete Riemannian manifolds with sectional curvature bounded below by $\kappa$ \cite{alexander-kapovitch-petrunin-buyalo}.
\end{thm}
...
\qeds
\section{Examples of CBB spaces}
