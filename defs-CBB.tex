...
here $\varpi\kappa$ denotes the diameter of model space $\Lob2\kappa$.

\index{$\Alex{}$}
\begin{thm}{Four-point comparison}
\label{df:1+3}
A quadruple of points $p,x^1,x^2,x^3$ in a metric space satisfies \index{$\Alex{}$!$\Alex\kappa$ comparison}\emph{$\Alex\kappa$ comparison} 
if 
...
\begin{subthm}{}
$\spc{L}$ is 
\index{$\Alex{}$!locally $\Alex{\kappa}$ space}
...
\item In the definition of $\Alex{\kappa}$, when $\kappa>0$ most authors assume in addition that the diameter is at most the model diameter~$\varpi\kappa$.
...
\begin{thm}{Example}
If $G< O(n+1)$ is a closed subgroup, then $\SS^n/G$ is $\Alex{1}$ and $\R^{n+1}/G$ is $\Alex{0}$.
\end{thm}
...
that is,
the $\Alex\kappa$ comparison holds for this quadruple in  $\spc{M}$.
\qeds
...
A metric space $\spc{X}$ is called \index{G-delta geodesic space}\emph{G-delta geodesic} 
...
we use the notation
\[\Str(x^1,x^2,\dots,x^\kay)=\bigcap_i\Str(x^i).\]
\end{thm}
...
Moreover, for any $q\in \Str(p)$ there is a unique geodesic $[p q]$.

In particular, $\spc{L}$ is G-delta geodesic.
\end{thm}

This theorem was proved by by Conrad Plaut \cite[Th. 27]{plaut:survey}.
...
Note that $\Str(p)=\bigcap_{n}\Omega_n$;
therefore, $\Str(p)$ is a dense G-delta set.
...
that is, $z_n$ is a Cauchy sequence, and its limit $z$ satisfies \ref{eq:thm:connect-1}.
\qeds
...
Let $p$, $x$ , $y$ be points in a complete length $\Alex{\kappa}$ space $\spc{L}$.
Suppose that the model triangle
...
The proof is contained in the first part of \ref{SHORT.2-sum}$+$\ref{SHORT.point-on-side}$\Rightarrow$\ref{SHORT.angle} of the proof above.
...
We say the triangle $\trig{x^1}{x^2}{x^3}$ is \emph{$\kappa$-thick} \index{$\kappa$-thick} if the natural map (see definition \ref{def:k-thin}) 
...
\begin{thm}{Corollary}
A complete G-delta geodesic space $\spc{L}$ is $\Alex{0}$ if and only if for any $p\in \spc{L}$, the function $\distfun[2]{p}{}{}\:\spc{L}\to\RR$ 
is $2$-concave.
\end{thm} 

\parit{Proof of \ref{thm:conc}.} 
...
\begin{subthm}{ex:busemann-CBB:a}
If $\kappa=0$, then the Busemann function $\bus_\gamma$ is  concave.
\end{subthm}

\begin{subthm}{ex:busemann-CBB:b}
...
Exercise~\ref{ex:busemann-CBA} is an analogous statement for upper curvature bound.
...
Often it is easier to write proofs in terms of function comparison but think in terms of developments.
...
When we say that the $\kappa$-development of $\gamma$ with respect to $p$ is defined, we always assume that $0<\dist{p}{\gamma(t)}{} < \varpi\kappa$ for
all 
$t\in \II$.
\end{thm}
...
\theta(t)
&=
\int\limits_{t_0}^{t}\frac{\sqrt{1-(\rho')^2}}{\sn\kappa\rho},
\end{align*}
...
We need the following analogs of sub- and super-graphs % we use sub-super- BUT NOT epi-hypo-
...
recall that geodesics $[p'x_i]$ exist for all $i$
(see \ref{thm:almost.geod}).
...
Consider the space $L$ glued from the solid model triangles $\trig{\tilde p'}{\tilde x_{0}}{\tilde x_1},\z\dots,\trig{\tilde p'}{\tilde x_{n-1}}{\tilde x_n}$ along the corresponding sides.
Note that $\theta$ is the total angle of $L$ at $\tilde p'$.
We can assume that $L$ has nonempty interior.
Otherwise all the triangles are degenerate, and therefore $\theta=0$;
the latter holds since $\tilde p'$ is not on the polygonal line $\tilde x_0\dots\tilde x_n$.
...
We obtain a convex curvilinear triangle formed by a curve $\tilde \gamma\:[0,T]\to\Lob2{\kappa}$ --- the limit of polygonal line $\tilde x_0\tilde x_1\dots \tilde x_n$ 
...
This proves the only-if part of \eqref{thm:devel}.
...
Since $\tilde \gamma$ is convex, for a fine partition we have that the polygonal line $\tilde y_0\tilde y_1\dots\tilde y_n$ is also convex.
...
(Let us recall that $\hinge x q y$ and $\hinge x q z$  are short notations for the pairs $([x q],[x y])$ and $([x q],[x z])$,  thus these two hinges automatically have common side $[x q]$.)
...
Our proof of the globalization theorem (\ref{thm:glob}) is based on presentations in \cite{plaut:dimension} and \cite{burago-burago-ivanov};
this proof was rediscovered independently by Urs Lang and Viktor Schroeder \cite{lang-schroeder:globalization}.
We will need the short hinge lemma (\ref{lem:devel-glob}) together with the following two lemmas.
...
Choose a geodesic $[x' p]$ and consider the  hinge $\hinge{x'}p q$ formed by $[x'p]$ and $[x' q]\subset [x q]$.(In fact by \ref{cor:unique-geod-cbb} the condition $[x' q]\subset [x q]$ always holds.)
...
Suppose $r\:\spc{X}\to \RR$ is a function, $p\in \spc{X}$,   and $\eps>0$.
...
Then the key lemma (\ref{key-lem:globalization}) applied to $p^*$ and $\ell$ slightly bigger than $r^*$ (say, such that $r^*<\ell<\min\{\varpi\kappa,\tfrac32\cdot s_0\}$) implies that
\[\mangle\hinge x{p^*}q
\ge\angk\kappa x{p^*}q\]
...
converge to points $p_\o, x_\o, y_\o$ in  $\spc{L}_\o$ as $n\to\o$,
...
(Note that according to Proposition~\ref{prp:A^omega}, $\spc{L}^\o$ is a complete length $\Alex{}$ space.)
\qeds
...
Many authors (but not us) exclude these spaces in the definition of Alexandrov space with positive lower curvature bound.
...
Lastly, if $\spc{L}$ is not geodesic and $\diam \spc{L}>\pi$, then the above argument applied to $\spc{L}^\o$ yields that each metric component of $\spc{L}^\o$ is isometric to one of the exceptional spaces. 
As all of those spaces are proper, $\spc{L}$ is a metric component in $\spc{L}^\o$.
\qeds
...
This definition works well for $\Alex{}$ spaces; for $\CAT{}$ spaces there is a similar but definition.
...
The value $\angk{\kappa+} p q r$ is called the \index{model angle!extended model angle}\emph{extended model angle} of the triple $p$, $q$, $r$.
\end{thm}
...
\qeds
\section{Sources of Examples of CBB spaces}
In this section we will collect various sources of examples of $\Alex{}$ spaces. Some of this material is addressed elsewhere in the book in which case we will just give references.

\parbf{Warped products}
These types of examples are discussed in detail in Chapter \ref{chapter:warped products}. Consult that chapter for the appropriate definitions.

\begin{thm}{Warped products}
...
\begin{subthm}{}
If $\spc{L}_1, \spc{L}_2$  are complete length $\Alex{\kappa}$ spaces with $\kappa\le 0$ then 
$\spc{L}_1\times \spc{L}_2$ is also complete length $\Alex{\kappa}$.

\end{subthm}

\begin{subthm}{}
Let $\spc{U}$ and $\spc{V}$ be complete length  $\Alex1$ spaces  with $\diam \le\pi$.
Then the spherical join $\spc{U}\star\spc{V}$ is also complete length  $\Alex1$.
...
\parbf{Convex hypersurfaces in smooth manifolds}

It is immediate from the Gauss formula that a if $(M^n,g)$ is a complete Riemannian manifold of $\sec\ge \kappa$ and $N^{n-1}\subset M$ is closed    hypersurface with nonnegative second fundamental form then $N$ also has $\sec\ge \kappa$ with respect to the induced metric. In particular $N$ is $\Alex{\kappa}$ with respect to the induced inner metric.


This has been generalized by  Sergei Buyalo \cite{buyalo} to the case of any (not necessarily smooth)  closed locally convex hypersurface in $M$ to show that  $N$ is $\Alex{\kappa'}$  for some $\kappa'<\kappa$.
The following sharp bound was proved by the authors in  \cite{alexander-kapovitch-petrunin-buyalo}

\begin{thm}{Theorem}
Let  $(M^n,g)$ is a complete Riemannian manifold of $\sec\ge \kappa$. Let $N \subset M$ be a locally convex closed hypersurface $M$ (not necessarily snooth). Then $N$ with the induced inner metric is $\Alex{\kappa}$.
\end{thm}

In particular any closed  convex hypersurface in $\R^n$ is $\Alex{0}$.

\parbf{Limits of manifolds with sectional curvature bounded below}

As is immediate from the definition Gromov-Hausdorff and ultralimits of $\Alex{k}$ spaces are again $\Alex{k}$.

In particular, Gromov Hausdorff limits of complete smooth manifolds with $\sec\ge \kappa$ are $\Alex{k}$. This is particularly useful because of the following consequence of Theorem\ref{thm:dim-infty}:

\begin{thm}{Compactness Theorem}\label{compact-thm-alex}
Let $D>0, \kappa\in \R, n\in \NN$. Then the class of $\Alex{k}$ spaces of dimension $\le n$ and $\diam\le D$ is compact in Gromov-Hausdorff topology.
Without the upper diameter bound the corresponding class is $\o$-compact or pointed Gromov-Hausdorff compact.%V:??? we don't defined pointed G-H convergence. Is it ok to mention it here? Or perhaps somewhere else.?
\end{thm}
This in particular implies Gromov precompactness theorem  \cite{gromov-MS} saying that the subclass of the above class consiting of smooth manifolds is precompact in Gromov-Hausdorff topology.

\parbf{Asymptotic cones of $\Alex{0}$ spaces}

A special case of the above theorem  is arises in the following situation.


Let $\spc{X}$ be a $\Alex{0}$ space. Let $r_n\to 0$ be any converging sequence of positive real numbers. Then the ultralimit

\[\spc{X}(\infty)
\df
\lim_{n\to\o} r_n\cdot\spc{X}.\]

is again  $\Alex{0}$. It is called the \index{asymptotic cone}\emph{asymptotic cone} of $\spc{X}$ or\emph{ the cone at infinity} of $\spc{X}$.

It has a canonical base point $\star$ equal to the $\o$-limit of $r_n\cdot x\in r_nX$ which is easily seen to be independent of $x$.

From now on we will restrict out attention to  noncompact length  finite dimensional  $\Alex{0}$ spaces.

The following result is well known, see e.g. \cite{Guij-Kap}.

\begin{thm}{Asymptotyc cones of finite dimensional $\Alex{0}$ spaces}

Let $\spc{X}$ be noncompact length  finite dimensional  $\Alex{0}$ space.
Then its asymptotic cone $\spc{X}(\infty)$ is independent of the choice of the sequence $r_n\to\infty$ and the ultrafiler $\o$.
Moreover $\spc{X}(\infty)$ is isometric to a metric Euclidean cone $\Cone(\Sigma)$ where the cone vertex corresponds to $\star$.
\end{thm}
\begin{proof}
Fix $p\in\spc{X}$. Since $X$ is finite dimensional the space of directions $\Sigma_p$ is compact. This easily implies that there exists $\epsilon(R)\to 0$ as $R\to\infty$ such that for every point $x$ with $\dist{p}{x}{}\ge R$ and any geodesic 
$[px]$ there is a ray $\gamma\:[0,\infty)\to \spc{X}$ such that 
\[
 \mangle\hinge p x {\gamma(R)}\le \epsilon(R)
\]


Therefore $\dist{x}{\gamma(R)}{}\le 10\epsilon(R)R$. This means that all points in $\spc{X}(\infty)$ come from rays starting at $p$. Moreover given two rays $\gamma_1,\gamma_2$ starting at $p$  monotonicity of the comparison angle $\angk 0 p{\gamma_1(t)}{\gamma_2(s)}$  implies that the limit $\lim_{n\to\o} r_n\cdot \dist{\gamma_1(t/r_n)}{\gamma_2(s/r_n)}{}$ is independent of the sequence $r_n$. This easily implies that $\spc{X}(\infty)$ is independent of the choice of the sequence $r_n\to\infty$ and the ultrafiler $\omega$ and moreover it is scale invariant, that is $r\spc{X}(\infty)$ is isometric to $\spc{X}(\infty)$ for any positive $r$ by an isometry preserving $\star$. This immediately implies that
$\spc{X}(\infty)$ is isometric to $T_\star \spc{X}(\infty)=\Cone \Sigma_\star$.
\end{proof}

The space $\Sigma=\Sigma_\star$ in the above theorem is called the \index{boundary at infinity} \emph{boundary at infinity} or \emph{the ideal boundary} of $\spc{X}$ and is denotes by \index{$\partial_\infty\spc{X}$} $\partial_\infty\spc{X}$. It is a length $\Alex{1}$ space of dimension equal to $\dim (\spc{X}(\infty))-1$.

Let us note that the cone at infinity is almost always singular even if $\spc{X}$ is a smooth Riemanninan manifold of nonnegative curvature.
\begin{thm}{Lemma}

 Let $\spc{X}$ be noncompact length  finite dimensional  $\Alex{0}$ space. Suppose $\spc{X}(\infty)$ is an $n$-dimensional  Riemannian manifold of nonnegative curvature with $n\ge 1$.
 
 Then $\spc{X}\iso \R^n\times \spc{Y}$ for some compact $\spc{Y}$.
\end{thm}
\begin{proof}
If $\spc{X}(\infty)$ is smooth that $\spc{X}(\infty)\iso T_\star \spc{X}(\infty)\iso \R^n$.
Now monotonicity of comparison angles easily implies that two rays in $\spc{X}$ starting at any  $p$ corrsponding to opposite points in $\partial_\infty\spc{X}$ form a line in $\spc{X}$. Now the result easily follows by the Splitting Theorem.
\end{proof}

\parbf{Polyhedral spaces}
These type of examples are discussed in detail in Chapter \ref{chapter:polyhedral}.


\begin{thm}{Polyhedral CBB spaces}
...
\parbf{Doubling and Gluing}

The following Theorem is due to the third author. 

\begin{thm}{Gluing theorem}
Let $\spc{L}_1, \spc{L}_2$ be complete length finite dimensional $\Alex{\kappa}$ spaces with nonempty boundary. Suppose $f\co \partial \spc{L}_1\to  \partial \spc{L}_2$ is an isometry.

Then the gluing of $\spc{L}$ of  $\spc{L}_1$ and $ \spc{L}_2$  along $f$ is  complete length $\Alex{\kappa}$ without boundary.%% V: Should we say anything about what boundary is? or just skip it?
\end{thm}

In  the special case $\spc{L}_1=\spc{L}_2$ and $f$ given by the identity map this theorem was proved earlier by Grigory Perelman \cite{perelman:spaces2}.

\begin{thm}{Example}
Let $\spc{L}_2=\SS^1\times [0,\infty)$ where the length of the $\SS^1$ factor is $L>0$.
Let $\spc{L}_2$ be a convex disk in $\Lob{}\kappa$ with $\kappa\ge 0$ such that the length of the boundary of $\spc{L}_2$ is also equal to $L$.

Then the "capped cylinder" obtained by the gluing of  $\spc{L}_1$ and  $\spc{L}_2$ along an isometry of their boundaries in $\Alex{0}$.
\end{thm}
In case of smooth spaces The Gluing Theorem admits a generalization to nonconvex boundaries due to Nikolay  Kosovski\u{\i} \cite{Kosovski}.

\begin{thm}{Non-convex Gluing}
Let $(M^n_1, g_1)$ and $(M^n_2,g_2)$ be  complete (as metric spaces)  smooth Riemannian manifolds with  nonempty boundary. Suppose  they both satisfy $\sec\ge\kappa$ and $f\co \partial M_1\to \partial M_2$ is an isometry such that when identified via $f$ the sum of the second fundamental forms $II_{\partial M_1}+II_{\partial M_2}\ge 0$ is nonnegative.
Then the gluing of $M_1$ and $M_2$ along $f$ is $\Alex{\kappa}$.
\end{thm}
...
it was first stated in \cite[footnote in 4.1.5]{burago-burago-ivanov}.
