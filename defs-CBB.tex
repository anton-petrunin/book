...
Since $\gamma$ does not pass thru $p'$, the comparison implies that the polygonal line $\tilde x_0\dots\tilde x_n$ does not pass thru $\tilde p'$.
...
Since $\gamma\in B_\pi(p')$ point-on-side comparison implies that the polygonal line $\tilde x_0\dots\tilde x_n$ can not pass thru the antipodal point of $\tilde p'$.

Consider the space $L$ glued from the model triangles $\trig{\tilde p'}{\tilde x_{0}}{\tilde x_1},\dots,\trig{\tilde p'}{\tilde x_{n-1}}{\tilde x_n}$ along the corresponding sides.
Note that $\theta$ is the total angle of $L$ at $\tilde p'$.
If the space $L$ has empty interior as a subset of $\SS^2$ then it  follows that it's a part of a great circle. If its length is $>\pi$ then it contains the antipodal point of $\tilde p'$ which by above is impossible.
Therefore $L$ is a polygonal region on $\SS^2$ with non empty interior.


Consider a minimizing geodesic $[\tilde x_0\tilde x_n]_L$.

By \ref{eq:thm:detr*} we may assume that $[\tilde x_0\tilde x_n]_L$ does not pass thru $\tilde p'$.
