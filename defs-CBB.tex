...
\label{df:1+3}
A quadruple of points $p,x^1,x^2,x^3$ in a metric space satisfies \emph{$\Alex\kappa$ comparison} \index{$\Alex\kappa$ comparison}
if 
...
in a $\Alex{\kappa}$ space is $\kappa$-thick.\index{$\kappa$-thick}
%*S. note change from "fat" to "thick"
\end{thm}

The following exercise is inspired by Busemann's definition in \cite{busemann}. 

%See also \cite{buser-karcher} concerning flats\index{flats}.
...
&=
\int\limits_{t_0}^{t}
\frac{\sqrt{1-(\rho'(u))^2}}{\sn\kappa\circ\rho(u)}\cdot du,
...
The set formed by all geodesics from  $\tilde p$ to the points on $\tilde \gamma$ is called the \index{development!subgraph/supergraph of a development}\emph{subgraph} of $\tilde \gamma$ with respect to $\tilde p$.
