...
for example there is a sequence of convex discs in $\mathbb{S}^2$ that converges to the hemisphere $\mathbb{S}^2_+$, which is not convex.
...
If for these two pseudometrics $\rho_1$ and $\rho_2$ the projections $\spc{X}\to(\spc{X}/{\sim},\rho_i)$ are \index{short map}\emph{short} (that is,  \emph{distance non-increasing}), then the same is true for $\rho(x,y)$.
