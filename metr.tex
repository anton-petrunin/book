...
In this chapter we fix some conventions assuming that the reader is familiar with basic notion in metric geometry.
...
is called \emph{$[0,\infty]$-metric space}.
\item The set $\ushort{\spc{X}}$ is called \emph{underling set}\index{underling set} of $\spc{X}$. 
...
With slight abuse of notations we may write $x\in\spc{X}$ meaning that $x$ is a point of $\spc{X}$.

The value $\dist{x}{y}{}$ is called \emph{distance}\index{distance} from $x$ to $y$.
If we need to emphasize 
that the distance is taken in the space ${\spc{X}}$, 
we write $\dist{x}{y}{\spc{X}}$;

The function $\dist{x}{}{}\:\ushort{\spc{X}}\to\RR$ defined as $\dist{x}{}{}\:y\mapsto\dist{x}{y}{}$ will be called \emph{distance function}\index{distance function} from $x$. 
...
\begin{thm}{Definition}
A $[0,\infty]$-metric space is called \emph{$(0,\infty]$-metric space}\index{$(0,\infty]$-metric space}
if $\dist{x}{y}{}=0$ if and only if $x=y$.
In this case the function $(x,y)\mapsto \dist{x}{y}{}$ is called \emph{$(0,\infty]$-metric}\index{$(0,\infty]$-metric}.
\end{thm}
...
The $(0,\infty)$-metric spaces will be also called \emph{metric spaces}
this is the main object of study in this book.

The notion of $[0,\infty]$-metric spaces is useful
in the various definitions and constructions.
First let us give one construction which forced us to allow infinite value for the distance.
\begin{itemize}
\item Assume $\{\spc{X}_\alpha\}_{\alpha\in\IndexSet}$ be a set of $[0,\infty]$-metric spaces.
The disjoint union 
$$\bm{X}=\bigsqcup_{\alpha\in\IndexSet}\spc{X}_\alpha$$ 
has natural $[0,\infty]$-metric on it defined the following way:
...
\end{matrix}
\]
The obtained metric space $\bm{X}$ will be called \emph{disjoint union}\index{disjoint union of metric spaces} of $\{\spc{X}_\alpha\}_{\alpha\in\IndexSet}$ denoted as $\bigsqcup_{\alpha\in\IndexSet}\spc{X}_\alpha$.

\item An other example is the construction of length-metric given in Section~\ref{sec:intrinsic}.
\end{itemize}

Now let us give a constructions which show that allowing distance between distinct point might vanish is useful.

\begin{itemize}

\item Assume a set comes with a set of $[0,\infty]$-metrics $\dist{}{}{\alpha}$ for $\alpha\in\IndexSet$.
Then 
\[\dist{x}{y}{}=\inf\set{\dist{x}{y}{\alpha}}{\alpha\in\IndexSet}\]
is a $[0,\infty]$-metric.

This construction is possible since the distance between distinct point might vanish;
even if each $\dist{}{}{\alpha}$ is a $(0,\infty)$-metric,
the $\spc{X}_\alpha$ has $(0,\infty)$-metric,
$\dist{x}{y}{}$ might be only $[0,\infty)$-metric.

\item Assume $\spc{X}$ is a set,
$\spc{Y}$ is metric spaces 
and $\map\:\spc{X}\to\spc{Y}$ be a map.
Then one can consider \emph{pulled back metric} on $\spc{X}$
defined as 
\[\dist{x}{y}{\spc{X}}=\dist{\map(x)}{\map(y)}{\spc{Y}}\]
for any $x,y\in\spc{X}$.
Even if  $\spc{Y}$ has $(0,\infty)$-metric,
the pulled back metric on $\spc{X}$ is only $[0,\infty)$
since $\dist{x}{y}{\spc{X}}=0$ if $\Phi(x)=\Phi(y)$;
i.e., 
The distance between distinct points in $\spc{X}$ 
the pulled back metric  
\end{itemize}

On the other hand, 
as you will see below,
nearly any question about the other 3 types of metric spaces can be reduced to the question about $(0,\infty)$-metric spaces.
...
Thus, $\dist{}{}{}$ defines the metric on the
quotient set $\ushort{\spc{X}}/{\sim}$ with equivalence relation 
where $x\sim y$ iff $\dist{x}{y}{}=0$.
This way we obtain a $(0,\infty]$-metric space, say $\spc{X}'$.
Often we will not distinguish between $(0,\infty]$-metric space $\spc{X}'$
and $[0,\infty]$-metric space $\spc{X}$. 


\item The relation $x\sim y$ iff $\dist{x}{y}{}<\infty$ is an equivalence relation.
The equivalence class of point $x\in\spc{X}$ will be called \emph{metric component}\index{metric component} %???Is it OK???
 of $x$; it will be denoted as $\spc{X}_x=\oBall(x,\infty)$.
Thus any metric space is a disjoint union of its metric component in the sense defined above.
\end{itemize}
...
are called respectively the  \emph{open}\index{open ball} and \emph{closed  balls}\index{closed ball}   of radius $R$ with center $x$.
\begin{itemize}
...
\item
For subsets $A, B\subset \spc{X}$, let us define distance from $A$ to a point $x$ in $\spc{X}$ as 
\[\dist{A}{x}{}
\df
\inf\set{\dist{a}{x}{}}{a\in A},\] 
and the distance between $A$ and $B$  as 
\[\dist{A}{B}{}
\df
...
\item Any  subset $A$ in a pseudometric space $\spc{X}$ will be also considered as a \emph{subspace}\index{subspace};
i.e. a pseudometric space with the pseudometric defined as restriction of the pseudometric of $\spc{X}$ to $A\times A\subset \spc{X}\times\spc{X}$.
\end{itemize}
...
if its underlying set is finite.

\item
A metric space $\spc{X}$ is called \emph{complete}\index{complete space} if any \emph{Cauchy sequence}\index{Cauchy sequence} in $\spc{X}$  converges.
Namely, a sequence of points $x_n\in\spc{X}$ is called a \emph{Cauchy sequence}\index{Cauchy sequence} if for any $\eps>0$, we have
$\dist{x_n}{x_m}{}<\eps$ for all large enough $n$ and $m$.
A sequence of points $x_n\in\spc{X}$ is called \emph{convergent}\index{convergent sequence} if there is $x\in \spc{X}$ such that for any $\eps>0$ we have $\dist{x_n}{x}{}<\eps$ for all large enough $n$.
...
\begin{thm}{Proposition}
A locally compact length space is proper.
\end{thm}
...
\section{Characteristics of metric spaces}

Let $\spc{X}$ be a metric space.
Then
\begin{itemize}
\item 
\emph{Diameter}\index{Diameter} of $\spc{X}$ is  defined as
\[\diam \spc{X}=\sup\set{\dist{x}{y}{}}{x,y\in \spc{X}}.\]

\item\emph{Radius}\index{Radius} of $\spc{X}$ is  defined as
\[\rad \spc{X}=\inf\set{R>0}{\oBall(x,R)= \spc{X}\ \t{for some}\ x\in\spc{X}}.\]

\item\emph{$\eps$-pack}\index{$\eps$-pack} of $\spc{X}$ is the maximal number  (possibly infinite) of points in $\spc{X}$ at distance $>\eps$ from each other;  it is denoted by $\pack_\eps\spc{X}$.
...
it will be called \emph{length-metric}\index{length-metric} on $\spc{X}$.
If $\yetdist{x}{y}{}=\dist{x}{y}{}$ for any pair of points $x,y\in\spc{X}$  then $\spc{X}$ is called \emph{length space}\index{length space}.

In other words, a metric space $\spc{X}$ is called 
\emph{length space}
...
Let us define few more variations of definitions of length spaces.

\begin{itemize}
\item Assume $R>0$.
If $\yetdist{x}{y}{}=\dist{x}{y}{}$ for any pair $\dist{x}{y}{}<R$ then the space $\spc{X}$ is called \emph{$R$-length space}\index{$R$-length space}.
...
then the space $\spc{X}$ is called \emph{locally length space}\index{locally length space}.
...
Note that $\infty$-length space is the same as length space.
...
which lies in $A$.
If in addition every geodesic from $p$ to $q$ lies in $A$, then $A$ is called \emph{convex}\index{convex set}

If this definition is applied only when  $\dist{p}{q}{}<R$, $R\in (0,\infty]$, 
then $A$ is called  \emph{(weakly) $R$-convex}%
\index{weakly convex set!weakly $R$-convex set}
\index{convex set!weakly $R$-convex set}.
\end{thm}

Note that \emph{(weakly) convex sets} can be also regarded as \emph{(weakly) $\infty$-convex sets}.

Note that inherited metric on a weakly convex set coinsides with its length-metric.
...
Then the  the projections $\iota_i\:\spc{X}_i\to \spc{X}_1\sqcup_{\phi}\spc{X}_2$ are also distance preserving and 
...
\begin{thm}{Exercise}
Let $\spc{X}$ be a complete metric space.
Assume group $G$ acts by on $\spc{X}$ by isometries.
Show that the quotient space $\spc{X}/G$ is complete.
\end{thm}
