...
\parbf{Generalizations.}
We  will often allow infinite values for the distance, but we may still use the term metric space.
If we need to emphasize that distances in a space might take infinite values, we will say \index{$\infty$-metric space}\emph{$\infty$-metric space}
and if we need to emphasize that the distances are always finite we will say \index{genuine metric space}\emph{genuine metric space}.

We will also need \emph{pseudometric} spaces --- those where distances between distinct points can vanish.
The described four types of generalized metric spaces can be distinguished by the interval of allowed values for distance between distinct points:
\begin{itemize}
\item a genuine metric is $(0,\infty)$-valued, 
\item an $\infty$-metric is  $(0,\infty]$-valued, 
\item a pseudometric might be $[0,\infty)$-valued or $[0,\infty]$-valued, depending on the context.
\end{itemize}
For example, if we need to be precise, we may say $[0,\infty)$-valued metric instead of pseudometric.

Metric spaces are the main object of study in this book.
...
\end{matrix}
\]
The obtained $\infty$-metric space $\bm{X}$ will be called the \emph{disjoint union}\index{disjoint union of metric spaces} of $\{\spc{X}_\alpha\}_{\alpha\in\IndexSet}$ and  denoted as \[\bigsqcup_{\alpha\in\IndexSet}\spc{X}_\alpha.\]
\end{thm}
...
\parbf{Corresponing metric space and metric component.}
...
It follows that any metric space is a \emph{disjoint union} of its metric components in the sense defined above; see Definition~\ref{def:disjoint-union}
...
The following definition will be applied only to geodesic spaces,
despite it makes sense for all metric spaces.
...
if for every two points $p,q\in A$ which can be connected by a geodesic,
there is a geodesic $[pq]$ of $\spc{X}$ 
which lies in $A$.
...
If these definitions are applied only for pairs of points such that $\dist{p}{q}{}<R$ and some fixed $R\in (0,\infty]$ and only for the geodesics of length $<R$
