...
Often we allowing the infinite value for the distance, but we may still use the term metric space.
If we need to emphasize that distances in the space might take infinite value, we say \index{$\infty$-metric space}\emph{$\infty$-metric space}
and if we need to emphasize that the distances are always finite we say \index{genuine metric space}\emph{genuine metric space}.

We will also need pseudometric spaces --- those where distance between distinct points can vanish.
...
\begin{itemize}
\item genuine metric is $(0,\infty)$-valued, 
\item $\infty$-metric is  $(0,\infty]$-valued, 
\item pseudometric might be $[0,\infty)$-valued or $[0,\infty]$-valued, depending on the context.
\end{itemize}
For example, if we need to be precise, we may say $[0,\infty)$-valued metric instead of pseudometric.

The metric spaces are main objects of study in this book.
