...
\end{itemize}
The value $\rho(x,y)$ is also called \index{distance}\emph{distance} between $x$ and $y$.
...
the interval should be guessed from the context, but by default metric is $(0,\infty)$-valued. 
...
The genuine metric spaces are main objects of study in this book.
...
that is if the metrics $\dist{}{}{\alpha}$ are genuine, then $\dist{}{}{}$ has to be a $[0,\infty)$-valued.
...
Often we will not distinguish between $\spc{X}'$ and~$\spc{X}$. 
...
for example there is a sequence of convex discs in $\SS^2$ that converges to the half sphere $\SS^2_+$, which is not convex.
