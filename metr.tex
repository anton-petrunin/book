...
In this chapter we fix conventions and notations. We are assuming that the reader is familiar with basic notions in metric geometry.
...
\item a $[0,\infty)$-valued metric may be called  a \index{genuine pseudometric}\emph{genuine pseudometric}.
\item a $[0,\infty]$-valued metric may be called a \index{pseudometric}\emph{pseudometric} or an \index{$\infty$-pseudometric}\emph{$\infty$-pseudometric}.
\end{itemize}
...
A map between two metric spaces is called an \index{isometry}\emph{isometry} if it is a bijection and preserves distances between points.
...
Now let us give examples showing that vanishing and infinite distance between distinct points can appear naturally and useful in constructions.
...
Let $\spc{X}$ be a set,
let $\spc{Y}$ be a metric space, and  
let $\map\:\spc{X}\to\spc{Y}$ be a map.
...
The equivalence class of a point $x\in\spc{X}$ will be called the \index{metric component}\emph{metric component} 
 of $x$; it will be denoted by $\spc{X}_x$\index{ $\spc{X}_x$ (metric component)}.
...
\end{align*}
are called the \index{open ball}\emph{open} and the \index{closed ball}\emph{closed  balls} of radius $R$ with center $x$, respectively.

If we need to emphasize that these balls are taken in the space $\spc{X}$,
we write\index{$\oBall(x,R)_{\spc{X}}$ (open ball)}\index{$\cBall[x,R]_{\spc{X}}$ (closed ball)}
...
and may write 
\[\oBall(R)_{\Lob{m}{\kappa}}\quad\text{and}\quad\cBall[R]_{\Lob{m}{\kappa}},\]
respectively.
...
let us denote the distance from $A$ to a point $x$ in $\spc{X}$ by $\distfun{A}{x}$\index{$\distfun{A}{x}$ (distance)};
that is,
\[\distfun{A}{x}
\df
\inf\set{\dist{a}{x}{}}{a\in A}.\] 

For any subset $A\subset\spc{X}$, the sets \index{$\oBall(A,R)$ (open $R$-neighborhood)}\index{$\cBall[A,R]$ (closed $R$-neighborhood)}
\begin{align*}
\oBall(A,R)&=\set{y\in \spc{X}}{ \distfun{A}{y}{}<R},
\\
\cBall[A,R]&=\set{y\in \spc{X}}{\distfun{A}{y}{}\le R}
\end{align*}
are called the  \index{open $R$-neighborhood}\emph{open} and \index{closed $R$-neighborhood}\emph{closed $R$-neighborhoods} of $A$, respectively.
...
Let $\{\Omega_n\}$, $n\in \NN$, be a collection of open dense subsets of complete metric space $\spc{X}$.
...
\begin{subthm}{}
$\spc{X}$ is \index{sequentially compact}\emph{sequentially compact}; that is, any sequence of points in $\spc{X}$ contains a convergent subsequence;
\end{subthm}

\begin{subthm}{}
$\spc{X}$ is complete and for any $\eps>0$ there is a \index{$\eps$-net}\emph{finite $\eps$-net} in $\spc{X}$; that is, there is a finite collection of points $p_1,\ldots,p_{N}$ such that $\bigcup_i\oBall(p_i,\eps)=\spc{X}$.
\end{subthm}
...
A \index{curve}\emph{curve} in a metric space $\spc{X}$ is a continuous map $\alpha\:\II\to \spc{X}$, where $\II$ is a {}\emph{real interval} (that is, an arbitrary convex subset of $\RR$).
...
it will be called the \index{induced length metric}\emph{induced length metric} on $\spc{X}$.
Clearly 
...
If $\yetdist{x}{y}{}=\dist{x}{y}{}$ for any pair of points $x,y$ in a metric space $\spc{X}$, then $\dist{}{}{}$ is called \index{length metric}\emph{length metric}, and $\spc{X}$ is called a \index{length space}\emph{length space}.
\end{thm}
...
\item
If any point in $\spc{X}$ admits a neighborhood $\Omega$ such that $\yetdist{x}{y}{}=\dist{x}{y}{}$ for any pair of points $x,y\in \Omega$, then $\spc{X}$ is called a \index{length space!locally length space}\emph{locally length space}.
\item
A metric space is called \index{geodesic space}\emph{geodesic} if for any two points $x,y$ with $\dist{x}{y}{}<\infty$ there is a geodesic $[x y]$ in $\spc X$.
\item
Assume $R>0$. A metric space is called 
\index{$R$-geodesic space}\emph{$R$-geodesic}
if for any two points $x,y$ such that $\dist{x}{y}{}<R$ there is a geodesic $[x y]$ in $\spc X$.
\end{itemize}
...
Let $\spc{X}$ be a metric space,
and let $\yetdist{}{}{}$ be the length metric on it.
Show the following:
...
\begin{enumerate}[(i)]
\item A point $z\in \spc{X}$ is called a \index{midpoint}\emph{midpoint} between $x$ and $y$ if 
...
The following lemma was essentially proved by Karl Menger \cite[Section 6]{menger}.
...
Suppose that for any two distinct points $x$ and $y$ in a complete metric space $\spc{X}$ there is yet another point  $z$ such that 
\[
\dist xz{}+\dist yz{}=\dist xy{}.
\]
Show that $\spc{X}$ is geodesic.
\end{thm}
...
Indeed, assume the contrary; that is, $\rho(x)+\dist{x}{y}{}<\rho(y)$ for $x,y\in \spc{X}$. 
Then 
$\cBall[x,\rho(x)+\eps]$ is a closed subset of $\cBall[y,\rho(y)]$ for $\eps>0$.
...
Let $\spc{X}$ be a geodesic space, and let $A\subset\spc{X}$.
...
or \emph{totally $R$-convex}, respectively.
...
\begin{thm}{Proposition}\label{prop:weak-convex-stable}% V: Should this Proposition be moved to Chapter 3???
...
If for the projections $\spc{X}\to(\spc{X}/{\sim},\rho_\alpha)$ are \index{short map}\emph{short} (that is, \textit{distance non-increasing}), then so is $\spc{X}\to(\spc{X}/{\sim},\rho)$.
...
Let $\sim$ be an equivalence relation on a length space~$\spc{X}$.
Then $\spc{X}/{\sim}$ is a length space.
\end{thm}
...
$(\spc{X}_1\sqcup\spc{X}_2)/{\sim}$ will be called the \index{gluing}\emph{gluing of $\spc{X}$ and $\spc{Y}$ along $\phi$} and is denoted by
...
and let $A\subset \spc{V}$ be a closed subset.
...
\item There is an isometric involution of $\spc{W}$ which is called a \index{reflection}\emph{reflection in $A$};
it will be denoted by $x\mapsto x'$.
...
Moreover, $a=a'$ $\Longleftrightarrow$ $a\in A$.
...
Given $x\in \spc{W}$, let $\bar x=x$ if $x\in \spc{V}$,
...
let us denote by $\Bnd(\spc{X},\RR)$\index{ $\Bnd(\spc{X},\RR)$ (space of bounded functions)} the space of all bounded functions on $\spc{X}$ equipped with the sup-norm
