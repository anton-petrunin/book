...
which lies in $A$.
If in addition every geodesic from $p$ to $q$ lies in $A$, then $A$ is called \emph{totally convex}\index{totally convex set}

If this definition is applied only when  $\dist{p}{q}{}<R$, $R\in (0,\infty]$, 
then $A$ is called correspondingly \emph{weakly or totally $R$-convex}%
\index{weakly convex set!weakly $R$-convex set}
\index{convex set!$R$-convex set}.
\end{thm}
