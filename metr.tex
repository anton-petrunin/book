...
$(\spc{X}_1\sqcup\spc{X}_2)/{\sim}$ will be called \index{gluing}\emph{gluing} of $\spc{X}$ and $\spc{Y}$ along $\phi$ and denoted as
...
A metric space $\spc{W}$ glued from two copies of $\spc{V}$ along $A$ is called \index{doubling}\emph{doubling of $\spc{V}$ in $A$}.
...
The map $\kur_p$ will be called \index{Kuratowski map}\emph{Kuratowski map at $p$}.
