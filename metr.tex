...
\\
\cBall[x,R]&=\set{y\in \spc{X}}{ \dist{x}{y}{}\le R}%V: I suggest using just square brackets  or just a bar over B for closed balls. Using both is an overkill.
\end{align*}
...
Since in the model space $\Lob{m}{\kappa}$ all balls of the same radiuses are isometric;
...
\parbf{G-delta sets.}
Recall that an arbitrary union of open balls in a metric space is called \index{open set}\emph{open set}.
A subset of a metric space is called a G-delta set if it can be presented as an intersection of countable number of open subsets. 
...
\begin{enumerate}[(i)]
\item A point $z\in \spc{X}$ is called a \index{midpoint}\emph{midpoint} of $x$ and $y$% V: the standard phrase in English is "midpoint between $x$ and $y$".  I would suggest switching to that everywhere.
if 
\[\dist{x}{z}{}=\dist{y}{z}{}=\tfrac12\cdot\dist[{{}}]{x}{y}{}.\]
\item Assume $\eps\ge 0$.
A point $z\in \spc{X}$ is called  an \index{$\eps$-midpoint}\emph{$\eps$-midpoint} of $x$ and $y$
if 
...
We say that $A$ is \index{totally convex}\emph{totally convex}
...
and \index{totally convex set!totally $R$-convex set}\emph{totally  $R$-convex}.
...
\begin{itemize}
\item The notion of \index{convex set}\index{weakly convex set}\emph{(weakly) convex set} is the same as {}\emph{(weakly) $\infty$-convex set}.
...
Let $\spc{X}_n,$ be a sequence of geodesic spaces. Let $\o$ be an ultrafilter on $\mathbb N$.%V: ultrafilers and ultralimits are only defined later in the book. should this be mentioned here?
...
that is  $\pi(\oBall(p,r))=\oBall(\pi(p),r)$ for any $p\in spc{X}, r>0$.  %$\spc{X}\to\spc{X}/G$ satisfies  is short and 1-co-Lipschitz. %%%V: many readers will not know what co-Lipschitz means.
\end{thm}
...
$(\spc{X}_1\sqcup\spc{X}_2)/{\sim}$ will be called the \index{gluing}\emph{gluing} of $\spc{X}$ and $\spc{Y}$ along $\phi$ and denoted as
...
A metric space $\spc{W}$ glued from two copies of $\spc{V}$ along $A$ is called the \index{doubling}\emph{doubling of $\spc{V}$ in $A$}.% V:  "doubling in $A$" sounds weird to me in English. I think "doubling along $A$" sounds much better. I would suggest switching the terminology.
