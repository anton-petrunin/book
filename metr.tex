...
The equivalence class of a point $x\in\spc{X}$ will be called the \index{metric component}\emph{metric component} 
 of $x$; it will be denoted by $\spc{X}_x$\index{ $\spc{X}_x$}.
...
we write\index{$\oBall(x,R)_{\spc{X}}$}\index{$\cBall[x,R]_{\spc{X}}$}
...
let us denote the distance from $A$ to a point $x$ in $\spc{X}$ by $\distfun{A}{x}$\index{$\distfun{A}{x}$};
that is,
\[\distfun{A}{x}
\df
...
let us denote by $\Bnd(\spc{X},\RR)$\index{ $\Bnd(\spc{X},\RR)$}  the space of all bounded functions on $\spc{X}$ equipped with the sup-norm
