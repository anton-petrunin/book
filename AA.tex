...
We can not not fight with this stupid convension.
???
...
where the infimum is taken for all all countable collection of $\vol^n$-measurable sets $V_i\subset \EE^n$
...
In particular, for any $p\in\spc{L}$, the function $\dist[2]{p}{}{}$ as well as the the restriction $\dist{p}{}{}|\spc{L}\backslash\{p\}$ are multidistance subfunctions,
...
Let us define the the $\kay$-jacobian of $h$ with respect to $g$ as supremum
...
and $\hat\beta(s_0)=\hat\alpha(s_0)$ for some some $s_0\in\II$ 
...
Since $f$ is ???-semicontinuous, the set $Q$ is is closed relatively subset in $\Dom f$.
...
exists by by corollary~\ref{cor:convex-nbhd}.} and
...
Therefore, for for all $i$ and all large $n$, we have $\angk\kappa {x_n}{a_i}{y_n}>\tfrac\pi2-\eps$.
...
\r.
$$
Note however, that the the map $(\kappa,t)\mapsto \phi_\kappa(t)$ is continuous but not analiytic 
