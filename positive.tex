...
Note that $\spc{L}\subset \cBall[z,\tfrac\pi2]$, otherwise if $y\in \spc{L}$ with $\dist{y}{z}{}>\tfrac\pi2$, then appling inequality $f''+f\le 0$ to the geodesic $[z y]$, we get $(\d_z f)(\dir z y)>0$; that is, $z$ is not a maximum of $f$.
...
According to doubling theorem ???, $\spc{L}\in\CBB21$ is a compact two-dimensional length $\Alex1$ space and its boundary consists of copies of side $[ac]$.
